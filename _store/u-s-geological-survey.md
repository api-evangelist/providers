---
aid: u-s-geological-survey
url: https://raw.githubusercontent.com/api-search/u-s-geological-survey/refs/heads/main/apis.yml
apis:
- aid: u-s-geological-survey:asset-identifier-service-ais
  name: Asset Identifier Service (AIS)
  tags: []
  humanURL: https://www.usgs.gov/tools/asset-identifier-service-ais
  properties:
  - url: https://www.usgs.gov/tools/asset-identifier-service-ais
    type: Documentation
  description: The USGS Asset Identifier Service (AIS) allows USGS personnel to reserve, register, publish, and manage USGS persistent identifiers to make our research more Findable, Accessible, Interoperable, and Reusable (FAIR). AIS includes separate modules for unique identifier types including the following:.
- aid: u-s-geological-survey:seismic-design-web-service
  name: Seismic Design Web Service
  tags: []
  humanURL: https://earthquake.usgs.gov/ws/designmaps/
  properties:
  - url: https://earthquake.usgs.gov/ws/designmaps/
    type: Documentation
  description: Web services produced by the U.S. Geological Survey for calculating parameter values from various seismic design reference documents.
- aid: u-s-geological-survey:real-time-notifications-feeds-and-web-services
  name: Real-Time Notifications, Feeds, and Web Services
  tags: []
  humanURL: https://earthquake.usgs.gov/earthquakes/feed/
  properties:
  - url: https://earthquake.usgs.gov/earthquakes/feed/
    type: Documentation
  description: Real-time Notifications, Feeds, and Web Services.
- aid: u-s-geological-survey:sciencebase
  name: ScienceBase
  tags: []
  humanURL: https://www.usgs.gov/tools/sciencebase
  properties:
  - url: https://www.usgs.gov/tools/sciencebase
    type: Documentation
  description: ScienceBase is a Trusted Digital Repository (TDR) in the U.S. Geological Survey (USGS). The platform is developed and maintained by the USGS Science Analytics and Synthesis (SAS) Science Data Management (SDM) team to provide both permission-controlled and public access to scientific data products and bureau resources.
- aid: u-s-geological-survey:science-data-catalog
  name: Science Data Catalog
  tags: []
  humanURL: https://www.usgs.gov/tools/science-data-catalog
  properties:
  - url: https://www.usgs.gov/tools/science-data-catalog
    type: Documentation
  description: The Science Data Catalog (SDC) is the official public and searchable index that aggregates descriptions of all public research data that have been published by the USGS. Federal research institutions are required by the Office of Management and Budget (OMB) to provide a publicly searchable index of their published research data.
- aid: u-s-geological-survey:model-catalog
  name: Model Catalog
  tags: []
  humanURL: https://www.usgs.gov/tools/model-catalog
  properties:
  - url: https://www.usgs.gov/tools/model-catalog
    type: Documentation
  description: The USGS Model Catalog compiles and connects data, software, and publications about scientific models developed by the U.S. Geological Survey (USGS) or developed by external organizations and used in USGS investigations.
- aid: u-s-geological-survey:web-service-for-geomagnetism-data
  name: Web Service for Geomagnetism Data
  tags: []
  humanURL: https://www.usgs.gov/tools/web-service-geomagnetism-data
  properties:
  - url: https://www.usgs.gov/tools/web-service-geomagnetism-data
    type: Documentation
  description: Web Service for Geomagnetism Data.
- aid: u-s-geological-survey:gbif-us
  name: GBIF-US
  tags: []
  humanURL: https://www.usgs.gov/tools/gbif-us
  properties:
  - url: https://www.usgs.gov/tools/gbif-us
    type: Documentation
  description: The Science Analytics and Synthesis Program (SAS) of the U.S. Geological Survey (USGS) manages the U.S. Node to GBIF, GBIF-US, to facilitate access to species occurrence data (presence and absence), analyses and investigations of species trends, drivers of change, and potential implications of future changes in biodiversity in the United States and U.S. Territories.
- aid: u-s-geological-survey:water-data-labs
  name: Water Data Labs
  tags: []
  humanURL: https://www.usgs.gov/tools/water-data-labs
  properties:
  - url: https://www.usgs.gov/tools/water-data-labs
    type: Documentation
  description: Water Data Labs is an experimental site that exposes tools and technologies in development by the USGS Water Data for the Nation software team. Here, users can explore and test experimental products, services, tools, and visualizations.Media.
- aid: u-s-geological-survey:usgs-water-services
  name: USGS Water Services
  tags: []
  humanURL: https://www.usgs.gov/tools/usgs-water-services
  properties:
  - url: https://www.usgs.gov/tools/usgs-water-services
    type: Documentation
  description: The USGS Water Services site serves USGS water data (streamflow, groundwater, water quality, site information, and statistics) via automated means using web services and extensible markup language (XML), as well as other popular media types. Services are invoked with the REST protocol. These services designed for high fault tolerance and very high availability.
- aid: u-s-geological-survey:goa-dashboard
  name: GOA - Dashboard
  tags: []
  humanURL: https://www.usgs.gov/tools/goa-dashboard
  properties:
  - url: https://www.usgs.gov/tools/goa-dashboard
    type: Documentation
  description: The USGS Southeast Regional Office has funded a cross-center collaboration between the Wetland and Aquatic Research Center and the Texas Water Science Center for the development of the Gulf of America Water Dashboard.
- aid: u-s-geological-survey:streamstats-web-services
  name: StreamStats Web Services
  tags: []
  humanURL: https://www.usgs.gov/tools/streamstats-web-services
  properties:
  - url: https://www.usgs.gov/tools/streamstats-web-services
    type: Documentation
  description: The StreamStats application uses data web services that were created for it. When in use, the application manages interactions between the user and the services. However, the web services can be directly accessed using the StreamStats Service API documented here or consumed by a custom client application using HTTP protocols.
- aid: u-s-geological-survey:wapato-lake-interactive-lake-level-tool
  name: Wapato Lake Interactive Lake Level Tool
  tags: []
  humanURL: https://www.usgs.gov/tools/wapato-lake-interactive-lake-level-tool
  properties:
  - url: https://www.usgs.gov/tools/wapato-lake-interactive-lake-level-tool
    type: Documentation
  description: The water level of Wapato Lake dictates the size (surface area) of the lake and the amount of water that it stores. This web page allows the user to interactively explore Wapato Lake water level data and how the water level translates into lake area and volume.
- aid: u-s-geological-survey:seismic-design-web-service
  name: Seismic Design Web Service
  tags: []
  humanURL: https://earthquake.usgs.gov/ws/designmaps/
  properties:
  - url: https://earthquake.usgs.gov/ws/designmaps/
    type: Documentation
  description: Web services produced by the U.S. Geological Survey for calculating parameter values from various seismic design reference documents.
- aid: u-s-geological-survey:earthquake-notifications-feeds-and-web-services
  name: Earthquake Notifications, Feeds, and Web Services
  tags: []
  humanURL: https://www.usgs.gov/tools/earthquake-notifications-feeds-and-web-services
  properties:
  - url: https://www.usgs.gov/tools/earthquake-notifications-feeds-and-web-services
    type: Documentation
  description: Know about earthquakes just after they happen in real-time via txt message, email, or social media. Plus, get everything you might need to add these earthquake alerts into your own web applications.
- aid: u-s-geological-survey:sciencebase
  name: ScienceBase
  tags: []
  humanURL: https://www.usgs.gov/tools/sciencebase
  properties:
  - url: https://www.usgs.gov/tools/sciencebase
    type: Documentation
  description: ScienceBase is a Trusted Digital Repository (TDR) in the U.S. Geological Survey (USGS). The platform is developed and maintained by the USGS Science Analytics and Synthesis (SAS) Science Data Management (SDM) team to provide both permission-controlled and public access to scientific data products and bureau resources.
- aid: u-s-geological-survey:streamstats
  name: StreamStats
  tags: []
  humanURL: https://www.usgs.gov/streamstats
  properties:
  - url: https://www.usgs.gov/streamstats
    type: Documentation
  description: StreamStats provides access to spatial analytical tools that are useful for water-resources planning and management, and for engineering and design purposes. The map-based user interface can be used to delineate drainage areas, get basin characteristics and estimates of flow statistics, and more. Available information varies from state to state.
- aid: u-s-geological-survey:joint-ecosystem-modeling-jem-support
  name: Joint Ecosystem Modeling (JEM) Support
  tags: []
  humanURL: https://www.usgs.gov/tools/joint-ecosystem-modeling-jem-support
  properties:
  - url: https://www.usgs.gov/tools/joint-ecosystem-modeling-jem-support
    type: Documentation
  description: The JEM community of practice is focused on ecological modeling and monitoring across the Greater Everglades, with particular interest in habitats, how various factors affect habitat change, and how the organisms dependent on those habitats respond today and into the future.
- aid: u-s-geological-survey:everglades-eco-modeling-data-visualization-and-tool-development
  name: Everglades Eco-Modeling Data Visualization and Tool Development
  tags: []
  humanURL: https://www.usgs.gov/tools/everglades-eco-modeling-data-visualization-and-tool-development
  properties:
  - url: https://www.usgs.gov/tools/everglades-eco-modeling-data-visualization-and-tool-development
    type: Documentation
  description: Working with the Joint Ecosystem Modeling (JEM) community of practice, the WARC Advanced Applications Team developed and maintains the EverVIEW Data Viewer desktop visualization platform, which allows users to easily visualize and inspect standards-compliant NetCDF modeling data and has experienced tremendous feature growth driven by user feedback.
- aid: u-s-geological-survey:coastal-information-management-system-cims
  name: Coastal Information Management System (CIMS)
  tags: []
  humanURL: https://www.usgs.gov/tools/coastal-information-management-system-cims
  properties:
  - url: https://www.usgs.gov/tools/coastal-information-management-system-cims
    type: Documentation
  description: WARCs Advanced Applications Team is responsible for data management and application development to support the biological monitoring components of coastal restoration projects in the Louisiana Coastal Protection and Restoration Authority portfolio.
- aid: u-s-geological-survey:integrated-taxonomic-information-system-itis
  name: Integrated Taxonomic Information System (ITIS)
  tags: []
  humanURL: https://www.usgs.gov/tools/integrated-taxonomic-information-system-itis
  properties:
  - url: https://www.usgs.gov/tools/integrated-taxonomic-information-system-itis
    type: Documentation
  description: The ITIS mission is to communicate a comprehensive taxonomy of global species that enables biodiversity information to be discovered, indexed, and connected across all human endeavors. Use the ITIS search, download, API, and web tools as a standardized reference of scientific names and their taxonomic hierarchy.
- aid: u-s-geological-survey:species-of-greatest-conservation-need-analysis-tool
  name: Species of Greatest Conservation Need Analysis Tool
  tags: []
  humanURL: https://www.usgs.gov/tools/species-greatest-conservation-need-analysis-tool
  properties:
  - url: https://www.usgs.gov/tools/species-greatest-conservation-need-analysis-tool
    type: Documentation
  description: Species of Greatest Conservation Need are lists of species designated in the 56 State Wildlife Action Plans which identify the species most in need of conservation action in that state or U.S. territory. USGS developed a process that allows these lists to be compiled into a national list, making their representation across taxa groups available via the Species Conservation Analysis Tool.
- aid: u-s-geological-survey:national-vegetation-classification-standard
  name: National Vegetation Classification Standard
  tags: []
  humanURL: https://www.usgs.gov/tools/national-vegetation-classification-standard
  properties:
  - url: https://www.usgs.gov/tools/national-vegetation-classification-standard
    type: Documentation
  description: The National Vegetation Classification (NVC) is your guide to inventorying plant communities. The standard allows users to produce uniform statistics about vegetation resources across the nation. CSAS.
- aid: u-s-geological-survey:arkansas-wateruse
  name: Arkansas Wateruse
  tags: []
  humanURL: https://www.usgs.gov/tools/arkansas-wateruse
  properties:
  - url: https://www.usgs.gov/tools/arkansas-wateruse
    type: Documentation
  description: Since 1985 site-specific water-use data for several categories have been collected and compiled annually by the Arkansas Natural Resources Commission (ANRC) in cooperation with the USGS.Data for the irrigation and livestock ( animal specialties ) categories are reported through the Conservation District Offices in each county.
name: U.S. Geological Survey
tags:
- Federal Government
- Geological
- Mining
- Natural Resources
type: Contract
image: https://kinlane-productions.s3.amazonaws.com/apis-json/apis-json-logo.jpg
access: 3rd-Party
common: []
created: '2024-11-14'
modified: '2026-04-07'
position: Consuming
description: The U.S. Geological Survey (USGS) is a scientific agency of the U.S. government that conducts research and provides data on the natural resources and hazards of the United States. The USGS is known for its work in mapping and monitoring earthquakes, volcanoes, and landslides to help mitigate risks and protect communities. They also study water resources, including monitoring groundwater levels and water quality, which is essential for managing and conserving water supplies.
maintainers:
- FN: Kin Lane
  email: info@apievangelist.com
specificationVersion: '0.19'
---

