---
aid: astronomyapi
url: https://example.com/apis/getting-started-v2-astronomyapi.yml
apis:
  - aid: astronomyapi:getting-started-v2-astronomyapi
    name: Getting Started | v2 | AstronomyAPI
    image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
    humanURL: https://docs.astronomyapi.com/
    overlays: []
    description: >-
      Welcome to Astronomy API, a web API for retrieving astronomical
      information. ~ https://docs.astronomyapi.com/
name: AstronomyAPI
tags:
  - Astronomy
type: Contract
image: >-
  https://kinlane-productions2.s3.amazonaws.com/apis-json-icons/getting-started-v2-astronomyapi.png
access: 3rd-Party
created: 2024/3/30
modified: '2024-12-14'
overlays:
  - url: overlays/apis-io-search.yml
    type: APIs.io Search
position: Consuming
description: >-
  Astronomy API is a programming interface for developers who are interested in
  integrating astronomical data in to their applications.
maintainers:
  - FN: API Evangelist
    email: info@apievangelist.com
specificationVersion: '0.16'

---