---
aid: apis-io
url: https://github.com/api-evangelist/search-api/blob/main/apis.yml
apis:
  - aid: apis-io:search-api
    name: APIs.io Search API
    tags:
      - APIs
      - Search Engine
      - Directory
    image: https://kinlane-productions2.s3.amazonaws.com/apis-io/apis-io-api-logo.jpg
    baseURL: https://search-api.apis.io
    contact:
      - FN: APIs.io
        email: info@apis.io
    humanURL: https://developer.apis.io/documentation
    properties:
      - url: https://github.com/api-evangelist/search-api/blob/main/team.yml
        type: Teams
      - url: https://github.com/api-evangelist/search-api
        type: GitHubRepository
      - url: >-
          https://github.com/api-evangelist/search-api/blob/main/.github/workflows/pipeline.yml
        type: GitHubActions
      - url: https://developer.apis.io/documentation/
        type: Documentation
      - url: https://github.com/api-evangelist/search-api/blob/main/openapi.yml
        type: OpenAPI
      - url: >-
          https://www.postman.com/api-evangelist/apis-io-api-evangelist-engine/collection/hn8xpmd/apis-io-search-api
        type: PostmanCollection
      - url: >-
          https://us-east-1.console.aws.amazon.com/apigateway/main/apis/ibmwu99rx3/stages
        type: Gateway
      - url: >-
          https://www.postman.com/api-evangelist/apis-io-api-evangelist-engine/environment/35240-f6035e55-e9f8-41b8-a2d8-b7ac4cd127bb/production
        type: StagingEnvironment
      - url: >-
          https://www.postman.com/api-evangelist/apis-io-api-evangelist-engine/environment/35240-f6035e55-e9f8-41b8-a2d8-b7ac4cd127bb/production
        type: GatewayEnvironment
      - url: https://github.com/api-evangelist/search-api/labels/change-log
        type: ChangeLog
      - url: https://github.com/api-evangelist/search-api/labels/road-map
        type: RoadMap
      - url: https://github.com/api-evangelist/search-api/blob/main/use-cases.yml
        type: UseCases
      - url: >-
          https://www.postman.com/api-evangelist/apis-io/monitor/APIs-io-Search---Status1ef6bc29-9da9-4040-b98b-cef03be1155e
        type: Status
      - url: >-
          https://www.postman.com/api-evangelist/apis-io/monitor/APIs-io-Search---Performance1ef6bc38-c969-4bd0-815b-25b7d1b1a503
        type: Performance
    description: >-
      This is the API to access APIs.io API search engine, from the product
      management perspective.
name: APIs.io Search API
tags:
  - APIs
  - Search Engine
  - Directory
type: Contract
image: https://kinlane-productions2.s3.amazonaws.com/apis-io/apis-io-api-logo.jpg
common:
  - url: https://github.com/api-evangelist/
    type: GitHubOrganization
  - url: >-
      https://www.postman.com/api-evangelist/apis-io-api-evangelist-engine/overview
    type: PostmanPublicWorkspace
  - url: https://developer.apis.io
    type: Portals
  - url: https://developer.apis.io/getting-started/
    type: GettingStarted
  - url: https://developer.apis.io/plans/
    type: Plans
  - url: https://developer.apis.io/authentication/
    type: Authentication
  - url: https://developer.apis.io/road-map/
    type: RoadMap
  - url: https://developer.apis.io/change-log/
    type: ChangeLog
  - url: https://developer.apis.io/versioning/
    type: Versioning
  - url: https://developer.apis.io/sdks/
    type: SDKs
  - url: https://developer.apis.io/blog/
    type: Blogs
  - url: https://developer.apis.io/atom.xml
    type: BlogFeeds
  - url: https://developer.apis.io/videos/
    type: Videos
  - url: https://developer.apis.io/support/
    type: Support
  - url: https://github.com/orgs/api-evangelist/discussions
    type: Forums
  - url: >-
      https://github.com/api-evangelist/search-api/issues?q=is%3Aissue+is%3Aopen+label%3Asupport
    type: SupportGitHubIssues
  - url: info@apievangelist.com
    type: SupportEmail
  - url: >-
      https://github.com/api-evangelist/search-api/issues?q=is%3Aissue+is%3Aopen+label%3Afeedback
    type: FeedbackGitHubIssues
  - url: info@apievangelist.com
    type: FeedbackEmail
  - url: >-
      https://github.com/api-evangelist/search-api/issues?q=is%3Aissue+is%3Aopen+label%3Aquestion
    type: QuestionsGitHubIssues
  - url: https://github.com/api-evangelist/policies/blob/main/policies.yml
    type: Policies
  - url: https://github.com/api-evangelist/rules/blob/main/operational-rules.yml
    type: OperationalRules
  - url: https://github.com/api-evangelist/rules/blob/main/api-rules.yml
    type: ApiRules
  - url: https://github.com/api-evangelist/lifecycle/blob/main/lifecycle.yml
    type: Lifecycle
  - url: https://github.com/api-evangelist/vocabulary/blob/main/vocabulary.yml
    type: Vocabulary
  - url: https://developer.apis.io/terms-of-service/
    type: TermsOfService
  - url: https://developer.apis.io/privacy-policy/
    type: PrivacyPolicy
  - url: https://developer.apis.io/licensing/
    type: InterfaceLicense
  - url: >-
      https://api-evangelist-tools.github.io/apis-io-search-api-operational-validator/
    type: OperationalValidation
  - url: https://api-evangelist-tools.github.io/apis-io-search-api-api-validator/
    type: APIValidation
created: '2024-08-20'
modified: '2024-09-23'
description: >-
  This is the contract for the APIs.io search API, allowing for programmatic
  search of the APIs in the index.
maintainers:
  - FN: Kin Lane
    email: info@apievangelist.com
    X-github: kinlane
specificationVersion: '0.18'

---