#!/usr/bin/env python3
"""Emit the two interactive market tools for every listing page that sells a report.

A market report has a buy side and a sell side and both arrive on the same page.
Each ranked listing that promotes a Trend Report gets:
...
REPORTS_BASE = "https://reports.apievangelist.com/reports/"
...
def tools_for(section_url, data_key, report_slug, stats):
    ...
    report_url = REPORTS_BASE + report_slug + "/"
    return [
        {
            "side": "For buyers",
...
#
# The body is _includes/tool-%(kind)s.html, shared by every market that sells a
# Trend Report, and it renders from site.data[data_key] — the same file the ranked
# listing on %(section_url)s renders from. The site-wide `permalink` pattern would
...
def primary_report(fm):
    """The report a section leads with — `paper:` (one) or the first of `papers:`."""
    if fm.get("paper"):
        return fm["paper"]
...
def main():
    ...
    for root, dirs, files in os.walk(SITE):
        ...
        report = primary_report(fm)
        data_key = fm.get("data_key")
        if not report or not data_key or not report.get("slug"):
            continue
        ...
        tools = tools_for(section_url, data_key, report["slug"], stats)
        ...
        for kind, tool in (("priorities", tools[0]), ("opportunities", tools[1])):
            ...
            write(os.path.join(root, name), STUB % {
                ...
                "report_title": esc(report.get("title") or ""),
                "report_url": tool["report"],
                "section": section_url,
            })
