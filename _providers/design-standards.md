---
access_model:
  confidence: medium
  label: Freemium
  onboarding: unknown
  pricing: freemium
  public: false
  source:
  - plans
  trial: false
  try_now: false
agent_readiness:
  band: human-only
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: false
    auth_clarity: false
    consent_identity: false
    dry_run_mode: false
    error_semantics: false
    event_surface_described: false
    idempotency: false
    mcp_server: false
    openapi_examples: false
    rate_limit_signal: documented
    spec_presence: false
    well_known_catalog: false
  schema_version: 0.2
  score: 3.2
  scored_at: '2026-08-17'
api_count: 1
apis:
- description: Rules, capabilities, vocabulary, and linked-data description for an organizational design standards program covering products, interfaces, and APIs.
  name: Design Standards Program
  slug: program
artifact_total: 6
common:
- group: docs
  title: ''
  type: Reference
  url: https://opensource.zalando.com/restful-api-guidelines/
- group: docs
  title: ''
  type: Reference
  url: https://www.w3.org/TR/WCAG21/
- group: build
  title: ''
  type: GitHubOrganization
  url: https://github.com/api-evangelist
created: '2025-01-01'
description: Design Standards encompasses the frameworks, guidelines, and best practices used in product, interface, and system design to ensure consistency, usability, and accessibility. Includes API design guidelines, naming and format conventions, accessibility conformance, deprecation policy, and governance practices.
finops:
- name: Design Standards Finops
  service_category: API
  slug: design-standards-finops
image: https://kinlane-images.s3.amazonaws.com/shared/apis-json/icons/design-standards.png
jsonld:
- class_count: 0
  name: Design Standards Context
  property_count: 0
  slug: design-standards
layout: provider
modified: '2026-04-28'
name: Design Standards
nav: Providers
network: true
overview: 'Design Standards publishes 1 API on the [APIs.io](https://apis.io/) network. Tagged areas include Accessibility, Design Standards, Design Systems, UX, and API Guidelines.


  The Design Standards catalog on APIs.io includes 1 JSON-LD context and 1 Spectral governance ruleset.'
plans:
- name: Design Standards Plans Pricing
  plan_count: 3
  slug: design-standards-plans-pricing
random_paper: 4
rate_limits:
- limit_count: 5
  name: Design Standards Rate Limits
  slug: design-standards-rate-limits
rules:
- name: Design Standards API Rules
  rule_count: 0
  severity_counts:
    error: 0
    hint: 0
    info: 0
    warn: 0
  slug: design-standards-rules
score:
  band: emerging
  composite: 14.4
  delta: 0.0
  facets:
    commercial_clarity: 15.8
    contract_quality: 8.1
    developer_ergonomics: 6.5
    discoverability: 50.0
    governance: 10.4
    operational_transparency: 13.2
  previous_composite: 14.4
  schema_version: 0.11.0
  scored_at: '2026-08-17'
  trend: flat
screenshot: https://raw.githubusercontent.com/api-evangelist/design-standards/refs/heads/main/screenshots/design-standards-2026-06-20T175933.png
slug: design-standards
tags:
- Accessibility
- Design Standards
- Design Systems
- UX
- API Guidelines
- Governance
---
