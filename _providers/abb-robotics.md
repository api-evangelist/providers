...
- description: Robot Web Services is the canonical REST/HTTP(S) API exposed by ABB OmniCore (RobotWare 7) and IRC5 controllers. It provides programmatic access to RAPID program data and execution, IO signals, motion
  ...
  slug: robot-web-services
- description: WebSocket-based subscription service that pushes change events for subscribed RWS resources (IO signals, RAPID symbols, controller state, execution state, e-log, motion system). Clients open a subscri
  name: ABB Robot Web Services Subscriptions
  slug: robot-web-services-subscriptions
- description: The PC SDK is a .NET software development kit that lets system integrators, third parties, and end users build custom PC applications and RobotStudio Add-Ins that communicate with an ABB robot control
  name: ABB PC SDK
  slug: pc-sdk
- description: The RobotStudio SDK enables developers to build custom applications, Add-Ins, and SmartComponents that extend RobotStudio. The current version targets RobotStudio 2026 and is built on .NET 10. Used by
  name: ABB RobotStudio SDK
  slug: robotstudio-sdk
artifact_total: 21
common:
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/abb-robotics/refs/heads/main/security/abb-robotics-domain-security.yml
  title: ''
  type: DomainSecurity
...
jsonld:
- class_count: 7
  name: Abb Robotics Context
  property_count: 28
  slug: abb-robotics-context
layout: provider
modified: '2026-05-25'
name: ABB Robotics
nav: Providers
network: true
overview: 'ABB Robotics publishes 4 APIs on the [APIs.io](https://apis.io/) network. Tagged areas include Robotics, Industrial Robotics, Industrial Automation, Manufacturing, and Robot Controllers.
