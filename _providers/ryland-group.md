---
access_model:
  confidence: low
  label: Unknown
  onboarding: unknown
  pricing: unknown
  public: false
  source: []
  trial: false
  try_now: false
agent_readiness:
  band: human-only
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: false
    agentic_commerce: false
    auth_clarity: false
    consent_identity: false
    delegated_identity: false
    dry_run_mode: false
    dynamic_client_registration: false
    error_semantics: false
    event_surface_described: false
    idempotency: false
    mcp_server: false
    openapi_examples: false
    protected_resource_metadata: false
    rate_limit_signal: false
    reversibility_documented: false
    spec_presence: false
    well_known_catalog: false
  schema_version: 0.2
  score: 0.0
  scored_at: '2026-08-30'
api_count: 0
artifact_total: 7
common:
- group: other
  title: ''
  type: ParentCompany
  url: https://apis.io/providers/lennar/
- group: company
  title: ''
  type: LinkedIn
  url: https://www.linkedin.com/company/the-ryland-group
- group: company
  title: ''
  type: Website
  url: https://www.ryland-group.com
- group: company
  title: ''
  type: About
  url: https://en.wikipedia.org/wiki/Ryland_Group
- group: design
  title: ''
  type: Vocabulary
  url: https://raw.githubusercontent.com/api-evangelist/ryland-group/refs/heads/main/vocabulary/ryland-group-vocabulary.yml
- group: design
  title: ''
  type: JSONLDContext
  url: https://raw.githubusercontent.com/api-evangelist/ryland-group/refs/heads/main/json-ld/ryland-group-context.jsonld
- group: docs
  title: ''
  type: JSONSchema
  url: https://raw.githubusercontent.com/api-evangelist/ryland-group/refs/heads/main/json-schema/ryland-home-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: https://raw.githubusercontent.com/api-evangelist/ryland-group/refs/heads/main/json-schema/ryland-mortgage-schema.json
created: '2026-03-24'
description: The Ryland Group was one of the largest U.S. homebuilders and a leading mortgage finance company before merging with Standard Pacific Corp in October 2015 to form CalAtlantic Group, Inc., which was subsequently acquired by Lennar in 2018. During its operation, Ryland Homes built homes across 17 states and provided financial services including consumer mortgage loans, title, and escrow services. The company's technology stack included Akamai, Amazon CloudFront, ASP.NET, Fiserv for financial services, and Microsoft Dynamics for CRM and ERP.
examples:
- key_count: 14
  name: Ryland Home Example
  slug: ryland-home-example
- key_count: 11
  name: Ryland Mortgage Example
  slug: ryland-mortgage-example
image: https://kinlane-images.s3.amazonaws.com/shared/apis-json/icons/ryland-group.png
json_schemas:
- name: Ryland Home
  property_count: 14
  slug: ryland-home
- name: Ryland Mortgage
  property_count: 11
  slug: ryland-mortgage
json_structures:
- name: Ryland Home Structure
  property_count: 0
  slug: ryland-home-structure
jsonld:
- class_count: 0
  name: Ryland Group Context
  property_count: 13
  slug: ryland-group-context
layout: provider
modified: '2026-05-02'
name: Ryland Group
nav: Providers
network: true
overview: 'Ryland Group is profiled on the [APIs.io](https://apis.io/) network. Tagged areas include Financial-Services, Homebuilder, Mortgage, and Real-Estate.


  The Ryland Group catalog on APIs.io includes 1 JSON-LD context and 1 Spectral governance ruleset.'
press:
- date: '2026-05-25'
  title: The Ryland Group 2026 Company Profile
  url: https://pitchbook.com/profiles/company/40361-95
- date: '2026-05-25'
  title: Standard Pacific, Ryland Group to create 4th largest U.S. ...
  url: https://www.constructiondive.com/news/standard-pacific-ryland-group-to-create-4th-largest-us-homebuilder-with/400754/
- date: '2026-05-25'
  title: Ryland Group Inc.
  url: https://www.nytimes.com/topic/company/ryland-group-inc
- date: '2026-05-25'
  title: Ryland Group 'BBB-' Credit Rating Affirmed; Outlook Is Stable
  url: https://www.spglobal.com/ratings/en/regulatory/article/-/view/type/HTML/id/554035
- date: '2026-05-25'
  title: Ryland/Standard Pacific merger complete as trading ...
  url: https://www.constructiondive.com/news/rylandstandard-pacific-merger-complete-as-trading-begins-for-new-calatlant/406642/
random_paper: 13
rules:
- effective_rule_count: 5
  extends: []
  name: Ryland Group API Rules
  rule_count: 5
  severity_counts:
    error: 0
    hint: 0
    info: 2
    warn: 3
  slug: ryland-group-jsonschema-spectral-rules
score:
  band: minimal
  composite: 9.7
  coverage:
    artifact_dirs: 10
    catalog_gap: 76.8
    catalog_max: 115.0
    note: Disclosure, not a penalty. catalog_gap is rubric points API Evangelist could add with no action by this provider; it is our backlog, not their gap, and it is NOT subtracted from the composite above.
  delta: 0.0
  facets:
    access_clarity: 0.0
    commercial_clarity: 0.0
    contract_governance: 25.0
    contract_quality: 10.7
    developer_ergonomics: 0.0
    discoverability: 40.7
    governance: 25.0
    operational_transparency: 0.0
  previous_composite: 9.7
  schema_version: 0.17.2
  scored_at: '2026-08-30'
  trend: flat
slug: ryland-group
tags:
- Financial-Services
- Homebuilder
- Mortgage
- Real-Estate
website: https://www.ryland-group.com
---
