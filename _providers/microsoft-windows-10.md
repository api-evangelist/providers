---
access_model:
  confidence: medium
  label: Paid
  onboarding: unknown
  pricing: paid
  public: false
  source:
  - plans
  trial: false
  try_now: false
agent_readiness:
  band: agent-aware
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: derived
    asyncapi_events: false
    auth_clarity: true
    consent_identity: false
    dry_run_mode: false
    error_semantics: false
    idempotency: false
    mcp_server: false
    openapi_examples: false
    rate_limit_signal: documented
    spec_presence: true
    well_known_catalog: false
  schema_version: 0.2
  score: 30.6
  scored_at: '2026-07-28'
agentic_access:
- acting_count: 56
  human_in_the_loop: 3
  name: Microsoft Windows 10 Agentic Access
  operation_count: 122
  slug: microsoft-windows-10-agentic-access
  summary_line: 122 operations · 56 acting · 3 human-in-the-loop
api_count: 77
apis:
- description: The Accelerometer API from Microsoft Windows 10 — 1 operation(s) for accelerometer.
  name: Microsoft Windows 10 Accelerometer API
  slug: microsoft-windows-10-accelerometer-api
- description: The Access API from Microsoft Windows 10 — 2 operation(s) for access.
  name: Microsoft Windows 10 Access API
  slug: microsoft-windows-10-access-api
- description: The Activation API from Microsoft Windows 10 — 1 operation(s) for activation.
  name: Microsoft Windows 10 Activation API
  slug: microsoft-windows-10-activation-api
- description: The Animations API from Microsoft Windows 10 — 1 operation(s) for animations.
  name: Microsoft Windows 10 Animations API
  slug: microsoft-windows-10-animations-api
- description: The API Categories API from Microsoft Windows 10 — 1 operation(s) for api categories.
  name: Microsoft Windows 10 API Categories API
  slug: microsoft-windows-10-api-categories-api
- description: The App Data API from Microsoft Windows 10 — 1 operation(s) for app data.
  name: Microsoft Windows 10 App Data API
  slug: microsoft-windows-10-app-data-api
- description: The Attestation API from Microsoft Windows 10 — 1 operation(s) for attestation.
  name: Microsoft Windows 10 Attestation API
  slug: microsoft-windows-10-attestation-api
- description: The Authentication API from Microsoft Windows 10 — 2 operation(s) for authentication.
  name: Microsoft Windows 10 Authentication API
  slug: microsoft-windows-10-authentication-api
- description: The Availability API from Microsoft Windows 10 — 1 operation(s) for availability.
  name: Microsoft Windows 10 Availability API
  slug: microsoft-windows-10-availability-api
- description: The Background Transfers API from Microsoft Windows 10 — 1 operation(s) for background transfers.
  name: Microsoft Windows 10 Background Transfers API
  slug: microsoft-windows-10-background-transfers-api
- description: The Badge Notifications API from Microsoft Windows 10 — 1 operation(s) for badge notifications.
  name: Microsoft Windows 10 Badge Notifications API
  slug: microsoft-windows-10-badge-notifications-api
- description: The Barometer API from Microsoft Windows 10 — 1 operation(s) for barometer.
  name: Microsoft Windows 10 Barometer API
  slug: microsoft-windows-10-barometer-api
- description: The BLE Scanning API from Microsoft Windows 10 — 1 operation(s) for ble scanning.
  name: Microsoft Windows 10 BLE Scanning API
  slug: microsoft-windows-10-ble-scanning-api
- description: The Change Tracking API from Microsoft Windows 10 — 1 operation(s) for change tracking.
  name: Microsoft Windows 10 Change Tracking API
  slug: microsoft-windows-10-change-tracking-api
- description: The Classes API from Microsoft Windows 10 — 2 operation(s) for classes.
  name: Microsoft Windows 10 Classes API
  slug: microsoft-windows-10-classes-api
- description: The Collections API from Microsoft Windows 10 — 1 operation(s) for collections.
  name: Microsoft Windows 10 Collections API
  slug: microsoft-windows-10-collections-api
- description: The Command Queues API from Microsoft Windows 10 — 1 operation(s) for command queues.
  name: Microsoft Windows 10 Command Queues API
  slug: microsoft-windows-10-command-queues-api
- description: The Compass API from Microsoft Windows 10 — 1 operation(s) for compass.
  name: Microsoft Windows 10 Compass API
  slug: microsoft-windows-10-compass-api
- description: The Connectivity API from Microsoft Windows 10 — 1 operation(s) for connectivity.
  name: Microsoft Windows 10 Connectivity API
  slug: microsoft-windows-10-connectivity-api
- description: The Controls API from Microsoft Windows 10 — 2 operation(s) for controls.
  name: Microsoft Windows 10 Controls API
  slug: microsoft-windows-10-controls-api
- description: The Credentials API from Microsoft Windows 10 — 2 operation(s) for credentials.
  name: Microsoft Windows 10 Credentials API
  slug: microsoft-windows-10-credentials-api
- description: The Devices API from Microsoft Windows 10 — 6 operation(s) for devices.
  name: Microsoft Windows 10 Devices API
  slug: microsoft-windows-10-devices-api
- description: The Drawing Attributes API from Microsoft Windows 10 — 1 operation(s) for drawing attributes.
  name: Microsoft Windows 10 Drawing Attributes API
  slug: microsoft-windows-10-drawing-attributes-api
- description: The Effects API from Microsoft Windows 10 — 1 operation(s) for effects.
  name: Microsoft Windows 10 Effects API
  slug: microsoft-windows-10-effects-api
- description: The Evaluation API from Microsoft Windows 10 — 1 operation(s) for evaluation.
  name: Microsoft Windows 10 Evaluation API
  slug: microsoft-windows-10-evaluation-api
- description: The File Operations API from Microsoft Windows 10 — 1 operation(s) for file operations.
  name: Microsoft Windows 10 File Operations API
  slug: microsoft-windows-10-file-operations-api
- description: The Files API from Microsoft Windows 10 — 2 operation(s) for files.
  name: Microsoft Windows 10 Files API
  slug: microsoft-windows-10-files-api
- description: The Folders API from Microsoft Windows 10 — 1 operation(s) for folders.
  name: Microsoft Windows 10 Folders API
  slug: microsoft-windows-10-folders-api
- description: The Functions API from Microsoft Windows 10 — 2 operation(s) for functions.
  name: Microsoft Windows 10 Functions API
  slug: microsoft-windows-10-functions-api
- description: The GATT Characteristics API from Microsoft Windows 10 — 1 operation(s) for gatt characteristics.
  name: Microsoft Windows 10 GATT Characteristics API
  slug: microsoft-windows-10-gatt-characteristics-api
- description: The GATT Operations API from Microsoft Windows 10 — 1 operation(s) for gatt operations.
  name: Microsoft Windows 10 GATT Operations API
  slug: microsoft-windows-10-gatt-operations-api
- description: The GATT Services API from Microsoft Windows 10 — 1 operation(s) for gatt services.
  name: Microsoft Windows 10 GATT Services API
  slug: microsoft-windows-10-gatt-services-api
- description: The Geofencing API from Microsoft Windows 10 — 1 operation(s) for geofencing.
  name: Microsoft Windows 10 Geofencing API
  slug: microsoft-windows-10-geofencing-api
- description: The Gyrometer API from Microsoft Windows 10 — 1 operation(s) for gyrometer.
  name: Microsoft Windows 10 Gyrometer API
  slug: microsoft-windows-10-gyrometer-api
- description: The HTTP Client API from Microsoft Windows 10 — 1 operation(s) for http client.
  name: Microsoft Windows 10 HTTP Client API
  slug: microsoft-windows-10-http-client-api
- description: The Human Presence API from Microsoft Windows 10 — 1 operation(s) for human presence.
  name: Microsoft Windows 10 Human Presence API
  slug: microsoft-windows-10-human-presence-api
- description: The Icons API from Microsoft Windows 10 — 1 operation(s) for icons.
  name: Microsoft Windows 10 Icons API
  slug: microsoft-windows-10-icons-api
- description: The Inclinometer API from Microsoft Windows 10 — 1 operation(s) for inclinometer.
  name: Microsoft Windows 10 Inclinometer API
  slug: microsoft-windows-10-inclinometer-api
- description: The Libraries API from Microsoft Windows 10 — 1 operation(s) for libraries.
  name: Microsoft Windows 10 Libraries API
  slug: microsoft-windows-10-libraries-api
- description: The Light Sensor API from Microsoft Windows 10 — 1 operation(s) for light sensor.
  name: Microsoft Windows 10 Light Sensor API
  slug: microsoft-windows-10-light-sensor-api
- description: The Messages API from Microsoft Windows 10 — 1 operation(s) for messages.
  name: Microsoft Windows 10 Messages API
  slug: microsoft-windows-10-messages-api
- description: The Models API from Microsoft Windows 10 — 2 operation(s) for models.
  name: Microsoft Windows 10 Models API
  slug: microsoft-windows-10-models-api
- description: The Namespaces API from Microsoft Windows 10 — 1 operation(s) for namespaces.
  name: Microsoft Windows 10 Namespaces API
  slug: microsoft-windows-10-namespaces-api
- description: The Orientation API from Microsoft Windows 10 — 1 operation(s) for orientation.
  name: Microsoft Windows 10 Orientation API
  slug: microsoft-windows-10-orientation-api
- description: The Pedometer API from Microsoft Windows 10 — 1 operation(s) for pedometer.
  name: Microsoft Windows 10 Pedometer API
  slug: microsoft-windows-10-pedometer-api
- description: The Photo Capture API from Microsoft Windows 10 — 1 operation(s) for photo capture.
  name: Microsoft Windows 10 Photo Capture API
  slug: microsoft-windows-10-photo-capture-api
- description: The Phrase Lists API from Microsoft Windows 10 — 1 operation(s) for phrase lists.
  name: Microsoft Windows 10 Phrase Lists API
  slug: microsoft-windows-10-phrase-lists-api
- description: The Pipeline API from Microsoft Windows 10 — 1 operation(s) for pipeline.
  name: Microsoft Windows 10 Pipeline API
  slug: microsoft-windows-10-pipeline-api
- description: The Position API from Microsoft Windows 10 — 1 operation(s) for position.
  name: Microsoft Windows 10 Position API
  slug: microsoft-windows-10-position-api
- description: The Presentation API from Microsoft Windows 10 — 1 operation(s) for presentation.
  name: Microsoft Windows 10 Presentation API
  slug: microsoft-windows-10-presentation-api
- description: The Preview API from Microsoft Windows 10 — 2 operation(s) for preview.
  name: Microsoft Windows 10 Preview API
  slug: microsoft-windows-10-preview-api
- description: The Proximity API from Microsoft Windows 10 — 1 operation(s) for proximity.
  name: Microsoft Windows 10 Proximity API
  slug: microsoft-windows-10-proximity-api
- description: The Recognition API from Microsoft Windows 10 — 2 operation(s) for recognition.
  name: Microsoft Windows 10 Recognition API
  slug: microsoft-windows-10-recognition-api
- description: The Resources API from Microsoft Windows 10 — 1 operation(s) for resources.
  name: Microsoft Windows 10 Resources API
  slug: microsoft-windows-10-resources-api
- description: The Responses API from Microsoft Windows 10 — 1 operation(s) for responses.
  name: Microsoft Windows 10 Responses API
  slug: microsoft-windows-10-responses-api
- description: The RFCOMM API from Microsoft Windows 10 — 1 operation(s) for rfcomm.
  name: Microsoft Windows 10 RFCOMM API
  slug: microsoft-windows-10-rfcomm-api
- description: The Scheduled Notifications API from Microsoft Windows 10 — 1 operation(s) for scheduled notifications.
  name: Microsoft Windows 10 Scheduled Notifications API
  slug: microsoft-windows-10-scheduled-notifications-api
- description: The Sensors API from Microsoft Windows 10 — 1 operation(s) for sensors.
  name: Microsoft Windows 10 Sensors API
  slug: microsoft-windows-10-sensors-api
- description: The Service Connections API from Microsoft Windows 10 — 1 operation(s) for service connections.
  name: Microsoft Windows 10 Service Connections API
  slug: microsoft-windows-10-service-connections-api
- description: The Sessions API from Microsoft Windows 10 — 2 operation(s) for sessions.
  name: Microsoft Windows 10 Sessions API
  slug: microsoft-windows-10-sessions-api
- description: The Strokes API from Microsoft Windows 10 — 2 operation(s) for strokes.
  name: Microsoft Windows 10 Strokes API
  slug: microsoft-windows-10-strokes-api
- description: The Structures API from Microsoft Windows 10 — 1 operation(s) for structures.
  name: Microsoft Windows 10 Structures API
  slug: microsoft-windows-10-structures-api
- description: The Styles API from Microsoft Windows 10 — 1 operation(s) for styles.
  name: Microsoft Windows 10 Styles API
  slug: microsoft-windows-10-styles-api
- description: The Surfaces API from Microsoft Windows 10 — 1 operation(s) for surfaces.
  name: Microsoft Windows 10 Surfaces API
  slug: microsoft-windows-10-surfaces-api
- description: The Tasks API from Microsoft Windows 10 — 2 operation(s) for tasks.
  name: Microsoft Windows 10 Tasks API
  slug: microsoft-windows-10-tasks-api
- description: The TCP Sockets API from Microsoft Windows 10 — 1 operation(s) for tcp sockets.
  name: Microsoft Windows 10 TCP Sockets API
  slug: microsoft-windows-10-tcp-sockets-api
- description: The Themes API from Microsoft Windows 10 — 1 operation(s) for themes.
  name: Microsoft Windows 10 Themes API
  slug: microsoft-windows-10-themes-api
- description: The Tile Notifications API from Microsoft Windows 10 — 1 operation(s) for tile notifications.
  name: Microsoft Windows 10 Tile Notifications API
  slug: microsoft-windows-10-tile-notifications-api
- description: The Toast Notifications API from Microsoft Windows 10 — 2 operation(s) for toast notifications.
  name: Microsoft Windows 10 Toast Notifications API
  slug: microsoft-windows-10-toast-notifications-api
- description: The Tracking API from Microsoft Windows 10 — 1 operation(s) for tracking.
  name: Microsoft Windows 10 Tracking API
  slug: microsoft-windows-10-tracking-api
- description: The Triggers API from Microsoft Windows 10 — 1 operation(s) for triggers.
  name: Microsoft Windows 10 Triggers API
  slug: microsoft-windows-10-triggers-api
- description: The UDP Sockets API from Microsoft Windows 10 — 1 operation(s) for udp sockets.
  name: Microsoft Windows 10 UDP Sockets API
  slug: microsoft-windows-10-udp-sockets-api
- description: The Video Recording API from Microsoft Windows 10 — 2 operation(s) for video recording.
  name: Microsoft Windows 10 Video Recording API
  slug: microsoft-windows-10-video-recording-api
- description: The Visits API from Microsoft Windows 10 — 1 operation(s) for visits.
  name: Microsoft Windows 10 Visits API
  slug: microsoft-windows-10-visits-api
- description: The Visuals API from Microsoft Windows 10 — 1 operation(s) for visuals.
  name: Microsoft Windows 10 Visuals API
  slug: microsoft-windows-10-visuals-api
- description: The Voice Commands API from Microsoft Windows 10 — 2 operation(s) for voice commands.
  name: Microsoft Windows 10 Voice Commands API
  slug: microsoft-windows-10-voice-commands-api
- description: The WebSockets API from Microsoft Windows 10 — 1 operation(s) for websockets.
  name: Microsoft Windows 10 WebSockets API
  slug: microsoft-windows-10-websockets-api
arazzos:
- description: Confirm background access, request it when unspecified, register a time-triggered background task, and read back its details.
  name: Microsoft Windows 10 Register Background Task with Access
  slug: microsoft-windows-10-background-task-register-workflow
- description: Discover a paired BLE device, drill into its GATT services and characteristics, and read a characteristic value.
  name: Microsoft Windows 10 Bluetooth LE Read GATT Characteristic
  slug: microsoft-windows-10-bluetooth-le-read-characteristic-workflow
- description: Check location permission, request it when unspecified, and then read the device's current geographic position.
  name: Microsoft Windows 10 Geolocation Access and Current Position
  slug: microsoft-windows-10-geolocation-access-and-position-workflow
- description: Ensure location access, create a circular geofence, and confirm it appears among the active geofences.
  name: Microsoft Windows 10 Create and Confirm Geofence
  slug: microsoft-windows-10-geolocation-create-geofence-workflow
- description: Pick a camera, initialize a capture session, start the preview, and capture a photo to a file.
  name: Microsoft Windows 10 Media Capture Photo
  slug: microsoft-windows-10-media-capture-photo-workflow
- description: Load an ONNX model, create a hardware-accelerated evaluation session, and run an inference against bound inputs.
  name: Microsoft Windows 10 Windows ML Load and Evaluate Model
  slug: microsoft-windows-10-ml-load-and-evaluate-workflow
- description: Create a toast notification collection, schedule a future toast, and confirm it appears in the pending list.
  name: Microsoft Windows 10 Schedule Grouped Toast Notification
  slug: microsoft-windows-10-notifications-schedule-toast-workflow
- description: List files in a known folder, inspect the first file, read its text content, and write updated content back.
  name: Microsoft Windows 10 Storage Read and Update File
  slug: microsoft-windows-10-storage-read-and-update-file-workflow
- description: Confirm Windows Hello is available, create a passwordless key credential, and retrieve its TPM attestation for server registration.
  name: Microsoft Windows 10 Windows Hello Enroll Key Credential
  slug: microsoft-windows-10-windows-hello-enroll-credential-workflow
- description: Verify the user with biometrics or PIN, open their key credential, and sign a server challenge for step-up authentication.
  name: Microsoft Windows 10 Windows Hello Step-Up Sign Challenge
  slug: microsoft-windows-10-windows-hello-stepup-sign-workflow
artifact_total: 284
collections:
- collection_type: postman
  name: Microsoft Windows 10 Windows Background Tasks API
  slug: postman-microsoft-windows-10-background-tasks
- collection_type: postman
  name: Microsoft Windows 10 Windows Bluetooth API
  slug: postman-microsoft-windows-10-bluetooth
- collection_type: postman
  name: Microsoft Windows 10 Windows Composition API
  slug: postman-microsoft-windows-10-composition
- collection_type: postman
  name: Microsoft Windows 10 Windows Cortana API
  slug: postman-microsoft-windows-10-cortana
- collection_type: postman
  name: Microsoft Windows 10 DirectX Graphics API
  slug: postman-microsoft-windows-10-directx
- collection_type: postman
  name: Microsoft Windows 10 Windows Geolocation API
  slug: postman-microsoft-windows-10-geolocation
- collection_type: postman
  name: Microsoft Windows 10 Windows Hello Authentication API
  slug: postman-microsoft-windows-10-hello
- collection_type: postman
  name: Microsoft Windows 10 Windows Ink API
  slug: postman-microsoft-windows-10-ink
- collection_type: postman
  name: Microsoft Windows 10 Windows Media Capture API
  slug: postman-microsoft-windows-10-media-capture
- collection_type: postman
  name: Microsoft Windows 10 Windows ML API
  slug: postman-microsoft-windows-10-ml
- collection_type: postman
  name: Microsoft Windows 10 Windows Networking API
  slug: postman-microsoft-windows-10-networking
- collection_type: postman
  name: Microsoft Windows 10 Windows Notifications API
  slug: postman-microsoft-windows-10-notifications
- collection_type: postman
  name: Microsoft Windows 10 Windows Sensors API
  slug: postman-microsoft-windows-10-sensors
- collection_type: postman
  name: Microsoft Windows 10 Windows Storage API
  slug: postman-microsoft-windows-10-storage
- collection_type: postman
  name: Microsoft Windows 10 Win32 API
  slug: postman-microsoft-windows-10-win32
- collection_type: postman
  name: Microsoft Windows 10 Windows Runtime (WinRT) API
  slug: postman-microsoft-windows-10-winrt
- collection_type: postman
  name: Microsoft Windows 10 WinUI API
  slug: postman-microsoft-windows-10-winui
- collection_type: open
  name: Microsoft Windows 10 Windows Background Tasks API
  slug: open-microsoft-windows-10-background-tasks
- collection_type: open
  name: Microsoft Windows 10 Windows Bluetooth API
  slug: open-microsoft-windows-10-bluetooth
- collection_type: open
  name: Microsoft Windows 10 Windows Composition API
  slug: open-microsoft-windows-10-composition
- collection_type: open
  name: Microsoft Windows 10 Windows Cortana API
  slug: open-microsoft-windows-10-cortana
- collection_type: open
  name: Microsoft Windows 10 DirectX Graphics API
  slug: open-microsoft-windows-10-directx
- collection_type: open
  name: Microsoft Windows 10 Windows Geolocation API
  slug: open-microsoft-windows-10-geolocation
- collection_type: open
  name: Microsoft Windows 10 Windows Hello Authentication API
  slug: open-microsoft-windows-10-hello
- collection_type: open
  name: Microsoft Windows 10 Windows Ink API
  slug: open-microsoft-windows-10-ink
- collection_type: open
  name: Microsoft Windows 10 Windows Media Capture API
  slug: open-microsoft-windows-10-media-capture
- collection_type: open
  name: Microsoft Windows 10 Windows ML API
  slug: open-microsoft-windows-10-ml
- collection_type: open
  name: Microsoft Windows 10 Windows Networking API
  slug: open-microsoft-windows-10-networking
- collection_type: open
  name: Microsoft Windows 10 Windows Notifications API
  slug: open-microsoft-windows-10-notifications
- collection_type: open
  name: Microsoft Windows 10 Windows Sensors API
  slug: open-microsoft-windows-10-sensors
- collection_type: open
  name: Microsoft Windows 10 Windows Storage API
  slug: open-microsoft-windows-10-storage
- collection_type: open
  name: Microsoft Windows 10 Win32 API
  slug: open-microsoft-windows-10-win32
- collection_type: open
  name: Microsoft Windows 10 Windows Runtime (WinRT) API
  slug: open-microsoft-windows-10-winrt
- collection_type: open
  name: Microsoft Windows 10 WinUI API
  slug: open-microsoft-windows-10-winui
common:
- group: agent
  title: ''
  type: AgenticAccess
  url: agentic-access/microsoft-windows-10-agentic-access.yml
- group: auth
  title: ''
  type: VulnerabilityDisclosure
  url: security/microsoft-windows-10-vulnerability-disclosure.yml
- group: auth
  title: ''
  type: DomainSecurity
  url: security/microsoft-windows-10-domain-security.yml
- group: build
  title: ''
  type: PostmanWorkspace
  url: https://www.postman.com/kinlaneapi/microsoft-windows-10/overview
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-background-task-register-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-bluetooth-le-read-characteristic-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-geolocation-access-and-position-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-geolocation-create-geofence-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-media-capture-photo-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-ml-load-and-evaluate-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-notifications-schedule-toast-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-storage-read-and-update-file-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-windows-hello-enroll-credential-workflow.yml
- group: design
  title: ''
  type: Arazzo
  url: arazzo/microsoft-windows-10-windows-hello-stepup-sign-workflow.yml
- group: start
  title: ''
  type: Portal
  url: https://developer.microsoft.com/en-us/windows/
- group: docs
  title: ''
  type: Documentation
  url: https://learn.microsoft.com/en-us/windows/apps/
- group: start
  title: ''
  type: GettingStarted
  url: https://learn.microsoft.com/en-us/windows/apps/get-started/
- group: auth
  title: ''
  type: Authentication
  url: https://learn.microsoft.com/en-us/windows/apps/develop/security/windows-hello
- group: company
  title: ''
  type: Blog
  url: https://blogs.windows.com/windowsdeveloper/
- group: operate
  title: ''
  type: StatusPage
  url: https://status.cloud.microsoft/
- group: operate
  title: ''
  type: Support
  url: https://developer.microsoft.com/en-us/windows/support
- group: commercial
  title: ''
  type: TermsOfService
  url: https://www.microsoft.com/en-us/legal/terms-of-use
- group: commercial
  title: ''
  type: PrivacyPolicy
  url: https://www.microsoft.com/en-us/privacy/privacystatement
- group: build
  title: ''
  type: GitHubOrganization
  url: https://github.com/microsoft
- group: operate
  title: ''
  type: Community
  url: https://developer.microsoft.com/en-us/windows/community/
- group: company
  title: ''
  type: Website
  url: https://www.microsoft.com
- group: start
  title: ''
  type: Login
  url: https://login.microsoftonline.com/
- group: start
  title: ''
  type: Signup
  url: https://developer.microsoft.com/en-us/
- group: build
  title: ''
  type: SDKs
  url: https://developer.microsoft.com/en-us/windows/downloads/windows-10-sdk/
- group: operate
  title: ''
  type: ChangeLog
  url: https://learn.microsoft.com/en-us/windows/apps/whats-new/whats-new-for-developers
- group: docs
  title: ''
  type: Reference
  url: https://learn.microsoft.com/en-us/windows/apps/api-reference/
- group: design
  title: ''
  type: JSONLD
  url: json-ld/microsoft-windows-10-context.jsonld
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/microsoft-windows-10-notification-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/microsoft-windows-10-storage-item-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/microsoft-windows-10-geolocation-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/microsoft-windows-10-sensor-reading-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/microsoft-windows-10-ml-model-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/microsoft-windows-10-bluetooth-device-schema.json
created: '2024-01-01'
description: Collection of APIs and developer resources for building applications on the Windows 10 platform, including Universal Windows Platform (UWP), Win32, and Windows Runtime APIs for desktop, mobile, and IoT applications.
finops:
- name: Microsoft Windows 10 Finops
  service_category: Operating System / Endpoint
  slug: microsoft-windows-10-finops
image: https://kinlane-images.s3.amazonaws.com/shared/apis-json/icons/microsoft-windows-10.png
json_schemas:
- name: AccelerometerReading
  property_count: 4
  slug: microsoft-windows-10-accelerometerreading
- name: ActivationRequest
  property_count: 2
  slug: microsoft-windows-10-activationrequest
- name: ActivationResult
  property_count: 3
  slug: microsoft-windows-10-activationresult
- name: ApiCategory
  property_count: 5
  slug: microsoft-windows-10-apicategory
- name: AttestationResult
  property_count: 3
  slug: microsoft-windows-10-attestationresult
- name: BackgroundAccessStatus
  property_count: 1
  slug: microsoft-windows-10-backgroundaccessstatus
- name: BackgroundDownloadRequest
  property_count: 4
  slug: microsoft-windows-10-backgrounddownloadrequest
- name: BackgroundTask
  property_count: 4
  slug: microsoft-windows-10-backgroundtask
- name: BackgroundTaskDetail
  property_count: 8
  slug: microsoft-windows-10-backgroundtaskdetail
- name: BackgroundTransferOperation
  property_count: 5
  slug: microsoft-windows-10-backgroundtransferoperation
- name: BadgeNotification
  property_count: 3
  slug: microsoft-windows-10-badgenotification
- name: BadgeNotificationRequest
  property_count: 2
  slug: microsoft-windows-10-badgenotificationrequest
- name: BarometerReading
  property_count: 2
  slug: microsoft-windows-10-barometerreading
- name: BLEScanRequest
  property_count: 3
  slug: microsoft-windows-10-blescanrequest
- name: BLEScanSession
  property_count: 2
  slug: microsoft-windows-10-blescansession
- name: Windows 10 Bluetooth Device
  property_count: 10
  slug: microsoft-windows-10-bluetooth-device
- name: BluetoothDevice
  property_count: 6
  slug: microsoft-windows-10-bluetoothdevice
- name: BluetoothDeviceDetail
  property_count: 9
  slug: microsoft-windows-10-bluetoothdevicedetail
- name: BoundingRect
  property_count: 4
  slug: microsoft-windows-10-boundingrect
- name: CaptureDevice
  property_count: 6
  slug: microsoft-windows-10-capturedevice
- name: CapturedPhoto
  property_count: 5
  slug: microsoft-windows-10-capturedphoto
- name: CaptureSession
  property_count: 4
  slug: microsoft-windows-10-capturesession
- name: ClassEvent
  property_count: 3
  slug: microsoft-windows-10-classevent
- name: ClassMethod
  property_count: 5
  slug: microsoft-windows-10-classmethod
- name: ClassProperty
  property_count: 4
  slug: microsoft-windows-10-classproperty
- name: CommandQueue
  property_count: 3
  slug: microsoft-windows-10-commandqueue
- name: CompassReading
  property_count: 4
  slug: microsoft-windows-10-compassreading
- name: CompositionAnimation
  property_count: 5
  slug: microsoft-windows-10-compositionanimation
- name: CompositionEffect
  property_count: 2
  slug: microsoft-windows-10-compositioneffect
- name: CompositionSurface
  property_count: 4
  slug: microsoft-windows-10-compositionsurface
- name: CompositionVisual
  property_count: 9
  slug: microsoft-windows-10-compositionvisual
- name: CreateAnimationRequest
  property_count: 5
  slug: microsoft-windows-10-createanimationrequest
- name: CreateCommandQueueRequest
  property_count: 2
  slug: microsoft-windows-10-createcommandqueuerequest
- name: CreateCredentialRequest
  property_count: 2
  slug: microsoft-windows-10-createcredentialrequest
- name: CreateEffectRequest
  property_count: 3
  slug: microsoft-windows-10-createeffectrequest
- name: CreateFolderRequest
  property_count: 3
  slug: microsoft-windows-10-createfolderrequest
- name: CreateGeofenceRequest
  property_count: 7
  slug: microsoft-windows-10-creategeofencerequest
- name: CreateInkStrokeRequest
  property_count: 2
  slug: microsoft-windows-10-createinkstrokerequest
- name: CreatePipelineStateRequest
  property_count: 7
  slug: microsoft-windows-10-createpipelinestaterequest
- name: CreateResourceRequest
  property_count: 7
  slug: microsoft-windows-10-createresourcerequest
- name: CreateSessionRequest
  property_count: 2
  slug: microsoft-windows-10-createsessionrequest
- name: CreateSurfaceRequest
  property_count: 4
  slug: microsoft-windows-10-createsurfacerequest
- name: CreateSwapChainRequest
  property_count: 6
  slug: microsoft-windows-10-createswapchainrequest
- name: CreateTcpSocketRequest
  property_count: 5
  slug: microsoft-windows-10-createtcpsocketrequest
- name: CreateUdpSocketRequest
  property_count: 2
  slug: microsoft-windows-10-createudpsocketrequest
- name: CreateVisualRequest
  property_count: 4
  slug: microsoft-windows-10-createvisualrequest
- name: CreateWebSocketRequest
  property_count: 4
  slug: microsoft-windows-10-createwebsocketrequest
- name: DatagramSocket
  property_count: 3
  slug: microsoft-windows-10-datagramsocket
- name: DirectXResource
  property_count: 8
  slug: microsoft-windows-10-directxresource
- name: EvaluationRequest
  property_count: 2
  slug: microsoft-windows-10-evaluationrequest
- name: EvaluationResult
  property_count: 4
  slug: microsoft-windows-10-evaluationresult
- name: FeatureDescriptor
  property_count: 6
  slug: microsoft-windows-10-featuredescriptor
- name: FileContent
  property_count: 4
  slug: microsoft-windows-10-filecontent
- name: FluentIcon
  property_count: 4
  slug: microsoft-windows-10-fluenticon
- name: FunctionParameter
  property_count: 5
  slug: microsoft-windows-10-functionparameter
- name: FunctionRequirements
  property_count: 5
  slug: microsoft-windows-10-functionrequirements
- name: GattCharacteristic
  property_count: 4
  slug: microsoft-windows-10-gattcharacteristic
- name: GattReadResult
  property_count: 2
  slug: microsoft-windows-10-gattreadresult
- name: GattService
  property_count: 3
  slug: microsoft-windows-10-gattservice
- name: GattWriteRequest
  property_count: 2
  slug: microsoft-windows-10-gattwriterequest
- name: GattWriteResult
  property_count: 1
  slug: microsoft-windows-10-gattwriteresult
- name: Geocircle
  property_count: 2
  slug: microsoft-windows-10-geocircle
- name: Geocoordinate
  property_count: 10
  slug: microsoft-windows-10-geocoordinate
- name: Geofence
  property_count: 7
  slug: microsoft-windows-10-geofence
- name: Windows 10 Geolocation
  property_count: 3
  slug: microsoft-windows-10-geolocation
- name: GeolocationAccessStatus
  property_count: 1
  slug: microsoft-windows-10-geolocationaccessstatus
- name: Geoposition
  property_count: 3
  slug: microsoft-windows-10-geoposition
- name: Geovisit
  property_count: 3
  slug: microsoft-windows-10-geovisit
- name: GraphicsAdapter
  property_count: 7
  slug: microsoft-windows-10-graphicsadapter
- name: GraphicsAdapterDetail
  property_count: 11
  slug: microsoft-windows-10-graphicsadapterdetail
- name: GyrometerReading
  property_count: 4
  slug: microsoft-windows-10-gyrometerreading
- name: HelloAvailability
  property_count: 2
  slug: microsoft-windows-10-helloavailability
- name: HttpRequest
  property_count: 5
  slug: microsoft-windows-10-httprequest
- name: HttpResponse
  property_count: 5
  slug: microsoft-windows-10-httpresponse
- name: HumanPresenceReading
  property_count: 6
  slug: microsoft-windows-10-humanpresencereading
- name: InclinometerReading
  property_count: 5
  slug: microsoft-windows-10-inclinometerreading
- name: InitializeCaptureRequest
  property_count: 5
  slug: microsoft-windows-10-initializecapturerequest
- name: InkDrawingAttributes
  property_count: 7
  slug: microsoft-windows-10-inkdrawingattributes
- name: InkPoint
  property_count: 6
  slug: microsoft-windows-10-inkpoint
- name: InkRecognitionResult
  property_count: 4
  slug: microsoft-windows-10-inkrecognitionresult
- name: InkRecognizer
  property_count: 2
  slug: microsoft-windows-10-inkrecognizer
- name: InkStroke
  property_count: 4
  slug: microsoft-windows-10-inkstroke
- name: InkStrokeDetail
  property_count: 5
  slug: microsoft-windows-10-inkstrokedetail
- name: InstallVCDRequest
  property_count: 1
  slug: microsoft-windows-10-installvcdrequest
- name: KeyCredential
  property_count: 4
  slug: microsoft-windows-10-keycredential
- name: LearningModel
  property_count: 8
  slug: microsoft-windows-10-learningmodel
- name: LearningModelDetail
  property_count: 9
  slug: microsoft-windows-10-learningmodeldetail
- name: LearningModelDevice
  property_count: 3
  slug: microsoft-windows-10-learningmodeldevice
- name: LearningModelSession
  property_count: 3
  slug: microsoft-windows-10-learningmodelsession
- name: LightSensorReading
  property_count: 3
  slug: microsoft-windows-10-lightsensorreading
- name: LoadModelRequest
  property_count: 2
  slug: microsoft-windows-10-loadmodelrequest
- name: Windows 10 Machine Learning Model
  property_count: 8
  slug: microsoft-windows-10-ml-model
- name: NetworkConnectivity
  property_count: 5
  slug: microsoft-windows-10-networkconnectivity
- name: Windows 10 Notification
  property_count: 12
  slug: microsoft-windows-10-notification
- name: OrientationReading
  property_count: 3
  slug: microsoft-windows-10-orientationreading
- name: PedometerReading
  property_count: 4
  slug: microsoft-windows-10-pedometerreading
- name: PhotoCaptureRequest
  property_count: 2
  slug: microsoft-windows-10-photocapturerequest
- name: PhraseList
  property_count: 2
  slug: microsoft-windows-10-phraselist
- name: PipelineState
  property_count: 2
  slug: microsoft-windows-10-pipelinestate
- name: ProximityReading
  property_count: 3
  slug: microsoft-windows-10-proximityreading
- name: RecognitionRequest
  property_count: 2
  slug: microsoft-windows-10-recognitionrequest
- name: RecordedMedia
  property_count: 6
  slug: microsoft-windows-10-recordedmedia
- name: RegisterTaskRequest
  property_count: 5
  slug: microsoft-windows-10-registertaskrequest
- name: RfcommConnection
  property_count: 4
  slug: microsoft-windows-10-rfcommconnection
- name: RfcommConnectionRequest
  property_count: 2
  slug: microsoft-windows-10-rfcommconnectionrequest
- name: ScheduledToastNotification
  property_count: 7
  slug: microsoft-windows-10-scheduledtoastnotification
- name: ScheduledToastNotificationRequest
  property_count: 6
  slug: microsoft-windows-10-scheduledtoastnotificationrequest
- name: Windows 10 Sensor Reading
  property_count: 3
  slug: microsoft-windows-10-sensor-reading
- name: SensorInfo
  property_count: 4
  slug: microsoft-windows-10-sensorinfo
- name: ServiceConnection
  property_count: 3
  slug: microsoft-windows-10-serviceconnection
- name: ServiceConnectionRequest
  property_count: 1
  slug: microsoft-windows-10-serviceconnectionrequest
- name: SignRequest
  property_count: 2
  slug: microsoft-windows-10-signrequest
- name: SignResult
  property_count: 2
  slug: microsoft-windows-10-signresult
- name: Windows 10 Storage Item
  property_count: 11
  slug: microsoft-windows-10-storage-item
- name: StorageFile
  property_count: 7
  slug: microsoft-windows-10-storagefile
- name: StorageFileDetail
  property_count: 10
  slug: microsoft-windows-10-storagefiledetail
- name: StorageFolder
  property_count: 4
  slug: microsoft-windows-10-storagefolder
- name: StorageLibrary
  property_count: 3
  slug: microsoft-windows-10-storagelibrary
- name: StorageLibraryChange
  property_count: 4
  slug: microsoft-windows-10-storagelibrarychange
- name: StreamSocket
  property_count: 6
  slug: microsoft-windows-10-streamsocket
- name: StyleResource
  property_count: 4
  slug: microsoft-windows-10-styleresource
- name: SwapChain
  property_count: 5
  slug: microsoft-windows-10-swapchain
- name: Theme
  property_count: 3
  slug: microsoft-windows-10-theme
- name: TileNotification
  property_count: 3
  slug: microsoft-windows-10-tilenotification
- name: TileNotificationRequest
  property_count: 5
  slug: microsoft-windows-10-tilenotificationrequest
- name: ToastCollection
  property_count: 4
  slug: microsoft-windows-10-toastcollection
- name: ToastCollectionRequest
  property_count: 4
  slug: microsoft-windows-10-toastcollectionrequest
- name: ToastContent
  property_count: 3
  slug: microsoft-windows-10-toastcontent
- name: ToastNotification
  property_count: 7
  slug: microsoft-windows-10-toastnotification
- name: ToastNotificationRequest
  property_count: 7
  slug: microsoft-windows-10-toastnotificationrequest
- name: TrackingRequest
  property_count: 3
  slug: microsoft-windows-10-trackingrequest
- name: TrackingSession
  property_count: 2
  slug: microsoft-windows-10-trackingsession
- name: TriggerType
  property_count: 4
  slug: microsoft-windows-10-triggertype
- name: UpdatePhraseListRequest
  property_count: 1
  slug: microsoft-windows-10-updatephraselistrequest
- name: Vector2
  property_count: 2
  slug: microsoft-windows-10-vector2
- name: Vector3
  property_count: 3
  slug: microsoft-windows-10-vector3
- name: VerifyUserRequest
  property_count: 1
  slug: microsoft-windows-10-verifyuserrequest
- name: VerifyUserResult
  property_count: 1
  slug: microsoft-windows-10-verifyuserresult
- name: VideoRecordingRequest
  property_count: 3
  slug: microsoft-windows-10-videorecordingrequest
- name: VoiceCommand
  property_count: 5
  slug: microsoft-windows-10-voicecommand
- name: VoiceCommandDefinition
  property_count: 3
  slug: microsoft-windows-10-voicecommanddefinition
- name: VoiceCommandResponseRequest
  property_count: 3
  slug: microsoft-windows-10-voicecommandresponserequest
- name: VoiceCommandSet
  property_count: 4
  slug: microsoft-windows-10-voicecommandset
- name: WebSocket
  property_count: 4
  slug: microsoft-windows-10-websocket
- name: Win32Function
  property_count: 6
  slug: microsoft-windows-10-win32function
- name: Win32FunctionDetail
  property_count: 7
  slug: microsoft-windows-10-win32functiondetail
- name: Win32Structure
  property_count: 4
  slug: microsoft-windows-10-win32structure
- name: WindowMessage
  property_count: 5
  slug: microsoft-windows-10-windowmessage
- name: WinRTClass
  property_count: 6
  slug: microsoft-windows-10-winrtclass
- name: WinRTClassDetail
  property_count: 7
  slug: microsoft-windows-10-winrtclassdetail
- name: WinRTNamespace
  property_count: 7
  slug: microsoft-windows-10-winrtnamespace
- name: WinUIControl
  property_count: 5
  slug: microsoft-windows-10-winuicontrol
- name: WinUIControlDetail
  property_count: 9
  slug: microsoft-windows-10-winuicontroldetail
- name: WriteFileRequest
  property_count: 3
  slug: microsoft-windows-10-writefilerequest
json_structures:
- name: Microsoft Windows 10 Structure
  property_count: 0
  slug: microsoft-windows-10-structure
jsonld:
- class_count: 0
  name: Microsoft Windows 10 Context
  property_count: 28
  slug: microsoft-windows-10-context
layout: provider
modified: '2026-05-19'
name: Microsoft Windows 10
nav: Providers
network: true
overview: 'Microsoft Windows 10 publishes 77 APIs on the [APIs.io](https://apis.io/) network, including Accelerometer API, Access API, Activation API, and 74 more. Tagged areas include Desktop, Operating System, UWP, Win32, and Windows.


  The Microsoft Windows 10 catalog on APIs.io includes 1 JSON-LD context and 1 Spectral governance ruleset.


  Microsoft Windows 10''s developer surface includes developer portal, documentation, getting-started guide, authentication, engineering blog, support, signup flow, and 31 more developer resources.'
plans:
- name: Microsoft Windows 10 Plans Pricing
  plan_count: 5
  slug: microsoft-windows-10-plans-pricing
random_paper: 4
rate_limits:
- limit_count: 3
  name: Microsoft Windows 10 Rate Limits
  slug: microsoft-windows-10-rate-limits
rules:
- name: Microsoft Windows 10 API Rules
  rule_count: 6
  severity_counts:
    error: 0
    hint: 0
    info: 2
    warn: 4
  slug: microsoft-windows-10-jsonschema-spectral-rules
score:
  band: exemplar
  composite: 66.3
  delta: -3.3
  facets:
    commercial_clarity: 73.7
    contract_quality: 62.0
    developer_ergonomics: 63.0
    discoverability: 75.9
    governance: 58.3
    operational_transparency: 68.4
  previous_composite: 69.6
  provenance:
    agentic_access: derived
    contracts:
      callable: 100.0
      derived: 0
      marker_coverage: 0.0
      total: 77
  schema_version: 0.6
  scored_at: '2026-07-28'
  trend: flat
screenshot: https://raw.githubusercontent.com/api-evangelist/microsoft-windows-10/refs/heads/main/screenshots/microsoft-windows-10-2026-06-20T185545.png
security:
- kind: domain-security
  name: Microsoft Windows 10 Domain Security
  slug: microsoft-windows-10-domain-security
  summary_line: TLSv1.3 · HSTS · DMARC
- kind: vulnerability-disclosure
  name: Microsoft Windows 10 Vulnerability Disclosure
  slug: microsoft-windows-10-vulnerability-disclosure
  summary_line: security.txt · contact published
slug: microsoft-windows-10
tags:
- Desktop
- Operating System
- UWP
- Win32
- Windows
website: https://www.microsoft.com
---
