---
access_model:
  confidence: low
  label: Unknown
  onboarding: unknown
  pricing: unknown
  public: false
  source: []
  trial: false
  try_now: false
agent_readiness:
  band: human-only
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: false
    agentic_commerce: false
    auth_clarity: false
    consent_identity: false
    delegated_identity: false
    dry_run_mode: false
    dynamic_client_registration: false
    error_semantics: false
    event_surface_described: false
    idempotency: false
    mcp_server: false
    openapi_examples: documented
    protected_resource_metadata: false
    rate_limit_signal: false
    reversibility_documented: false
    spec_presence: false
    well_known_catalog: false
  schema_version: '0.2'
  score: 1.3
  scored_at: '2026-09-17'
api_count: 6
apis:
- description: The open source Renovate CLI is a Node.js application distributed via npm and as Docker images on Docker Hub and GitHub Container Registry. It can be run locally, in CI, or as a long-lived self-hosted
  name: Renovate CLI
  slug: renovate-cli
- description: Renovate's behavior is driven by repository-level configuration files (`renovate.json`, `renovate.json5`, `.github/renovate.json`, `.gitlab/renovate.json`, `.renovaterc`, or a deprecated `renovate` ke
  name: Renovate Configuration
  slug: renovate-configuration
- description: Renovate ships 100+ package manager modules covering JavaScript (npm, bun, deno, nvm, nodenv), Python (poetry, pipenv, pip-compile, pep621, pyenv), Java/JVM (maven, gradle, sbt, ant), .NET (nuget, cak
  name: Renovate Package Managers
  slug: renovate-managers
- description: Renovate has pluggable platform adapters for GitHub (cloud and Enterprise Server), GitLab (SaaS and CE/EE), Bitbucket Cloud, Bitbucket Data Center / Server, Azure DevOps, Gitea, Forgejo, AWS CodeCommi
  name: Renovate Git Platforms
  slug: renovate-platforms
- description: Mend operates a hosted Renovate service that runs Renovate as a GitHub App, Bitbucket Cloud app, and Azure DevOps integration on Mend infrastructure. A generous Community Cloud free tier covers unlimi
  name: Mend Hosted Renovate
  slug: mend-hosted-renovate
- description: Merge Confidence is a Mend-operated data service that classifies each candidate update (Low / Neutral / High / Very High) using package age, adoption, test pass rates, and known vulnerabilities. Renov
  name: Renovate Merge Confidence
  slug: renovate-merge-confidence
artifact_total: 32
common:
- group: company
  title: ''
  type: Website
  url: https://renovatebot.com
- group: auth
  title: ''
  type: SecurityPolicy
  url: https://github.com/renovatebot/renovate/blob/main/SECURITY.md
- group: build
  title: ''
  type: CodeOfConduct
  url: https://github.com/renovatebot/renovate/blob/main/CODE_OF_CONDUCT.md
- group: docs
  title: ''
  type: ContributionGuide
  url: https://github.com/renovatebot/renovate/blob/main/.github/contributing.md
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/renovate-bot/refs/heads/main/security/renovate-bot-trust-center.yml
  title: ''
  type: TrustCenter
  url: security/renovate-bot-trust-center.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/renovate-bot/refs/heads/main/security/renovate-bot-domain-security.yml
  title: ''
  type: DomainSecurity
  url: security/renovate-bot-domain-security.yml
- group: start
  title: ''
  type: Portal
  url: https://www.mend.io/renovate/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/
- group: start
  title: ''
  type: GettingStarted
  url: https://docs.renovatebot.com/getting-started/installing-onboarding/
- group: build
  title: ''
  type: SourceCode
  url: https://github.com/renovatebot/renovate
- group: build
  title: ''
  type: GitHubOrganization
  url: https://github.com/renovatebot
- group: docs
  title: ''
  type: Documentation
  url: https://github.com/apps/renovate
- group: start
  title: ''
  type: Signup
  url: https://developer.mend.io/
- group: operate
  title: ''
  type: ChangeLog
  url: https://github.com/renovatebot/renovate/releases
- group: operate
  title: ''
  type: Issues
  url: https://github.com/renovatebot/renovate/issues
- group: operate
  title: ''
  type: Forums
  url: https://github.com/renovatebot/renovate/discussions
- group: operate
  title: ''
  type: Contact
  url: https://docs.renovatebot.com/getting-help/
- group: operate
  title: ''
  type: Support
  url: https://docs.renovatebot.com/getting-help/
- group: commercial
  title: ''
  type: License
  url: https://github.com/renovatebot/renovate/blob/main/license
- group: build
  title: ''
  type: Package
  url: https://www.npmjs.com/package/renovate
- group: build
  title: ''
  type: Package
  url: https://hub.docker.com/r/renovate/renovate
- group: build
  title: ''
  type: Package
  url: https://github.com/renovatebot/renovate/pkgs/container/renovate
- group: build
  title: ''
  type: Tools
  url: https://github.com/renovatebot/github-action
- group: build
  title: ''
  type: Tools
  url: https://github.com/renovatebot/pre-commit-hooks
- group: build
  title: ''
  type: SDKs
  url: https://github.com/renovatebot/renovate/tree/main/lib
- group: build
  title: ''
  type: CodeExamples
  url: https://github.com/renovatebot/.github
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/self-hosted-configuration/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/self-hosted-experimental/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/configuration-options/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/configuration-templates/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/config-presets/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/key-concepts/dependency-dashboard/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/key-concepts/automerge/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/modules/manager/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/modules/datasource/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/modules/versioning/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/modules/platform/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/modules/manager/regex/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/merge-confidence/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/troubleshooting/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/security-and-permissions/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/getting-started/private-packages/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.renovatebot.com/mend-hosted/overview/
- group: commercial
  title: ''
  type: PrivacyPolicy
  url: https://www.mend.io/privacy-policy/
- group: commercial
  title: ''
  type: TermsOfService
  url: https://www.mend.io/terms-of-service/
- group: auth
  title: ''
  type: TrustCenter
  url: https://www.mend.io/security-and-compliance/
- group: company
  title: ''
  type: Blog
  url: https://www.mend.io/blog/
- group: company
  title: ''
  type: LinkedIn
  url: https://www.linkedin.com/company/mend-io/
- group: company
  title: ''
  type: Twitter
  url: https://twitter.com/mend_io
- group: commercial
  title: ''
  type: Plans
  url: https://docs.renovatebot.com/mend-hosted/overview/
created: '2026-05-25T00:00:00.000Z'
description: Renovate is an open source automated dependency update bot maintained by Mend. It scans repositories for references to dependencies across 100+ package managers — npm, pip, poetry, Maven, Gradle, NuGet, Go modules, Cargo, Bundler, Docker, Helm, Kubernetes, Terraform, GitHub Actions and many more — and opens pull/merge requests that update versions, lockfiles, and digests. Renovate runs as a CLI, Docker container, GitHub App, GitLab Runner, Bitbucket Cloud app, or Azure DevOps integration, supports GitHub, GitLab, Bitbucket, Azure DevOps, Gitea, Forgejo, AWS CodeCommit, and Gerrit, and is configured via a renovate.json file with shareable presets, packageRules, automerge, grouping, scheduling, and a Dependency Dashboard. Mend provides a free Community Cloud hosted tier for unlimited public and private repositories and a paid Enterprise Cloud tier with higher concurrency, Merge Confidence Workflows, and support.
features:
- Open source dependency update bot written in TypeScript and distributed under AGPL-3.0-only
- 100+ package manager modules across JavaScript, Python, Java/JVM, .NET, Go, Rust, Ruby, Docker, Kubernetes, Terraform, CI systems, Bazel, Ansible, Homebrew, asdf, mise, nix, proto, and more
- Pluggable Git platform adapters for GitHub, GitLab, Bitbucket Cloud/DC, Azure DevOps, Gitea, Forgejo, AWS CodeCommit, Gerrit, SCM-Manager
- Repository configuration via renovate.json / renovate.json5 / .github/renovate.json / .gitlab/renovate.json / .renovaterc with full JSON Schema
- Shareable config presets (extends) including the curated config:recommended preset
- packageRules for conditional behaviour by package, depType, manager, or file pattern
- Dependency Dashboard issue summarising every open and pending update
- Automerge with PR, branch, or PR-comment strategies and separate automergeSchedule
- Grouping of related updates (group, group:allNonMajor, group:monorepos, etc.)
- Lock file maintenance and digest pinning for Docker, GitHub Actions, and Helm
- Custom (regex) manager for updating versions in arbitrary file formats
- Private package and registry support (npmrc, hostRules, Vault / cloud secrets)
- Self-hosted execution as Node.js CLI, Docker container (Docker Hub + GHCR), GitHub Action, or pre-commit hook
- Mend-hosted Renovate as a GitHub App, Bitbucket Cloud app, and Azure DevOps integration
- Merge Confidence ratings (Low / Neutral / High / Very High) per update from developer.mend.io/api/mc/
- Vulnerability alerts integration (GitHub Dependabot alerts, OSV)
- Configurable schedules to limit when PRs are opened or merged
- baseBranchPatterns to run Renovate across multiple branches in a single repo
- Deprecation handling with migration PRs for replacement packages
- 90+ versioning modules so version comparisons match the upstream ecosystem
- Multiple-times-per-day release cadence (43,000+ versions on npm, 5,000+ GitHub releases)
- Datasource modules covering npm, PyPI, Docker, Maven, NuGet, GitHub releases/tags, GitLab releases/tags, Helm, crate, Go proxy, Conan, RubyGems, packagist, Galaxy, Terraform Registry, and more
- Pre-commit hook support
- JSON Schema (renovate-schema.json) for IDE validation of every configuration option
image: https://kinlane-images.s3.amazonaws.com/shared/apis-json/icons/renovate-bot.png
layout: provider
modified: '2026-09-16'
name: Renovate
nav: Providers
network: true
overview: 'Renovate publishes 6 APIs on the [APIs.io](https://apis.io/) network. Tagged areas include DependencyUpdates, Developer Tools, Bots, Automation, and Software Supply Chain.


  Renovate''s developer surface includes developer portal, documentation, getting-started guide, signup flow, changelog, support, tooling, and 43 more developer resources.'
random_paper: 6
score:
  band: thin
  composite: 28.9
  coverage:
    artifact_dirs: 4
    catalog_earned: 41.0
    catalog_earned_first_party: 0.0
    catalog_gap: 74.0
    catalog_max: 115.0
    note: 'Disclosure, not a penalty. catalog_gap is rubric points API Evangelist could add with no action by this provider, and it is NOT subtracted from the composite above. It is our backlog EXCEPT where this provider already did the work: catalog_earned is how much of the class was satisfied at all, and catalog_earned_first_party how much of that came from artifacts the provider published rather than ones we generated (roadmap#221). catalog_earned_first_party is a FLOOR, not the whole share: only ~40 of the rubric''s 113 checks carry a provenance class at all, so a check we cannot attribute counts toward neither side. Read it as "at least this much was theirs", never as "the rest was ours".'
  delta: 0.0
  facets:
    access_clarity: 34.2
    contract_governance: 0.0
    contract_quality: 14.7
    developer_ergonomics: 40.5
    discoverability: 64.8
    operational_transparency: 28.9
  previous_composite: 28.9
  schema_version: 0.22.0
  scored_at: '2026-09-17'
  trend: flat
  upsert:
    applies: false
    note: 'Not scored: no parseable contract to read. Never-measured is not the same fact as measured-empty, so this is absent rather than zero.'
    reason: no_specs
screenshot: https://raw.githubusercontent.com/api-evangelist/renovate-bot/refs/heads/main/screenshots/renovate-bot-2026-06-20T192849.png
security:
- kind: domain-security
  name: Renovate Bot Domain Security
  slug: renovate-bot-domain-security
  summary_line: TLSv1.3 · HSTS · DMARC
- kind: trust-center
  name: Renovate Bot Trust Center
  slug: renovate-bot-trust-center
  summary_line: trust center published
slug: renovate-bot
tags:
- DependencyUpdates
- Developer Tools
- Bots
- Automation
- Software Supply Chain
- Open-Source
- Mend
- AGPLv3
- Node.js
- TypeScript
website: https://renovatebot.com
---
