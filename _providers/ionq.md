---
access_model:
  confidence: medium
  label: Freemium · Self-serve signup
  onboarding: self-serve
  pricing: freemium
  public: false
  source:
  - plans
  - authentication
  - security
  trial: false
  try_now: true
agent_readiness:
  band: agent-aware
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: derived
    agentic_commerce: false
    auth_clarity: bearer
    consent_identity: false
    delegated_identity: false
    dry_run_mode: false
    dynamic_client_registration: false
    error_semantics: documented
    event_surface_described: false
    idempotency: false
    mcp_server: false
    openapi_examples: partial
    protected_resource_metadata: false
    rate_limit_signal: documented
    reversibility_documented: documented
    spec_presence: true
    well_known_catalog: false
  schema_version: '0.2'
  score: 26.9
  scored_at: '2026-09-25'
agentic_access:
- acting_count: 7
  human_in_the_loop: 0
  name: Ionq Agentic Access
  operation_count: 25
  slug: ionq-agentic-access
  summary_line: 25 operations · 7 acting
api_count: 1
apis:
- description: The previous-generation IonQ Quantum Cloud REST API (v0.3) for submitting and managing jobs, characterizations, backends, and reports. Superseded by v0.4 which adds Sessions, separated cost/results en
  name: IonQ Quantum Cloud API (v0.3 Legacy)
  slug: ionq-quantum-cloud-api-v0-3
- baseURL: https://api.ionq.co/v0.4
  baseurl_source: declared
  description: The backends API from IonQ — 2 operation(s) for backends.
  name: IonQ Backends API
  slug: ionq-backends-api
- baseURL: https://api.ionq.co/v0.4
  baseurl_source: declared
  description: The characterizations API from IonQ — 2 operation(s) for characterizations.
  name: IonQ Characterizations API
  slug: ionq-characterizations-api
- baseURL: https://api.ionq.co/v0.4
  baseurl_source: declared
  description: The Jobs API from IonQ — 11 operation(s) for jobs.
  name: IonQ Jobs API
  slug: ionq-jobs-api
- baseURL: https://api.ionq.co/v0.4
  baseurl_source: declared
  description: The Sessions API from IonQ — 4 operation(s) for sessions.
  name: IonQ Sessions API
  slug: ionq-sessions-api
- baseURL: https://api.ionq.co/v0.4
  baseurl_source: declared
  description: The usage API from IonQ — 1 operation(s) for usage.
  name: IonQ Usage API
  slug: ionq-usage-api
- baseURL: https://api.ionq.co/v0.4
  baseurl_source: declared
  description: The whoami API from IonQ — 1 operation(s) for whoami.
  name: IonQ Whoami API
  slug: ionq-whoami-api
artifact_total: 55
collections:
- collection_type: postman
  name: IonQ Cloud Platform backends API
  slug: postman-ionq-backends-api
- collection_type: postman
  name: IonQ Cloud Platform backends characterizations API
  slug: postman-ionq-characterizations-api
- collection_type: postman
  name: IonQ Cloud Platform backends Jobs API
  slug: postman-ionq-jobs-api
- collection_type: postman
  name: IonQ Cloud Platform backends Sessions API
  slug: postman-ionq-sessions-api
- collection_type: postman
  name: IonQ Cloud Platform backends usage API
  slug: postman-ionq-usage-api
- collection_type: postman
  name: IonQ Cloud Platform backends whoami API
  slug: postman-ionq-whoami-api
- collection_type: open
  name: API Collection
  slug: open-.refine-report
- collection_type: open
  name: IonQ Cloud Platform backends API
  slug: open-ionq-backends-api
- collection_type: open
  name: IonQ Cloud Platform backends characterizations API
  slug: open-ionq-characterizations-api
- collection_type: open
  name: IonQ Cloud Platform backends Jobs API
  slug: open-ionq-jobs-api
- collection_type: open
  name: IonQ Cloud Platform API
  slug: open-ionq-quantum-cloud
- collection_type: open
  name: IonQ Cloud Platform backends Sessions API
  slug: open-ionq-sessions-api
- collection_type: open
  name: IonQ Cloud Platform backends usage API
  slug: open-ionq-usage-api
- collection_type: open
  name: IonQ Cloud Platform backends whoami API
  slug: open-ionq-whoami-api
common:
- group: company
  title: ''
  type: Website
  url: https://www.ionq.com/
- group: build
  title: ''
  type: PostmanWorkspace
  url: https://www.postman.com/kinlaneapi/ionq/overview
- group: agent
  href: https://raw.githubusercontent.com/api-evangelist/ionq/refs/heads/main/agentic-access/ionq-agentic-access.yml
  title: ''
  type: AgenticAccess
  url: agentic-access/ionq-agentic-access.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/ionq/refs/heads/main/security/ionq-domain-security.yml
  title: ''
  type: DomainSecurity
  url: security/ionq-domain-security.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/ionq/refs/heads/main/authentication/ionq-authentication.yml
  title: ''
  type: Authentication
  url: authentication/ionq-authentication.yml
- group: start
  title: ''
  type: Portal
  url: https://ionq.com
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/api-reference/v0.4/introduction
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/api-reference/v0.3/introduction
- group: start
  title: ''
  type: Portal
  url: https://cloud.ionq.com
- group: start
  title: ''
  type: Signup
  url: https://cloud.ionq.com
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/user-manual/introduction
- group: other
  title: ''
  type: Glossary
  url: https://docs.ionq.com/user-manual/glossary
- group: auth
  title: ''
  type: Authentication
  url: https://docs.ionq.com/guides/managing-api-keys
- group: auth
  title: ''
  type: Authentication
  url: https://docs.ionq.com/guides/connecting-saml-identity-providers
- group: auth
  title: ''
  type: Authentication
  url: https://docs.ionq.com/guides/dotenv-project-api-keys
- group: start
  title: ''
  type: GettingStarted
  url: https://docs.ionq.com/guides/direct-api-submission
- group: start
  title: ''
  type: GettingStarted
  url: https://docs.ionq.com/guides/getting-started-with-native-gates
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/guides/cloud-usage
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/guides/error-mitigation-debiasing
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/guides/qpu-submission-checklist
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/guides/hosted-hybrid-service
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/guides/simulation-with-noise-models
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/guides/qaoa-solve-with-fire-opal
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/user-manual/platform-systems
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/user-manual/backends
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/user-manual/jobs
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/user-manual/projects
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/user-manual/organizations
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/user-manual/accounts
- group: docs
  title: ''
  type: Documentation
  url: https://docs.ionq.com/partners/amazon-braket
- group: design
  title: ''
  type: Versioning
  url: https://docs.ionq.com/api-reference/v0.4/migration-from-v0.3
- group: design
  title: ''
  type: ErrorCodes
  url: https://docs.ionq.com/api-reference/v0.3/error-codes
- group: operate
  title: ''
  type: StatusPage
  url: https://status.ionq.co
- group: operate
  title: ''
  type: Support
  url: https://support.ionq.com
- group: operate
  title: ''
  type: Support
  url: mailto:support@ionq.com
- group: operate
  title: ''
  type: Contact
  url: https://ionq.com/contact
- group: company
  title: ''
  type: Blog
  url: https://ionq.com/news
- group: company
  title: ''
  type: Blog
  url: https://ionq.com/posts
- group: docs
  title: ''
  type: Documentation
  url: https://ionq.com/resources
- group: start
  title: ''
  type: Portal
  url: https://ionq.com/quantum-cloud
- group: docs
  title: ''
  type: Documentation
  url: https://ionq.com/products
- group: docs
  title: ''
  type: Documentation
  url: https://ionq.com/company
- group: docs
  title: ''
  type: Documentation
  url: https://ionq.com/careers
- group: docs
  title: ''
  type: Documentation
  url: https://investors.ionq.com
- group: commercial
  title: ''
  type: TermsOfService
  url: https://ionq.com/legal/terms-of-service
- group: commercial
  title: ''
  type: PrivacyPolicy
  url: https://ionq.com/legal/privacy-policy
- group: docs
  title: ''
  type: Documentation
  url: https://ionq.com/legal
- group: build
  title: ''
  type: GitHubOrganization
  url: https://github.com/ionq
- group: build
  title: ''
  type: SDKs
  url: https://github.com/ionq/ionq-core-python
- group: build
  title: ''
  type: SDKs
  url: https://pypi.org/project/ionq-core/
- group: build
  title: ''
  type: SDKs
  url: https://github.com/ionq/blqs
- group: build
  title: ''
  type: Tools
  url: https://github.com/ionq/pyGSTi
- group: build
  title: ''
  type: Tools
  url: https://github.com/ionq/qrmi
- group: build
  title: ''
  type: Tools
  url: https://github.com/ionq/torchquantum-dist
- group: build
  title: ''
  type: Tools
  url: https://github.com/ionq/QC-App-Oriented-Benchmarks
- group: build
  title: ''
  type: CodeExamples
  url: https://github.com/ionq/forte_benchmarking
- group: build
  title: ''
  type: CodeExamples
  url: https://github.com/ionq/forte_aq29_data
- group: build
  title: ''
  type: SDKs
  url: https://docs.ionq.com/sdks/qiskit/index
- group: build
  title: ''
  type: SDKs
  url: https://docs.ionq.com/sdks/cirq/index
- group: build
  title: ''
  type: SDKs
  url: https://docs.ionq.com/sdks/pennylane/index
- group: build
  title: ''
  type: SDKs
  url: https://docs.ionq.com/sdks/qbraid/index
- group: build
  title: ''
  type: SDKs
  url: https://docs.ionq.com/sdks/cuda-q
- group: build
  title: ''
  type: SDKs
  url: https://docs.ionq.com/sdks/tensorflow
- group: company
  title: ''
  type: LinkedIn
  url: https://www.linkedin.com/company/ionq
- group: company
  title: ''
  type: Twitter
  url: https://x.com/ionq_inc
- group: learn
  title: ''
  type: YouTube
  url: https://www.youtube.com/@ionq_inc
- group: commercial
  href: https://raw.githubusercontent.com/api-evangelist/ionq/refs/heads/main/plans/ionq-plans-pricing.yml
  title: ''
  type: Plans
  url: plans/ionq-plans-pricing.yml
- group: operate
  href: https://raw.githubusercontent.com/api-evangelist/ionq/refs/heads/main/rate-limits/ionq-rate-limits.yml
  title: ''
  type: RateLimits
  url: rate-limits/ionq-rate-limits.yml
- group: commercial
  href: https://raw.githubusercontent.com/api-evangelist/ionq/refs/heads/main/finops/ionq-finops.yml
  title: ''
  type: FinOps
  url: finops/ionq-finops.yml
created: '2026-05-24T00:00:00.000Z'
description: IonQ (NYSE IONQ) is a College Park, Maryland-based public trapped-ion quantum computing company. IonQ designs, manufactures, and operates trapped-ion quantum computers (Aria, Forte, Forte Enterprise, Tempo, Harmony) and delivers them through the IonQ Quantum Cloud REST API, on-premises Forte Enterprise systems, and partner clouds (AWS Braket, Microsoft Azure Quantum, Google Cloud). The platform supports submission of QIS and native-gate circuits, multi-circuit and Quantum Function jobs, reserved Sessions, error mitigation, and organization-level usage and cost reporting. IonQ's quantum networking, security (QKD), sensing, and space-infrastructure businesses extend the trapped-ion core into a broader quantum systems portfolio.
examples:
- key_count: 7
  name: Ionq Create Job Example
  slug: ionq-create-job-example
- key_count: 2
  name: Ionq Create Session Example
  slug: ionq-create-session-example
features:
- Trapped-ion QPUs with 99.99% world-record two-qubit gate fidelity
- Cloud access to qpu.aria-1, qpu.aria-2, qpu.forte-1, qpu.forte-enterprise-1, and Tempo systems
- On-premises Forte Enterprise rack-mounted quantum system
- Ideal and noisy IonQ simulator (up to 29 qubits) with Aria and Forte noise models
- REST API v0.4 with Jobs, Sessions, Backends, Characterizations, Cost, and Usage endpoints
- Sessions API (beta) for reserved-block QPU execution with cost, duration, and job-count limits
- Per-job cost endpoint plus organization-level usage aggregation in USD
- Per-job estimate endpoint for cost projection before submission
- Multi-circuit job submissions and Quantum Function jobs (QAOA, Hamiltonian energy)
- QIS gateset and native-gate (GPI, GPI2, MS, ZZ) circuit inputs
- Error mitigation via symmetric debiasing
- Project-scoped API keys with .env workflow and SAML SSO for identity federation
- Native integrations with Qiskit, Cirq, PennyLane, qBraid, CUDA-Q, TensorFlow Quantum
- Partner cloud access via AWS Braket, Microsoft Azure Quantum, and Google Cloud Marketplace
- ionq-core Python client generated from the OpenAPI spec under Apache 2.0
- Hosted hybrid service for classical/quantum loop execution
- Q-CTRL Fire Opal integration for QAOA performance enhancement
- Forte Enterprise on-premises deployment option
- Quantum networking, QKD, sensing, and space-based earth-observation product lines (Florida LambdaRail statewide quantum-safe network)
finops:
- name: Ionq Finops
  service_category: ''
  slug: ionq-finops
image: https://kinlane-images.s3.amazonaws.com/shared/apis-json/icons/ionq.png
json_schemas:
- name: IonQ Backend
  property_count: 9
  slug: ionq-backend
- name: IonQ Job
  property_count: 14
  slug: ionq-job
- name: IonQ Session
  property_count: 12
  slug: ionq-session
json_structures:
- name: Ionq Job Structure
  property_count: 0
  slug: ionq-job-structure
jsonld:
- class_count: 29
  name: Ionq Context
  property_count: 2
  slug: ionq-context
layout: provider
modified: '2026-05-24'
name: IonQ
nav: Providers
network: true
overview: 'IonQ publishes 7 APIs on the [APIs.io](https://apis.io/) network, including Backends API, Characterizations API, Jobs API, and 4 more. Tagged areas include Quantum Computing, Trapped Ion, QPU, ARIA, and Forte.


  The IonQ catalog on APIs.io includes 1 JSON-LD context and 2 Spectral governance rulesets.


  IonQ''s developer surface includes authentication, developer portal, documentation, signup flow, getting-started guide, support, engineering blog, and 63 more developer resources.'
plans:
- name: Ionq Plans Pricing
  plan_count: 4
  slug: ionq-plans-pricing
random_paper: 21
rate_limits:
- limit_count: 0
  name: Ionq Rate Limits
  slug: ionq-rate-limits
rules:
- effective_rule_count: 5
  extends: []
  name: IonQ API Rules
  rule_count: 5
  severity_counts:
    error: 0
    hint: 0
    info: 1
    warn: 4
  slug: ionq-jsonschema-spectral-rules
- effective_rule_count: 47
  extends:
  - spectral:oas
  name: IonQ API Rules
  rule_count: 6
  severity_counts:
    error: 2
    hint: 0
    info: 0
    warn: 4
  slug: ionq-rules
score:
  band: strong
  composite: 56.7
  coverage:
    artifact_dirs: 18
    catalog_earned: 72.3
    catalog_earned_first_party: 0.0
    catalog_gap: 42.7
    catalog_max: 115.0
    note: 'Disclosure, not a penalty. catalog_gap is rubric points API Evangelist could add with no action by this provider, and it is NOT subtracted from the composite above. It is our backlog EXCEPT where this provider already did the work: catalog_earned is how much of the class was satisfied at all, and catalog_earned_first_party how much of that came from artifacts the provider published rather than ones we generated (roadmap#221). catalog_earned_first_party is a FLOOR, not the whole share: only ~40 of the rubric''s 113 checks carry a provenance class at all, so a check we cannot attribute counts toward neither side. Read it as "at least this much was theirs", never as "the rest was ours".'
  delta: -1.1
  facets:
    access_clarity: 70.5
    contract_governance: 13.6
    contract_quality: 61.5
    developer_ergonomics: 67.9
    discoverability: 66.1
    operational_transparency: 36.8
  jurisdiction:
    basis: provider tags (build_countries.py / build_regions.py)
    note: A first approximation of where this provider operates, derived from the tags on its profile. NOT a legal determination of domicile or regulatory scope, and it does not yet decide which regimes the regulatory facet evaluates (roadmap#85).
    regions:
    - north-america
  previous_composite: 57.8
  provenance:
    agentic_access: derived
    contracts:
      callable: 100.0
      derived: 0
      marker_coverage: 0.0
      total: 6
  regulatory:
    applies: true
    matched_via: fallback
    regime: Horizontal (data, software, accessibility, platform)
    regime_id: horizontal
    score: 24.5
  schema_version: 0.23.0
  scored_at: '2026-09-25'
  trend: flat
  upsert:
    applies: true
    score: 0.0
screenshot: https://raw.githubusercontent.com/api-evangelist/ionq/refs/heads/main/screenshots/ionq-2026-06-20T183533.png
security:
- kind: authentication
  name: Ionq Authentication
  slug: ionq-authentication
  summary_line: apiKey · 1 scheme
- kind: domain-security
  name: Ionq Domain Security
  slug: ionq-domain-security
  summary_line: TLSv1.3 · HSTS · DMARC
slug: ionq
tags:
- Quantum Computing
- Trapped Ion
- QPU
- ARIA
- Forte
- Tempo
- Quantum Cloud
- Cloud
- Hardware
- Public Company
website: https://www.ionq.com/
---
