...
access_model:
  confidence: probed
  label: 'No key required by the API, but the host serves no anonymous programmatic request: every path answers 403 behind a Cloudflare managed challenge, and robots.txt disallows /books/ -- the only API endpoint -- to every user-agent.'
  onboarding: open
  pricing: free
  public: false
  source:
