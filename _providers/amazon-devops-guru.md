---
access_model:
  confidence: medium
  label: Freemium · Self-serve signup
  onboarding: self-serve
  pricing: freemium
  public: false
  source:
  - plans
  - authentication
  - security
  trial: false
  try_now: true
agent_readiness:
  band: agent-ready
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: derived
    agentic_commerce: false
    auth_clarity: bearer
    consent_identity: false
    delegated_identity: false
    dry_run_mode: false
    dynamic_client_registration: false
    error_semantics: verified
    event_surface_described: false
    idempotency: verified
    mcp_server: false
    openapi_examples: partial
    protected_resource_metadata: false
    rate_limit_signal: documented
    reversibility_documented: false
    spec_presence: true
    well_known_catalog: false
  schema_version: '0.2'
  score: 34.5
  scored_at: '2026-09-16'
agentic_access:
- acting_count: 24
  human_in_the_loop: 0
  name: Amazon Devops Guru Agentic Access
  operation_count: 31
  slug: amazon-devops-guru-agentic-access
  summary_line: 31 operations · 24 acting
api_count: 1
apis:
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Accounts API from Amazon DevOps Guru — 3 operation(s) for accounts.
  name: Amazon DevOps Guru Accounts API
  slug: amazon-devops-guru-accounts-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Anomalies API from Amazon DevOps Guru — 2 operation(s) for anomalies.
  name: Amazon DevOps Guru Anomalies API
  slug: amazon-devops-guru-anomalies-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Channels API from Amazon DevOps Guru — 2 operation(s) for channels.
  name: Amazon DevOps Guru Channels API
  slug: amazon-devops-guru-channels-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Cost Estimation API from Amazon DevOps Guru — 1 operation(s) for cost estimation.
  name: Amazon DevOps Guru Cost Estimation API
  slug: amazon-devops-guru-cost-estimation-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Event Sources API from Amazon DevOps Guru — 1 operation(s) for event sources.
  name: Amazon DevOps Guru Event Sources API
  slug: amazon-devops-guru-event-sources-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Events API from Amazon DevOps Guru — 1 operation(s) for events.
  name: Amazon DevOps Guru Events API
  slug: amazon-devops-guru-events-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Feedback API from Amazon DevOps Guru — 1 operation(s) for feedback.
  name: Amazon DevOps Guru Feedback API
  slug: amazon-devops-guru-feedback-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Insights API from Amazon DevOps Guru — 3 operation(s) for insights.
  name: Amazon DevOps Guru Insights API
  slug: amazon-devops-guru-insights-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The List Log Anomalies API from Amazon DevOps Guru — 1 operation(s) for list log anomalies.
  name: Amazon DevOps Guru List Log Anomalies API
  slug: amazon-devops-guru-list-log-anomalies-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Organization API from Amazon DevOps Guru — 5 operation(s) for organization.
  name: Amazon DevOps Guru Organization API
  slug: amazon-devops-guru-organization-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Recommendations API from Amazon DevOps Guru — 1 operation(s) for recommendations.
  name: Amazon DevOps Guru Recommendations API
  slug: amazon-devops-guru-recommendations-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Resource Collections API from Amazon DevOps Guru — 2 operation(s) for resource collections.
  name: Amazon DevOps Guru Resource Collections API
  slug: amazon-devops-guru-resource-collections-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Service Integrations API from Amazon DevOps Guru — 1 operation(s) for service integrations.
  name: Amazon DevOps Guru Service Integrations API
  slug: amazon-devops-guru-service-integrations-api
- baseURL: https://devops-guru.amazonaws.com
  baseurl_source: declared
  description: The Monitored Resources API from Amazon DevOps Guru — 1 operation(s) for monitored resources.
  name: Amazon DevOps Guru Monitored Resources API
  slug: amazon-devops-guru-monitored-resources-api
artifact_total: 133
collections:
- collection_type: postman
  name: Amazon DevOps Guru Accounts API
  slug: postman-amazon-devops-guru-accounts-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Anomalies API
  slug: postman-amazon-devops-guru-anomalies-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Channels API
  slug: postman-amazon-devops-guru-channels-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Cost Estimation API
  slug: postman-amazon-devops-guru-cost-estimation-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Event Sources API
  slug: postman-amazon-devops-guru-event-sources-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Events API
  slug: postman-amazon-devops-guru-events-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Feedback API
  slug: postman-amazon-devops-guru-feedback-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Insights API
  slug: postman-amazon-devops-guru-insights-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts List Log Anomalies API
  slug: postman-amazon-devops-guru-list-log-anomalies-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts MonitoredResources API
  slug: postman-amazon-devops-guru-monitoredresources-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Organization API
  slug: postman-amazon-devops-guru-organization-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Recommendations API
  slug: postman-amazon-devops-guru-recommendations-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Resource Collections API
  slug: postman-amazon-devops-guru-resource-collections-api
- collection_type: postman
  name: Amazon DevOps Guru Accounts Service Integrations API
  slug: postman-amazon-devops-guru-service-integrations-api
- collection_type: open
  name: API Collection
  slug: open-.refine-report
- collection_type: open
  name: Amazon DevOps Guru Accounts API
  slug: open-amazon-devops-guru-accounts-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Anomalies API
  slug: open-amazon-devops-guru-anomalies-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Channels API
  slug: open-amazon-devops-guru-channels-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Cost Estimation API
  slug: open-amazon-devops-guru-cost-estimation-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Event Sources API
  slug: open-amazon-devops-guru-event-sources-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Events API
  slug: open-amazon-devops-guru-events-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Feedback API
  slug: open-amazon-devops-guru-feedback-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Insights API
  slug: open-amazon-devops-guru-insights-api
- collection_type: open
  name: Amazon DevOps Guru Accounts List Log Anomalies API
  slug: open-amazon-devops-guru-list-log-anomalies-api
- collection_type: open
  name: Amazon DevOps Guru Accounts MonitoredResources API
  slug: open-amazon-devops-guru-monitoredresources-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Organization API
  slug: open-amazon-devops-guru-organization-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Recommendations API
  slug: open-amazon-devops-guru-recommendations-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Resource Collections API
  slug: open-amazon-devops-guru-resource-collections-api
- collection_type: open
  name: Amazon DevOps Guru Accounts Service Integrations API
  slug: open-amazon-devops-guru-service-integrations-api
common:
- group: build
  title: ''
  type: PostmanWorkspace
  url: https://www.postman.com/kinlaneapi/amazon-devops-guru/overview
- group: agent
  href: https://raw.githubusercontent.com/api-evangelist/amazon-devops-guru/refs/heads/main/agentic-access/amazon-devops-guru-agentic-access.yml
  title: ''
  type: AgenticAccess
  url: agentic-access/amazon-devops-guru-agentic-access.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/amazon-devops-guru/refs/heads/main/security/amazon-devops-guru-trust-center.yml
  title: ''
  type: TrustCenter
  url: security/amazon-devops-guru-trust-center.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/amazon-devops-guru/refs/heads/main/security/amazon-devops-guru-vulnerability-disclosure.yml
  title: ''
  type: VulnerabilityDisclosure
  url: security/amazon-devops-guru-vulnerability-disclosure.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/amazon-devops-guru/refs/heads/main/security/amazon-devops-guru-domain-security.yml
  title: ''
  type: DomainSecurity
  url: security/amazon-devops-guru-domain-security.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/amazon-devops-guru/refs/heads/main/authentication/amazon-devops-guru-authentication.yml
  title: ''
  type: Authentication
  url: authentication/amazon-devops-guru-authentication.yml
- group: start
  title: ''
  type: Portal
  url: https://aws.amazon.com/devops-guru/
- group: company
  title: ''
  type: Website
  url: https://aws.amazon.com/devops-guru/
- group: docs
  title: ''
  type: Documentation
  url: https://docs.aws.amazon.com/devops-guru/
- group: commercial
  title: ''
  type: TermsOfService
  url: https://aws.amazon.com/service-terms/
- group: commercial
  title: ''
  type: PrivacyPolicy
  url: https://aws.amazon.com/privacy/
- group: operate
  title: ''
  type: Support
  url: https://aws.amazon.com/premiumsupport/
- group: company
  title: ''
  type: Blog
  url: https://aws.amazon.com/blogs/devops/category/artificial-intelligence/amazon-devops-guru/
- group: build
  title: ''
  type: GitHubOrganization
  url: https://github.com/aws
- group: start
  title: ''
  type: Console
  url: https://console.aws.amazon.com/devops-guru/
- group: start
  title: ''
  type: Signup
  url: https://portal.aws.amazon.com/billing/signup
- group: start
  title: ''
  type: Login
  url: https://signin.aws.amazon.com/
- group: operate
  title: ''
  type: StatusPage
  url: https://health.aws.amazon.com/health/status
- group: operate
  title: ''
  type: Contact
  url: https://aws.amazon.com/contact-us/
- group: design
  href: https://raw.githubusercontent.com/api-evangelist/amazon-devops-guru/refs/heads/main/rules/amazon-devops-guru-spectral-rules.yml
  title: ''
  type: SpectralRules
  url: rules/amazon-devops-guru-spectral-rules.yml
- group: design
  href: https://raw.githubusercontent.com/api-evangelist/amazon-devops-guru/refs/heads/main/vocabulary/amazon-devops-guru-vocabulary.yaml
  title: ''
  type: Vocabulary
  url: vocabulary/amazon-devops-guru-vocabulary.yaml
created: '2026-03-16'
description: Amazon DevOps Guru is a machine learning-powered service that makes it easy to improve an application's operational performance and availability. It detects behaviors that deviate from normal operating patterns so you can identify operational issues long before they impact your customers.
examples:
- key_count: 5
  name: Amazon Devops Guru Anomalous Log Group Example
  slug: amazon-devops-guru-anomalous-log-group-example
- key_count: 1
  name: Amazon Devops Guru Cloud Formation Collection Example
  slug: amazon-devops-guru-cloud-formation-collection-example
- key_count: 3
  name: Amazon Devops Guru Cloud Formation Health Example
  slug: amazon-devops-guru-cloud-formation-health-example
- key_count: 8
  name: Amazon Devops Guru Event Example
  slug: amazon-devops-guru-event-example
- key_count: 1
  name: Amazon Devops Guru Event Sources Config Example
  slug: amazon-devops-guru-event-sources-config-example
- key_count: 3
  name: Amazon Devops Guru Insight Health Example
  slug: amazon-devops-guru-insight-health-example
- key_count: 1
  name: Amazon Devops Guru Log Anomaly Showcase Example
  slug: amazon-devops-guru-log-anomaly-showcase-example
- key_count: 2
  name: Amazon Devops Guru Notification Channel Example
  slug: amazon-devops-guru-notification-channel-example
- key_count: 1
  name: Amazon Devops Guru Ops Center Integration Example
  slug: amazon-devops-guru-ops-center-integration-example
- key_count: 3
  name: Amazon Devops Guru Performance Insights Metric Query Example
  slug: amazon-devops-guru-performance-insights-metric-query-example
- key_count: 14
  name: Amazon Devops Guru Proactive Anomaly Example
  slug: amazon-devops-guru-proactive-anomaly-example
- key_count: 9
  name: Amazon Devops Guru Proactive Insight Example
  slug: amazon-devops-guru-proactive-insight-example
- key_count: 13
  name: Amazon Devops Guru Reactive Anomaly Example
  slug: amazon-devops-guru-reactive-anomaly-example
- key_count: 8
  name: Amazon Devops Guru Reactive Insight Example
  slug: amazon-devops-guru-reactive-insight-example
- key_count: 7
  name: Amazon Devops Guru Recommendation Example
  slug: amazon-devops-guru-recommendation-example
- key_count: 2
  name: Amazon Devops Guru Resource Collection Example
  slug: amazon-devops-guru-resource-collection-example
- key_count: 1
  name: Amazon Devops Guru Service Collection Example
  slug: amazon-devops-guru-service-collection-example
- key_count: 3
  name: Amazon Devops Guru Service Health Example
  slug: amazon-devops-guru-service-health-example
- key_count: 2
  name: Amazon Devops Guru Service Insight Health Example
  slug: amazon-devops-guru-service-insight-health-example
- key_count: 1
  name: Amazon Devops Guru Sns Channel Config Example
  slug: amazon-devops-guru-sns-channel-config-example
- key_count: 2
  name: Amazon Devops Guru Tag Collection Example
  slug: amazon-devops-guru-tag-collection-example
- key_count: 4
  name: Amazon Devops Guru Tag Health Example
  slug: amazon-devops-guru-tag-health-example
features:
- description: Uses machine learning to detect behavioral deviations across hundreds of AWS metrics without manual threshold configuration.
  name: ML-Powered Anomaly Detection
- description: Identifies anomalies before they become operational issues, allowing teams to remediate before customer impact.
  name: Proactive Insights
- description: Surfaces insights when active operational issues are detected to accelerate root cause analysis.
  name: Reactive Insights
- description: Provides specific remediation recommendations with links to relevant documentation and AWS console pages.
  name: Actionable Recommendations
- description: Analyzes CloudWatch Logs for log-based anomalies to include log patterns in operational insights.
  name: CloudWatch Logs Integration
- description: Automatically creates OpsCenter OpsItems for detected insights to streamline incident management.
  name: AWS OpsCenter Integration
- description: Define which applications to monitor by specifying CloudFormation stack names for precise application-scoped coverage.
  name: CloudFormation-Based Coverage
- description: Enable DevOps Guru across an entire AWS Organization to centrally monitor all accounts and regions.
  name: Organizations Integration
finops:
- name: Amazon Devops Guru Finops
  service_category: API
  slug: amazon-devops-guru-finops
image: https://a0.awsstatic.com/libra-css/images/logos/aws_logo_smile_1200x630.png
json_schemas:
- name: AnomalousLogGroup
  property_count: 5
  slug: amazon-devops-guru-anomalous-log-group
- name: CloudFormationCollection
  property_count: 1
  slug: amazon-devops-guru-cloud-formation-collection
- name: CloudFormationHealth
  property_count: 3
  slug: amazon-devops-guru-cloud-formation-health
- name: Event
  property_count: 8
  slug: amazon-devops-guru-event
- name: EventSourcesConfig
  property_count: 1
  slug: amazon-devops-guru-event-sources-config
- name: InsightHealth
  property_count: 3
  slug: amazon-devops-guru-insight-health
- name: LogAnomalyShowcase
  property_count: 1
  slug: amazon-devops-guru-log-anomaly-showcase
- name: NotificationChannel
  property_count: 2
  slug: amazon-devops-guru-notification-channel
- name: OpsCenterIntegration
  property_count: 1
  slug: amazon-devops-guru-ops-center-integration
- name: PerformanceInsightsMetricQuery
  property_count: 3
  slug: amazon-devops-guru-performance-insights-metric-query
- name: ProactiveAnomaly
  property_count: 14
  slug: amazon-devops-guru-proactive-anomaly
- name: ProactiveInsight
  property_count: 9
  slug: amazon-devops-guru-proactive-insight
- name: ReactiveAnomaly
  property_count: 13
  slug: amazon-devops-guru-reactive-anomaly
- name: ReactiveInsight
  property_count: 8
  slug: amazon-devops-guru-reactive-insight
- name: Recommendation
  property_count: 7
  slug: amazon-devops-guru-recommendation
- name: ResourceCollection
  property_count: 2
  slug: amazon-devops-guru-resource-collection
- name: ServiceCollection
  property_count: 1
  slug: amazon-devops-guru-service-collection
- name: ServiceHealth
  property_count: 3
  slug: amazon-devops-guru-service-health
- name: ServiceInsightHealth
  property_count: 2
  slug: amazon-devops-guru-service-insight-health
- name: SnsChannelConfig
  property_count: 1
  slug: amazon-devops-guru-sns-channel-config
- name: TagCollection
  property_count: 2
  slug: amazon-devops-guru-tag-collection
- name: TagHealth
  property_count: 4
  slug: amazon-devops-guru-tag-health
json_structures:
- name: Amazon Devops Guru Anomalous Log Group Structure
  property_count: 5
  slug: amazon-devops-guru-anomalous-log-group-structure
- name: Amazon Devops Guru Cloud Formation Collection Structure
  property_count: 1
  slug: amazon-devops-guru-cloud-formation-collection-structure
- name: Amazon Devops Guru Cloud Formation Health Structure
  property_count: 3
  slug: amazon-devops-guru-cloud-formation-health-structure
- name: Amazon Devops Guru Event Sources Config Structure
  property_count: 1
  slug: amazon-devops-guru-event-sources-config-structure
- name: Amazon Devops Guru Event Structure
  property_count: 8
  slug: amazon-devops-guru-event-structure
- name: Amazon Devops Guru Insight Health Structure
  property_count: 3
  slug: amazon-devops-guru-insight-health-structure
- name: Amazon Devops Guru Log Anomaly Showcase Structure
  property_count: 1
  slug: amazon-devops-guru-log-anomaly-showcase-structure
- name: Amazon Devops Guru Notification Channel Structure
  property_count: 2
  slug: amazon-devops-guru-notification-channel-structure
- name: Amazon Devops Guru Ops Center Integration Structure
  property_count: 1
  slug: amazon-devops-guru-ops-center-integration-structure
- name: Amazon Devops Guru Performance Insights Metric Query Structure
  property_count: 3
  slug: amazon-devops-guru-performance-insights-metric-query-structure
- name: Amazon Devops Guru Proactive Anomaly Structure
  property_count: 14
  slug: amazon-devops-guru-proactive-anomaly-structure
- name: Amazon Devops Guru Proactive Insight Structure
  property_count: 9
  slug: amazon-devops-guru-proactive-insight-structure
- name: Amazon Devops Guru Reactive Anomaly Structure
  property_count: 13
  slug: amazon-devops-guru-reactive-anomaly-structure
- name: Amazon Devops Guru Reactive Insight Structure
  property_count: 8
  slug: amazon-devops-guru-reactive-insight-structure
- name: Amazon Devops Guru Recommendation Structure
  property_count: 7
  slug: amazon-devops-guru-recommendation-structure
- name: Amazon Devops Guru Resource Collection Structure
  property_count: 2
  slug: amazon-devops-guru-resource-collection-structure
- name: Amazon Devops Guru Service Collection Structure
  property_count: 1
  slug: amazon-devops-guru-service-collection-structure
- name: Amazon Devops Guru Service Health Structure
  property_count: 3
  slug: amazon-devops-guru-service-health-structure
- name: Amazon Devops Guru Service Insight Health Structure
  property_count: 2
  slug: amazon-devops-guru-service-insight-health-structure
- name: Amazon Devops Guru Sns Channel Config Structure
  property_count: 1
  slug: amazon-devops-guru-sns-channel-config-structure
- name: Amazon Devops Guru Tag Collection Structure
  property_count: 2
  slug: amazon-devops-guru-tag-collection-structure
- name: Amazon Devops Guru Tag Health Structure
  property_count: 4
  slug: amazon-devops-guru-tag-health-structure
jsonld:
- class_count: 23
  name: Amazon Devops Guru Context
  property_count: 54
  slug: amazon-devops-guru-context
layout: provider
modified: '2026-05-19'
name: Amazon DevOps Guru
nav: Providers
network: true
overview: 'Amazon DevOps Guru publishes 14 APIs on the [APIs.io](https://apis.io/) network, including Accounts API, Anomalies API, Channels API, and 11 more. Tagged areas include Anomaly Detection, DevOps, Machine-Learning, and Operational Intelligence.


  The Amazon DevOps Guru catalog on APIs.io includes 1 JSON-LD context and 2 Spectral governance rulesets.


  Amazon DevOps Guru''s developer surface includes authentication, developer portal, documentation, support, engineering blog, developer console, signup flow, and 14 more developer resources.'
plans:
- name: Amazon Devops Guru Plans Pricing
  plan_count: 3
  slug: amazon-devops-guru-plans-pricing
random_paper: 18
rate_limits:
- limit_count: 5
  name: Amazon Devops Guru Rate Limits
  slug: amazon-devops-guru-rate-limits
rules:
- effective_rule_count: 5
  extends: []
  name: Amazon DevOps Guru API Rules
  rule_count: 5
  severity_counts:
    error: 0
    hint: 0
    info: 1
    warn: 4
  slug: amazon-devops-guru-jsonschema-spectral-rules
- effective_rule_count: 64
  extends:
  - spectral:oas
  name: Amazon DevOps Guru API Rules
  rule_count: 23
  severity_counts:
    error: 11
    hint: 0
    info: 2
    warn: 10
  slug: amazon-devops-guru-spectral-rules
score:
  band: developing
  composite: 53.4
  coverage:
    artifact_dirs: 17
    catalog_earned: 71.5
    catalog_earned_first_party: 0.0
    catalog_gap: 43.5
    catalog_max: 115.0
    note: 'Disclosure, not a penalty. catalog_gap is rubric points API Evangelist could add with no action by this provider, and it is NOT subtracted from the composite above. It is our backlog EXCEPT where this provider already did the work: catalog_earned is how much of the class was satisfied at all, and catalog_earned_first_party how much of that came from artifacts the provider published rather than ones we generated (roadmap#221). catalog_earned_first_party is a FLOOR, not the whole share: only ~40 of the rubric''s 113 checks carry a provenance class at all, so a check we cannot attribute counts toward neither side. Read it as "at least this much was theirs", never as "the rest was ours".'
  delta: 1.1
  facets:
    access_clarity: 61.8
    contract_governance: 28.8
    contract_quality: 75.5
    developer_ergonomics: 46.4
    discoverability: 66.7
    operational_transparency: 26.3
  previous_composite: 52.3
  provenance:
    agentic_access: derived
    contracts:
      callable: 100.0
      derived: 0
      marker_coverage: 0.0
      total: 14
  schema_version: 0.22.0
  scored_at: '2026-09-16'
  trend: flat
  upsert:
    applies: true
    score: 0.0
screenshot: https://raw.githubusercontent.com/api-evangelist/amazon-devops-guru/refs/heads/main/screenshots/amazon-devops-guru-2026-06-20T171630.png
security:
- kind: authentication
  name: Amazon Devops Guru Authentication
  slug: amazon-devops-guru-authentication
  summary_line: apiKey · 1 scheme
- kind: domain-security
  name: Amazon Devops Guru Domain Security
  slug: amazon-devops-guru-domain-security
  summary_line: TLSv1.3 · HSTS · DMARC
- kind: vulnerability-disclosure
  name: Amazon Devops Guru Vulnerability Disclosure
  slug: amazon-devops-guru-vulnerability-disclosure
  summary_line: security.txt · contact published
- kind: trust-center
  name: Amazon Devops Guru Trust Center
  slug: amazon-devops-guru-trust-center
  summary_line: PCI DSS, HIPAA, FedRAMP, GDPR, FIPS 140
slug: amazon-devops-guru
tags:
- Anomaly Detection
- DevOps
- Machine-Learning
- Operational Intelligence
use_cases:
- description: Detect potential issues in application behavior before they impact end users using ML-powered proactive insights.
  name: Proactive Operational Monitoring
- description: Rapidly identify the root cause of operational incidents by correlating anomalies, events, and recommendations.
  name: Incident Root Cause Analysis
- description: Use continuous behavioral monitoring to identify performance bottlenecks and optimization opportunities.
  name: Application Performance Optimization
- description: Monitor operational health across all accounts in an AWS Organization from a single pane of glass.
  name: Multi-Account Operations
- description: Integrate DevOps Guru insights into CI/CD pipelines to gate deployments on operational health status.
  name: DevOps Pipeline Integration
website: https://aws.amazon.com/devops-guru/
---
