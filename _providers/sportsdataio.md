---
access_model:
  confidence: high
  label: Freemium (free trial) · Self-serve signup
  onboarding: self-serve
  pricing: freemium
  public: false
  source:
  - plans
  - authentication
  trial: true
  try_now: true
agent_readiness:
  band: agent-ready
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: derived
    agentic_commerce: false
    auth_clarity: bearer
    consent_identity: false
    delegated_identity: false
    dry_run_mode: na
    dynamic_client_registration: false
    error_semantics: verified
    event_surface_described: false
    idempotency: na
    mcp_server: false
    openapi_examples: false
    protected_resource_metadata: false
    rate_limit_signal: documented
    reversibility_documented: na
    spec_presence: true
    well_known_catalog: false
  schema_version: '0.2'
  score: 29.6
  scored_at: '2026-09-17'
agentic_access:
- acting_count: 0
  human_in_the_loop: 0
  name: Sportsdataio Agentic Access
  operation_count: 467
  slug: sportsdataio-agentic-access
  summary_line: 467 operations
api_count: 5
apis:
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The MLB v3 Headshots API from SportsDataIO — 1 operation(s) for mlb v3 headshots.
  name: SportsDataIO MLB v3 Headshots API
  slug: sportsdataio-mlb-v3-headshots-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The MLB v3 Odds API from SportsDataIO — 34 operation(s) for mlb v3 odds.
  name: SportsDataIO MLB v3 Odds API
  slug: sportsdataio-mlb-v3-odds-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The MLB v3 Play-by-Play API from SportsDataIO — 3 operation(s) for mlb v3 play-by-play.
  name: SportsDataIO MLB v3 Play-by-Play API
  slug: sportsdataio-mlb-v3-play-by-play-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The MLB v3 Projections API from SportsDataIO — 8 operation(s) for mlb v3 projections.
  name: SportsDataIO MLB v3 Projections API
  slug: sportsdataio-mlb-v3-projections-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The MLB v3 RotoBaller Premium News API from SportsDataIO — 2 operation(s) for mlb v3 rotoballer premium news.
  name: SportsDataIO MLB v3 RotoBaller Premium News API
  slug: sportsdataio-mlb-v3-rotoballer-premium-news-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The MLB v3 Scores API from SportsDataIO — 27 operation(s) for mlb v3 scores.
  name: SportsDataIO MLB v3 Scores API
  slug: sportsdataio-mlb-v3-scores-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The MLB v3 Stats API from SportsDataIO — 17 operation(s) for mlb v3 stats.
  name: SportsDataIO MLB v3 Stats API
  slug: sportsdataio-mlb-v3-stats-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NBA v3 Headshots API from SportsDataIO — 1 operation(s) for nba v3 headshots.
  name: SportsDataIO NBA v3 Headshots API
  slug: sportsdataio-nba-v3-headshots-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NBA v3 Odds API from SportsDataIO — 34 operation(s) for nba v3 odds.
  name: SportsDataIO NBA v3 Odds API
  slug: sportsdataio-nba-v3-odds-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NBA v3 Play-by-Play API from SportsDataIO — 3 operation(s) for nba v3 play-by-play.
  name: SportsDataIO NBA v3 Play-by-Play API
  slug: sportsdataio-nba-v3-play-by-play-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NBA v3 Projections API from SportsDataIO — 6 operation(s) for nba v3 projections.
  name: SportsDataIO NBA v3 Projections API
  slug: sportsdataio-nba-v3-projections-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NBA v3 RotoBaller Premium News API from SportsDataIO — 2 operation(s) for nba v3 rotoballer premium news.
  name: SportsDataIO NBA v3 RotoBaller Premium News API
  slug: sportsdataio-nba-v3-rotoballer-premium-news-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NBA v3 Scores API from SportsDataIO — 28 operation(s) for nba v3 scores.
  name: SportsDataIO NBA v3 Scores API
  slug: sportsdataio-nba-v3-scores-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NBA v3 Stats API from SportsDataIO — 14 operation(s) for nba v3 stats.
  name: SportsDataIO NBA v3 Stats API
  slug: sportsdataio-nba-v3-stats-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NFL v3 Headshots API from SportsDataIO — 1 operation(s) for nfl v3 headshots.
  name: SportsDataIO NFL v3 Headshots API
  slug: sportsdataio-nfl-v3-headshots-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NFL v3 Odds API from SportsDataIO — 34 operation(s) for nfl v3 odds.
  name: SportsDataIO NFL v3 Odds API
  slug: sportsdataio-nfl-v3-odds-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NFL v3 Play-by-Play API from SportsDataIO — 6 operation(s) for nfl v3 play-by-play.
  name: SportsDataIO NFL v3 Play-by-Play API
  slug: sportsdataio-nfl-v3-play-by-play-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NFL v3 Projections API from SportsDataIO — 13 operation(s) for nfl v3 projections.
  name: SportsDataIO NFL v3 Projections API
  slug: sportsdataio-nfl-v3-projections-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NFL v3 RotoBaller Premium News API from SportsDataIO — 3 operation(s) for nfl v3 rotoballer premium news.
  name: SportsDataIO NFL v3 RotoBaller Premium News API
  slug: sportsdataio-nfl-v3-rotoballer-premium-news-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NFL v3 Scores API from SportsDataIO — 45 operation(s) for nfl v3 scores.
  name: SportsDataIO NFL v3 Scores API
  slug: sportsdataio-nfl-v3-scores-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NFL v3 Stats API from SportsDataIO — 34 operation(s) for nfl v3 stats.
  name: SportsDataIO NFL v3 Stats API
  slug: sportsdataio-nfl-v3-stats-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NHL v3 Headshots API from SportsDataIO — 1 operation(s) for nhl v3 headshots.
  name: SportsDataIO NHL v3 Headshots API
  slug: sportsdataio-nhl-v3-headshots-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NHL v3 Odds API from SportsDataIO — 34 operation(s) for nhl v3 odds.
  name: SportsDataIO NHL v3 Odds API
  slug: sportsdataio-nhl-v3-odds-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NHL v3 Play-by-Play API from SportsDataIO — 3 operation(s) for nhl v3 play-by-play.
  name: SportsDataIO NHL v3 Play-by-Play API
  slug: sportsdataio-nhl-v3-play-by-play-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NHL v3 Projections API from SportsDataIO — 4 operation(s) for nhl v3 projections.
  name: SportsDataIO NHL v3 Projections API
  slug: sportsdataio-nhl-v3-projections-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NHL v3 RotoBaller Premium News API from SportsDataIO — 2 operation(s) for nhl v3 rotoballer premium news.
  name: SportsDataIO NHL v3 RotoBaller Premium News API
  slug: sportsdataio-nhl-v3-rotoballer-premium-news-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NHL v3 Scores API from SportsDataIO — 28 operation(s) for nhl v3 scores.
  name: SportsDataIO NHL v3 Scores API
  slug: sportsdataio-nhl-v3-scores-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The NHL v3 Stats API from SportsDataIO — 14 operation(s) for nhl v3 stats.
  name: SportsDataIO NHL v3 Stats API
  slug: sportsdataio-nhl-v3-stats-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The Soccer v4 Headshots API from SportsDataIO — 1 operation(s) for soccer v4 headshots.
  name: SportsDataIO Soccer v4 Headshots API
  slug: sportsdataio-soccer-v4-headshots-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The Soccer v4 Odds API from SportsDataIO — 28 operation(s) for soccer v4 odds.
  name: SportsDataIO Soccer v4 Odds API
  slug: sportsdataio-soccer-v4-odds-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The Soccer v4 Projections API from SportsDataIO — 4 operation(s) for soccer v4 projections.
  name: SportsDataIO Soccer v4 Projections API
  slug: sportsdataio-soccer-v4-projections-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The Soccer v4 Scores API from SportsDataIO — 21 operation(s) for soccer v4 scores.
  name: SportsDataIO Soccer v4 Scores API
  slug: sportsdataio-soccer-v4-scores-api
- baseURL: https://api.sportsdata.io
  baseurl_source: declared
  description: The Soccer v4 Stats API from SportsDataIO — 11 operation(s) for soccer v4 stats.
  name: SportsDataIO Soccer v4 Stats API
  slug: sportsdataio-soccer-v4-stats-api
artifact_total: 187
collections:
- collection_type: open
  name: API Collection
  slug: open-.refine-report
- collection_type: open
  name: MLB MLB v3 Headshots API
  slug: open-sportsdataio-mlb-v3-headshots-api
- collection_type: open
  name: MLB MLB v3 Headshots MLB v3 Odds API
  slug: open-sportsdataio-mlb-v3-odds-api
- collection_type: open
  name: MLB MLB v3 Headshots MLB v3 Play-by-Play API
  slug: open-sportsdataio-mlb-v3-play-by-play-api
- collection_type: open
  name: MLB MLB v3 Headshots MLB v3 Projections API
  slug: open-sportsdataio-mlb-v3-projections-api
- collection_type: open
  name: MLB MLB v3 Headshots MLB v3 RotoBaller Premium News API
  slug: open-sportsdataio-mlb-v3-rotoballer-premium-news-api
- collection_type: open
  name: MLB MLB v3 Headshots MLB v3 Scores API
  slug: open-sportsdataio-mlb-v3-scores-api
- collection_type: open
  name: MLB MLB v3 Headshots MLB v3 Stats API
  slug: open-sportsdataio-mlb-v3-stats-api
- collection_type: open
  name: MLB API
  slug: open-sportsdataio-mlb
- collection_type: open
  name: MLB MLB v3 Headshots NBA v3 Headshots API
  slug: open-sportsdataio-nba-v3-headshots-api
- collection_type: open
  name: MLB MLB v3 Headshots NBA v3 Odds API
  slug: open-sportsdataio-nba-v3-odds-api
- collection_type: open
  name: MLB MLB v3 Headshots NBA v3 Play-by-Play API
  slug: open-sportsdataio-nba-v3-play-by-play-api
- collection_type: open
  name: MLB MLB v3 Headshots NBA v3 Projections API
  slug: open-sportsdataio-nba-v3-projections-api
- collection_type: open
  name: MLB MLB v3 Headshots NBA v3 RotoBaller Premium News API
  slug: open-sportsdataio-nba-v3-rotoballer-premium-news-api
- collection_type: open
  name: MLB MLB v3 Headshots NBA v3 Scores API
  slug: open-sportsdataio-nba-v3-scores-api
- collection_type: open
  name: MLB MLB v3 Headshots NBA v3 Stats API
  slug: open-sportsdataio-nba-v3-stats-api
- collection_type: open
  name: NBA API
  slug: open-sportsdataio-nba
- collection_type: open
  name: MLB MLB v3 Headshots NFL v3 Headshots API
  slug: open-sportsdataio-nfl-v3-headshots-api
- collection_type: open
  name: MLB MLB v3 Headshots NFL v3 Odds API
  slug: open-sportsdataio-nfl-v3-odds-api
- collection_type: open
  name: MLB MLB v3 Headshots NFL v3 Play-by-Play API
  slug: open-sportsdataio-nfl-v3-play-by-play-api
- collection_type: open
  name: MLB MLB v3 Headshots NFL v3 Projections API
  slug: open-sportsdataio-nfl-v3-projections-api
- collection_type: open
  name: MLB MLB v3 Headshots NFL v3 RotoBaller Premium News API
  slug: open-sportsdataio-nfl-v3-rotoballer-premium-news-api
- collection_type: open
  name: MLB MLB v3 Headshots NFL v3 Scores API
  slug: open-sportsdataio-nfl-v3-scores-api
- collection_type: open
  name: MLB MLB v3 Headshots NFL v3 Stats API
  slug: open-sportsdataio-nfl-v3-stats-api
- collection_type: open
  name: NFL API
  slug: open-sportsdataio-nfl
- collection_type: open
  name: MLB MLB v3 Headshots NHL v3 Headshots API
  slug: open-sportsdataio-nhl-v3-headshots-api
- collection_type: open
  name: MLB MLB v3 Headshots NHL v3 Odds API
  slug: open-sportsdataio-nhl-v3-odds-api
- collection_type: open
  name: MLB MLB v3 Headshots NHL v3 Play-by-Play API
  slug: open-sportsdataio-nhl-v3-play-by-play-api
- collection_type: open
  name: MLB MLB v3 Headshots NHL v3 Projections API
  slug: open-sportsdataio-nhl-v3-projections-api
- collection_type: open
  name: MLB MLB v3 Headshots NHL v3 RotoBaller Premium News API
  slug: open-sportsdataio-nhl-v3-rotoballer-premium-news-api
- collection_type: open
  name: MLB MLB v3 Headshots NHL v3 Scores API
  slug: open-sportsdataio-nhl-v3-scores-api
- collection_type: open
  name: MLB MLB v3 Headshots NHL v3 Stats API
  slug: open-sportsdataio-nhl-v3-stats-api
- collection_type: open
  name: NHL API
  slug: open-sportsdataio-nhl
- collection_type: open
  name: MLB MLB v3 Headshots Soccer v4 Headshots API
  slug: open-sportsdataio-soccer-v4-headshots-api
- collection_type: open
  name: MLB MLB v3 Headshots Soccer v4 Odds API
  slug: open-sportsdataio-soccer-v4-odds-api
- collection_type: open
  name: MLB MLB v3 Headshots Soccer v4 Projections API
  slug: open-sportsdataio-soccer-v4-projections-api
- collection_type: open
  name: MLB MLB v3 Headshots Soccer v4 Scores API
  slug: open-sportsdataio-soccer-v4-scores-api
- collection_type: open
  name: MLB MLB v3 Headshots Soccer v4 Stats API
  slug: open-sportsdataio-soccer-v4-stats-api
- collection_type: open
  name: SOCCER API
  slug: open-sportsdataio-soccer
common:
- group: agent
  href: https://raw.githubusercontent.com/api-evangelist/sportsdataio/refs/heads/main/agentic-access/sportsdataio-agentic-access.yml
  title: ''
  type: AgenticAccess
  url: agentic-access/sportsdataio-agentic-access.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/sportsdataio/refs/heads/main/security/sportsdataio-domain-security.yml
  title: ''
  type: DomainSecurity
  url: security/sportsdataio-domain-security.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/sportsdataio/refs/heads/main/authentication/sportsdataio-authentication.yml
  title: ''
  type: Authentication
  url: authentication/sportsdataio-authentication.yml
- group: build
  title: ''
  type: GitHubOrganization
  url: https://github.com/fantasydata
- group: company
  title: ''
  type: LinkedIn
  url: https://www.linkedin.com/company/sportsdatadotio
- group: company
  title: ''
  type: Website
  url: https://sportsdata.io/
- group: start
  title: ''
  type: Portal
  url: https://sportsdata.io/developers
- group: docs
  title: ''
  type: Documentation
  url: https://sportsdata.io/developers
- group: commercial
  title: ''
  type: Pricing
  url: https://sportsdata.io/pricing
- group: start
  title: ''
  type: Login
  url: https://sportsdata.io/login
- group: start
  title: ''
  type: Signup
  url: https://sportsdata.io/signup
- group: operate
  title: ''
  type: Support
  url: https://support.sportsdata.io/
- group: build
  title: ''
  type: SDKs
  url: https://sportsdata.io/developers/integration-tools
- group: docs
  title: ''
  type: OpenAPI
  url: https://sportsdata.io/developers/sports-data-open-api-swagger-files
created: '2025-03-01'
description: SportsDataIO is an award-winning sports data and content provider producing millions of data points across thousands of games annually. The platform offers robust REST APIs, responsive widgets, database downloads, and fantasy sports tools covering 14+ major sports including NFL, MLB, NBA, NHL, Soccer, Golf, NASCAR, MMA/UFC, Tennis, Formula 1, College Football, College Basketball, WNBA, and Olympics. Data includes live scores, statistics, odds feeds, fantasy projections, injuries, lineups, depth charts, standings, and player news.
finops:
- name: Sportsdataio Finops
  service_category: Sports Data
  slug: sportsdataio-finops
image: https://kinlane-images.s3.amazonaws.com/shared/apis-json/icons/sportsdataio.png
json_schemas:
- name: Area
  property_count: 4
  slug: sportsdataio-area
- name: BettingEntityMetadata
  property_count: 3
  slug: sportsdataio-bettingentitymetadata
- name: BettingEntityMetadataCollection
  property_count: 7
  slug: sportsdataio-bettingentitymetadatacollection
- name: BettingEvent
  property_count: 25
  slug: sportsdataio-bettingevent
- name: BettingMarket
  property_count: 21
  slug: sportsdataio-bettingmarket
- name: BettingMarketResult
  property_count: 14
  slug: sportsdataio-bettingmarketresult
- name: BettingMarketSplit
  property_count: 13
  slug: sportsdataio-bettingmarketsplit
- name: BettingOutcome
  property_count: 21
  slug: sportsdataio-bettingoutcome
- name: BettingOutcomeResult
  property_count: 7
  slug: sportsdataio-bettingoutcomeresult
- name: BettingResultingMetadata
  property_count: 3
  slug: sportsdataio-bettingresultingmetadata
- name: BettingSplit
  property_count: 8
  slug: sportsdataio-bettingsplit
- name: Booking
  property_count: 8
  slug: sportsdataio-booking
- name: BoxScore
  property_count: 4
  slug: sportsdataio-boxscore
- name: BoxScoreV3
  property_count: 7
  slug: sportsdataio-boxscorev3
- name: Bye
  property_count: 3
  slug: sportsdataio-bye
- name: CanceledMembership
  property_count: 5
  slug: sportsdataio-canceledmembership
- name: Coach
  property_count: 5
  slug: sportsdataio-coach
- name: Competition
  property_count: 9
  slug: sportsdataio-competition
- name: CompetitionDetail
  property_count: 12
  slug: sportsdataio-competitiondetail
- name: ConsensusOutcome
  property_count: 22
  slug: sportsdataio-consensusoutcome
- name: DepthChart
  property_count: 8
  slug: sportsdataio-depthchart
- name: DepthChartWeekly
  property_count: 8
  slug: sportsdataio-depthchartweekly
- name: DfsSlate
  property_count: 14
  slug: sportsdataio-dfsslate
- name: DfsSlateGame
  property_count: 6
  slug: sportsdataio-dfsslategame
- name: DfsSlatePlayer
  property_count: 14
  slug: sportsdataio-dfsslateplayer
- name: DfsSlatePlayerOwnershipProjection
  property_count: 4
  slug: sportsdataio-dfsslateplayerownershipprojection
- name: DfsSlateWithOwnershipProjection
  property_count: 7
  slug: sportsdataio-dfsslatewithownershipprojection
- name: FantasyDefenseGame
  property_count: 110
  slug: sportsdataio-fantasydefensegame
- name: FantasyDefenseGameProjection
  property_count: 110
  slug: sportsdataio-fantasydefensegameprojection
- name: FantasyDefenseSeason
  property_count: 91
  slug: sportsdataio-fantasydefenseseason
- name: FantasyDefenseSeasonProjection
  property_count: 91
  slug: sportsdataio-fantasydefenseseasonprojection
- name: FantasyGame
  property_count: 29
  slug: sportsdataio-fantasygame
- name: Game
  property_count: 82
  slug: sportsdataio-game
- name: GameBettingSplit
  property_count: 7
  slug: sportsdataio-gamebettingsplit
- name: GameInfo
  property_count: 21
  slug: sportsdataio-gameinfo
- name: GameInfoResult
  property_count: 25
  slug: sportsdataio-gameinforesult
- name: GameOdd
  property_count: 18
  slug: sportsdataio-gameodd
- name: GameOddOutcome
  property_count: 7
  slug: sportsdataio-gameoddoutcome
- name: GameOddResult
  property_count: 18
  slug: sportsdataio-gameoddresult
- name: Goal
  property_count: 12
  slug: sportsdataio-goal
- name: GoalieDepthChart
  property_count: 9
  slug: sportsdataio-goaliedepthchart
- name: Goaltender
  property_count: 7
  slug: sportsdataio-goaltender
- name: Headshot
  property_count: 11
  slug: sportsdataio-headshot
- name: Injury
  property_count: 18
  slug: sportsdataio-injury
- name: Inning
  property_count: 5
  slug: sportsdataio-inning
- name: Lineup
  property_count: 9
  slug: sportsdataio-lineup
- name: MatchupTrends
  property_count: 5
  slug: sportsdataio-matchuptrends
- name: Membership
  property_count: 11
  slug: sportsdataio-membership
- name: News
  property_count: 18
  slug: sportsdataio-news
- name: OpponentSeason
  property_count: 59
  slug: sportsdataio-opponentseason
- name: Penalty
  property_count: 13
  slug: sportsdataio-penalty
- name: PenaltyShootout
  property_count: 8
  slug: sportsdataio-penaltyshootout
- name: Period
  property_count: 7
  slug: sportsdataio-period
- name: Pitch
  property_count: 13
  slug: sportsdataio-pitch
- name: Play
  property_count: 37
  slug: sportsdataio-play
- name: PlayByPlay
  property_count: 2
  slug: sportsdataio-playbyplay
- name: SportsDataIO Player
  property_count: 19
  slug: sportsdataio-player
- name: PlayerBasic
  property_count: 19
  slug: sportsdataio-playerbasic
- name: PlayerDetail
  property_count: 76
  slug: sportsdataio-playerdetail
- name: PlayerGame
  property_count: 139
  slug: sportsdataio-playergame
- name: PlayerGameBasic
  property_count: 64
  slug: sportsdataio-playergamebasic
- name: PlayerGameProjection
  property_count: 139
  slug: sportsdataio-playergameprojection
- name: PlayerGameRedZone
  property_count: 169
  slug: sportsdataio-playergameredzone
- name: PlayerInfo
  property_count: 5
  slug: sportsdataio-playerinfo
- name: PlayerLine
  property_count: 6
  slug: sportsdataio-playerline
- name: PlayerOwnership
  property_count: 12
  slug: sportsdataio-playerownership
- name: PlayerSeason
  property_count: 117
  slug: sportsdataio-playerseason
- name: PlayerSeasonProjection
  property_count: 117
  slug: sportsdataio-playerseasonprojection
- name: PlayerSeasonRedZone
  property_count: 141
  slug: sportsdataio-playerseasonredzone
- name: PlayerSeasonThirdDown
  property_count: 141
  slug: sportsdataio-playerseasonthirddown
- name: PlayoffAggregateScore
  property_count: 7
  slug: sportsdataio-playoffaggregatescore
- name: PlayStat
  property_count: 75
  slug: sportsdataio-playstat
- name: Quarter
  property_count: 6
  slug: sportsdataio-quarter
- name: Referee
  property_count: 6
  slug: sportsdataio-referee
- name: Round
  property_count: 14
  slug: sportsdataio-round
- name: Schedule
  property_count: 31
  slug: sportsdataio-schedule
- name: ScheduleBasic
  property_count: 20
  slug: sportsdataio-schedulebasic
- name: Score
  property_count: 78
  slug: sportsdataio-score
- name: ScoreBasic
  property_count: 33
  slug: sportsdataio-scorebasic
- name: ScoringDetail
  property_count: 12
  slug: sportsdataio-scoringdetail
- name: ScoringPlay
  property_count: 16
  slug: sportsdataio-scoringplay
- name: Season
  property_count: 5
  slug: sportsdataio-season
- name: SeasonTeam
  property_count: 8
  slug: sportsdataio-seasonteam
- name: Series
  property_count: 4
  slug: sportsdataio-series
- name: Sportsbook
  property_count: 2
  slug: sportsdataio-sportsbook
- name: Stadium
  property_count: 22
  slug: sportsdataio-stadium
- name: Standing
  property_count: 36
  slug: sportsdataio-standing
- name: StartingGoaltenders
  property_count: 12
  slug: sportsdataio-startinggoaltenders
- name: StartingLineups
  property_count: 16
  slug: sportsdataio-startinglineups
- name: Team
  property_count: 18
  slug: sportsdataio-team
- name: TeamBasic
  property_count: 23
  slug: sportsdataio-teambasic
- name: TeamDepthChart
  property_count: 2
  slug: sportsdataio-teamdepthchart
- name: TeamDepthChartWeekly
  property_count: 4
  slug: sportsdataio-teamdepthchartweekly
- name: TeamDetail
  property_count: 29
  slug: sportsdataio-teamdetail
- name: TeamGame
  property_count: 119
  slug: sportsdataio-teamgame
- name: TeamGameTrends
  property_count: 16
  slug: sportsdataio-teamgametrends
- name: TeamGoalieDepthChart
  property_count: 2
  slug: sportsdataio-teamgoaliedepthchart
- name: TeamLine
  property_count: 6
  slug: sportsdataio-teamline
- name: TeamSeason
  property_count: 110
  slug: sportsdataio-teamseason
- name: TeamTrends
  property_count: 4
  slug: sportsdataio-teamtrends
- name: Timeframe
  property_count: 18
  slug: sportsdataio-timeframe
- name: Transaction
  property_count: 11
  slug: sportsdataio-transaction
- name: Unauthorized
  property_count: 4
  slug: sportsdataio-unauthorized
- name: Venue
  property_count: 14
  slug: sportsdataio-venue
json_structures:
- name: Sportsdataio Nfl Structure
  property_count: 0
  slug: sportsdataio-nfl-structure
- name: Sportsdataio Structure
  property_count: 0
  slug: sportsdataio-structure
jsonld:
- class_count: 39
  name: Sportsdataio Context
  property_count: 4
  slug: sportsdataio-context
layout: provider
modified: '2026-05-19'
name: SportsDataIO
nav: Providers
network: true
overview: 'SportsDataIO publishes 33 APIs on the [APIs.io](https://apis.io/) network, including MLB v3 Headshots API, MLB v3 Odds API, MLB v3 Play-by-Play API, and 30 more. Tagged areas include Sports Data, Statistics, Live Scores, Fantasy Sports, and Odds.


  The SportsDataIO catalog on APIs.io includes 1 JSON-LD context and 2 Spectral governance rulesets.


  SportsDataIO''s developer surface includes authentication, developer portal, documentation, pricing, signup flow, support, and 8 more developer resources.'
plans:
- name: Sportsdataio Plans Pricing
  plan_count: 4
  slug: sportsdataio-plans-pricing
random_paper: 11
rate_limits:
- limit_count: 5
  name: Sportsdataio Rate Limits
  slug: sportsdataio-rate-limits
rules:
- effective_rule_count: 5
  extends: []
  name: SportsDataIO API Rules
  rule_count: 5
  severity_counts:
    error: 0
    hint: 0
    info: 1
    warn: 4
  slug: sportsdataio-jsonschema-spectral-rules
- effective_rule_count: 50
  extends:
  - spectral:oas
  name: SportsDataIO API Rules
  rule_count: 9
  severity_counts:
    error: 6
    hint: 0
    info: 0
    warn: 3
  slug: sportsdataio-rules
score:
  band: thin
  composite: 38.2
  coverage:
    artifact_dirs: 16
    catalog_earned: 61.5
    catalog_earned_first_party: 0.0
    catalog_gap: 53.5
    catalog_max: 115.0
    note: 'Disclosure, not a penalty. catalog_gap is rubric points API Evangelist could add with no action by this provider, and it is NOT subtracted from the composite above. It is our backlog EXCEPT where this provider already did the work: catalog_earned is how much of the class was satisfied at all, and catalog_earned_first_party how much of that came from artifacts the provider published rather than ones we generated (roadmap#221). catalog_earned_first_party is a FLOOR, not the whole share: only ~40 of the rubric''s 113 checks carry a provenance class at all, so a check we cannot attribute counts toward neither side. Read it as "at least this much was theirs", never as "the rest was ours".'
  delta: 0.0
  facets:
    access_clarity: 22.4
    contract_governance: 13.6
    contract_quality: 59.1
    developer_ergonomics: 42.9
    discoverability: 74.1
    operational_transparency: 10.5
  previous_composite: 38.2
  provenance:
    agentic_access: derived
    contracts:
      callable: 100.0
      derived: 0
      marker_coverage: 0.0
      total: 33
  schema_version: 0.22.0
  scored_at: '2026-09-17'
  trend: flat
  upsert:
    applies: false
    note: 'Not scored: this provider''s published contracts declare no write operations, and a read-only API cannot create-or-update. Excluded from the denominator, not zeroed.'
    reason: read_only
screenshot: https://raw.githubusercontent.com/api-evangelist/sportsdataio/refs/heads/main/screenshots/sportsdataio-2026-06-20T194350.png
security:
- kind: authentication
  name: Sportsdataio Authentication
  slug: sportsdataio-authentication
  summary_line: apiKey · 2 schemes
- kind: domain-security
  name: Sportsdataio Domain Security
  slug: sportsdataio-domain-security
  summary_line: TLSv1.3 · HSTS · DMARC
slug: sportsdataio
tags:
- Sports Data
- Statistics
- Live Scores
- Fantasy Sports
- Odds
- NFL
- NBA
- MLB
- NHL
- Soccer
website: https://sportsdata.io/
---
