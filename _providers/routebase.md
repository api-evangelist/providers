...
api_count: 1
apis:
- baseURL: https://api.routebase.dev
  baseurl_source: declared
  description: Read specifications and their versions, export them as OpenAPI, and record which version a deployment put into an environment.
  name: Routebase API Specs API
  slug: routebase-api-specs-api
- baseURL: https://api.routebase.dev
  baseurl_source: declared
  description: Run test suites from a pipeline and collect the result. These endpoints back the `routebase` CLI and the Routebase GitHub Action, and they are the ones a CI job calls directly.
  name: Routebase CI & Test Runs API
  slug: routebase-ci-test-runs-api
- baseURL: https://api.routebase.dev
  baseurl_source: declared
  description: Keep documentation pages in your own repository and push them into Routebase from a pipeline. These endpoints back `routebase docs push` and `routebase docs pull`.
  name: Routebase Docs as Code API
  slug: routebase-docs-as-code-api
- baseURL: https://api.routebase.dev
  baseurl_source: declared
  description: SCIM 2.0 user and group provisioning, as consumed by Okta, Microsoft Entra ID and other identity providers. Available on plans that include SSO.
  name: Routebase SCIM API
  slug: routebase-scim-api
- baseURL: https://api.routebase.dev
  baseurl_source: declared
  description: Start an OWASP scan, poll it to completion, and read the findings. The SARIF export feeds GitHub code scanning and comparable tools.
  name: Routebase Security API
...
nav: Providers
network: true
overview: 'Routebase publishes 5 APIs on the [APIs.io](https://apis.io/) network, including API Specs API, CI & Test Runs API, Docs as Code API, and 2 more. Tagged areas include API lifecycle management, API Design, OpenAPI, API Documentation, and API Testing.


  The Routebase catalog on APIs.io includes 2 event-driven AsyncAPI specifications.


  Routebase''s developer surface includes getting-started guide, support, engineering blog, pricing, signup flow, CLI, changelog, and 27 more developer resources.'
plans:
