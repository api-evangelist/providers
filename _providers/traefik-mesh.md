---
access_model:
  confidence: low
  label: Unknown
  onboarding: unknown
  pricing: unknown
  public: false
  source: []
  trial: false
  try_now: false
agent_readiness:
  band: agent-aware
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: derived
    agentic_commerce: false
    auth_clarity: false
    consent_identity: false
    delegated_identity: false
    dry_run_mode: na
    dynamic_client_registration: false
    error_semantics: verified
    event_surface_described: false
    idempotency: na
    mcp_server: false
    openapi_examples: partial
    protected_resource_metadata: false
    rate_limit_signal: false
    reversibility_documented: na
    spec_presence: true
    well_known_catalog: false
  schema_version: '0.2'
  score: 26.7
  scored_at: '2026-09-14'
agentic_access:
- acting_count: 0
  human_in_the_loop: 0
  name: Traefik Mesh Agentic Access
  operation_count: 5
  slug: traefik-mesh-agentic-access
  summary_line: 5 operations
api_count: 1
apis:
- baseURL: http://controller-pod-ip:9000
  baseurl_source: declared
  description: Endpoints for retrieving the current dynamic Traefik configuration generated by the Traefik Mesh controller.
  name: Traefik Mesh Configuration API
  slug: traefik-mesh-configuration-api
- baseURL: http://controller-pod-ip:9000
  baseurl_source: declared
  description: Endpoints for checking the readiness and status of the Traefik Mesh controller and proxy nodes.
  name: Traefik Mesh Status API
  slug: traefik-mesh-status-api
- baseURL: http://controller-pod-ip:9000
  baseurl_source: declared
  description: Endpoints for inspecting the mesh topology as seen by the controller.
  name: Traefik Mesh Topology API
  slug: traefik-mesh-topology-api
artifact_total: 32
collections:
- collection_type: open
  name: API Collection
  slug: open-.refine-report
- collection_type: open
  name: Traefik Mesh Controller Configuration API
  slug: open-traefik-mesh-configuration-api
- collection_type: open
  name: Traefik Mesh Controller API
  slug: open-traefik-mesh-controller
- collection_type: open
  name: Traefik Mesh Controller Configuration Status API
  slug: open-traefik-mesh-status-api
- collection_type: open
  name: Traefik Mesh Controller Configuration Topology API
  slug: open-traefik-mesh-topology-api
common:
- group: operate
  title: ''
  type: IssueTracker
  url: https://github.com/traefik/mesh/issues
- group: build
  title: ''
  type: CodeOfConduct
  url: https://github.com/traefik/mesh/blob/master/CODE_OF_CONDUCT.md
- group: docs
  title: ''
  type: ContributionGuide
  url: https://github.com/traefik/mesh/blob/master/CONTRIBUTING.md
- group: agent
  href: https://raw.githubusercontent.com/api-evangelist/traefik-mesh/refs/heads/main/agentic-access/traefik-mesh-agentic-access.yml
  title: ''
  type: AgenticAccess
  url: agentic-access/traefik-mesh-agentic-access.yml
- group: auth
  href: https://raw.githubusercontent.com/api-evangelist/traefik-mesh/refs/heads/main/security/traefik-mesh-domain-security.yml
  title: ''
  type: DomainSecurity
  url: security/traefik-mesh-domain-security.yml
- group: company
  title: ''
  type: Website
  url: https://traefik.io/traefik-mesh
- group: docs
  title: ''
  type: Documentation
  url: https://doc.traefik.io/traefik-mesh/
- group: build
  title: ''
  type: GitHubOrganization
  url: https://github.com/traefik
- group: build
  title: ''
  type: GitHubRepository
  url: https://github.com/traefik/mesh
- group: other
  title: ''
  type: HelmChart
  url: https://github.com/traefik/mesh-helm-chart
- group: company
  title: ''
  type: Blog
  url: https://traefik.io/blog/
- group: operate
  title: ''
  type: Community
  url: https://community.traefik.io/
- group: operate
  title: ''
  type: Slack
  url: https://slack.traefik.io/
- group: commercial
  title: ''
  type: TermsOfService
  url: https://traefik.io/terms/
- group: commercial
  title: ''
  type: PrivacyPolicy
  url: https://traefik.io/privacy/
- group: commercial
  title: ''
  type: License
  url: https://github.com/traefik/mesh/blob/master/LICENSE
- group: design
  href: https://raw.githubusercontent.com/api-evangelist/traefik-mesh/refs/heads/main/rules/traefik-mesh-rules.yml
  title: ''
  type: Spectral
  url: rules/traefik-mesh-rules.yml
- group: design
  href: https://raw.githubusercontent.com/api-evangelist/traefik-mesh/refs/heads/main/json-ld/traefik-mesh-context.jsonld
  title: ''
  type: JSONLD
  url: json-ld/traefik-mesh-context.jsonld
- group: design
  href: https://raw.githubusercontent.com/api-evangelist/traefik-mesh/refs/heads/main/vocabulary/traefik-mesh-vocabulary.yml
  title: ''
  type: Vocabulary
  url: vocabulary/traefik-mesh-vocabulary.yml
- group: operate
  title: ''
  type: ReleaseNotes
  url: https://github.com/traefik/mesh/releases
- group: operate
  title: ''
  type: ChangeLog
  url: https://github.com/traefik/mesh/releases
- group: operate
  title: ''
  type: MaintenanceStatus
  url: ''
- group: agent
  title: ''
  type: LlmsText
  url: https://traefik.io/llms.txt
created: '2026-05-03'
description: 'Traefik Mesh (formerly Maesh) is a lightweight, non-invasive service mesh built on top of Traefik Proxy for Kubernetes. It provides automatic traffic management, observability, and security for microservices without requiring sidecar containers. Traefik Mesh is compliant with the Service Mesh Interface (SMI) specification and supports HTTP, TCP, and UDP traffic modes, traffic splitting, access control via TrafficTarget, rate limiting, circuit breaking, and retries. The project is effectively dormant: the last released version is v1.4.8 (2022-08-19), and commit activity since then has been limited to CI and documentation maintenance. The GitHub repository is not formally archived as of this profile date and there is no official deprecation banner on the project''s README or documentation site, but the lack of feature releases for nearly four years and the absence of an SMI-compatible successor inside Traefik Labs'' product line (Traefik Hub is positioned as Kubernetes-native
  API management, not a service mesh) make this an unmaintained project for practical purposes.'
examples:
- key_count: 3
  name: Traefik Mesh Get Configuration Example
  slug: traefik-mesh-get-configuration-example
features:
- description: No sidecar containers required; uses per-node Traefik Proxy instances.
  name: Non-Invasive Service Mesh
- description: Full support for Service Mesh Interface (SMI) specification including TrafficSplit, TrafficTarget, HTTPRouteGroup, and TCPRoute.
  name: SMI Compliance
- description: Retry, circuit breaking, rate limiting, and traffic splitting for HTTP services.
  name: HTTP Traffic Management
- description: Transparent proxy support for TCP and UDP traffic in addition to HTTP.
  name: TCP and UDP Support
- description: All traffic forbidden by default unless explicitly allowed via SMI TrafficTarget resources.
  name: Access Control List (ACL) Mode
- description: Route traffic subsets to different service versions using SMI TrafficSplit.
  name: Traffic Splitting
- description: Configure mesh behavior per service via Kubernetes annotations.
  name: Kubernetes Annotations
- description: Easy deployment and configuration via official Helm chart.
  name: Helm Installation
image: https://kinlane-images.s3.amazonaws.com/shared/apis-json/icons/traefik-mesh.png
integrations:
- description: Uses Traefik Proxy as the data plane proxy on each Kubernetes node.
  name: Traefik Proxy
- description: Native Kubernetes integration via Custom Resource Definitions and annotations.
  name: Kubernetes
- description: Implements Service Mesh Interface specification for interoperability.
  name: SMI Spec
json_schemas:
- name: Traefik Mesh Pod Info
  property_count: 3
  slug: traefik-mesh-pod-info
- name: Traefik Mesh Service Entry
  property_count: 5
  slug: traefik-mesh-service-entry
json_structures:
- name: Traefik Mesh Pod Info Structure
  property_count: 0
  slug: traefik-mesh-pod-info-structure
jsonld:
- class_count: 22
  name: Traefik Mesh Context
  property_count: 0
  slug: traefik-mesh-context
layout: provider
modified: '2026-05-23'
name: Traefik Mesh
nav: Providers
network: true
overview: 'Traefik Mesh publishes 3 APIs on the [APIs.io](https://apis.io/) network: Configuration API, Status API, and Topology API. Tagged areas include Kubernetes, Service Mesh, Open-Source, SMI, and Traffic Management.


  The Traefik Mesh catalog on APIs.io includes 1 JSON-LD context and 2 Spectral governance rulesets.


  Traefik Mesh''s developer surface includes documentation, engineering blog, release notes, changelog, and 18 more developer resources.'
random_paper: 19
rules:
- effective_rule_count: 5
  extends: []
  name: Traefik Mesh API Rules
  rule_count: 5
  severity_counts:
    error: 0
    hint: 0
    info: 2
    warn: 3
  slug: traefik-mesh-jsonschema-spectral-rules
- effective_rule_count: 15
  extends: []
  name: Traefik Mesh API Rules
  rule_count: 15
  severity_counts:
    error: 8
    hint: 0
    info: 0
    warn: 7
  slug: traefik-mesh-rules
score:
  band: thin
  composite: 31.1
  coverage:
    artifact_dirs: 13
    catalog_earned: 63.3
    catalog_earned_first_party: 0.0
    catalog_gap: 36.8
    catalog_max: 100.0
    note: 'Disclosure, not a penalty. catalog_gap is rubric points API Evangelist could add with no action by this provider, and it is NOT subtracted from the composite above. It is our backlog EXCEPT where this provider already did the work: catalog_earned is how much of the class was satisfied at all, and catalog_earned_first_party how much of that came from artifacts the provider published rather than ones we generated (roadmap#221). catalog_earned_first_party is a FLOOR, not the whole share: only ~40 of the rubric''s 113 checks carry a provenance class at all, so a check we cannot attribute counts toward neither side. Read it as "at least this much was theirs", never as "the rest was ours".'
  delta: 0.0
  facets:
    access_clarity: 0.0
    contract_governance: 25.0
    contract_quality: 59.1
    developer_ergonomics: 16.7
    discoverability: 75.9
    operational_transparency: 18.4
  previous_composite: 31.1
  provenance:
    agentic_access: derived
    contracts:
      callable: 0.0
      derived: 0
      marker_coverage: 0.0
      total: 3
  schema_version: 0.22.0
  scored_at: '2026-09-14'
  trend: flat
  upsert:
    applies: false
    note: 'Not scored: this provider''s published contracts declare no write operations, and a read-only API cannot create-or-update. Excluded from the denominator, not zeroed.'
    reason: read_only
screenshot: https://raw.githubusercontent.com/api-evangelist/traefik-mesh/refs/heads/main/screenshots/traefik-mesh-2026-06-20T195533.png
security:
- kind: domain-security
  name: Traefik Mesh Domain Security
  slug: traefik-mesh-domain-security
  summary_line: TLSv1.3 · HSTS · DMARC
slug: traefik-mesh
tags:
- Kubernetes
- Service Mesh
- Open-Source
- SMI
- Traffic Management
- Dormant
- Unmaintained
use_cases:
- description: Gradually shift traffic to new service versions using SMI TrafficSplit.
  name: Canary Deployments
- description: Enforce zero-trust networking with SMI TrafficTarget ACL mode.
  name: Service-to-Service Access Control
- description: Inspect live mesh configuration, topology, and node readiness via controller API.
  name: Observability
- description: Apply retries, circuit breakers, and rate limits without changing application code.
  name: Microservices Traffic Management
website: https://traefik.io/traefik-mesh
---
