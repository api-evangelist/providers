---
access_model:
  confidence: high
  label: Paid · Self-serve signup
  onboarding: self-serve
  pricing: paid
  public: false
  source:
  - plans
  - authentication
  trial: false
  try_now: false
agent_readiness:
  band: agent-aware
  dimensions:
    agent_card: false
    agent_skills: false
    agentic_access: derived
    agentic_commerce: false
    auth_clarity: bearer
    consent_identity: false
    delegated_identity: false
    dry_run_mode: false
    dynamic_client_registration: false
    error_semantics: false
    event_surface_described: false
    idempotency: false
    mcp_server: false
    openapi_examples: partial
    protected_resource_metadata: false
    rate_limit_signal: documented
    reversibility_documented: documented
    spec_presence: true
    well_known_catalog: false
  schema_version: 0.2
  score: 24.0
  scored_at: '2026-08-30'
agentic_access:
- acting_count: 23
  human_in_the_loop: 0
  name: Airbus Oneatlas Agentic Access
  operation_count: 36
  slug: airbus-oneatlas-agentic-access
  summary_line: 36 operations · 23 acting
api_count: 7
apis:
- description: OneAtlas Data Living Library is a platform that provides users with access to a diverse range of geospatial data and satellite imagery. The library is constantly updated with the latest data, ensuring
  name: OneAtlas Data Living Library
  slug: oneatlas-data-living-library
- description: 'The OneAtlas Data Pay-Per-Order API is a powerful tool that enables users to access and purchase high-resolution satellite imagery, elevation data, and other geospatial information on a pay-as-you-go '
  name: OneAtlas Data Pay-Per-Order API
  slug: oneatlas-data-pay-per-order-api
- description: 'The OneAtlas Data Pay-Per-Order Tasking API allows users to directly request high-resolution satellite imagery of specific locations at their convenience. With this API, users can easily select areas '
  name: OneAtlas Data Pay-Per-Order Tasking API
  slug: oneatlas-data-pay-per-order-tasking-api
- description: OneAtlas Basemap API is a geospatial service that provides access to high-resolution satellite imagery and basemaps. It allows users to retrieve up-to-date visual information of any location on Earth,
  name: OneAtlas Basemap API
  slug: oneatlas-basemap-api
- description: OneAtlas Radar API is a powerful tool that allows developers to access and integrate radar satellite data into their applications and processes. By utilizing this API, users can access real-time and h
  name: OneAtlas Radar API
  slug: oneatlas-radar-api
- description: The Access API from Airbus OneAtlas — 13 operation(s) for access.
  name: Airbus OneAtlas Access API
  slug: airbus-oneatlas-access-api
- description: Allows users to query their current balance.
  name: Airbus OneAtlas accounting/balance API
  slug: airbus-oneatlas-accounting-balance-api
- description: Allows users to query their transactions.
  name: Airbus OneAtlas accounting/transactions API
  slug: airbus-oneatlas-accounting-transactions-api
- description: The APIkey management API from Airbus OneAtlas — 2 operation(s) for apikey management.
  name: Airbus OneAtlas APIkey management API
  slug: airbus-oneatlas-apikey-management-api
- description: The Attempts API from Airbus OneAtlas — 1 operation(s) for attempts.
  name: Airbus OneAtlas Attempts API
  slug: airbus-oneatlas-attempts-api
- description: The Catalog search API from Airbus OneAtlas — 4 operation(s) for catalog search.
  name: Airbus OneAtlas Catalog search API
  slug: airbus-oneatlas-catalog-search-api
- description: The Contract related Information API from Airbus OneAtlas — 6 operation(s) for contract related information.
  name: Airbus OneAtlas Contract related Information API
  slug: airbus-oneatlas-contract-related-information-api
- description: The Contracts API from Airbus OneAtlas — 2 operation(s) for contracts.
  name: Airbus OneAtlas Contracts API
  slug: airbus-oneatlas-contracts-api
- description: The Control API from Airbus OneAtlas — 1 operation(s) for control.
  name: Airbus OneAtlas Control API
  slug: airbus-oneatlas-control-api
- description: The Convert API from Airbus OneAtlas — 1 operation(s) for convert.
  name: Airbus OneAtlas Convert API
  slug: airbus-oneatlas-convert-api
- description: The Deliver API from Airbus OneAtlas — 4 operation(s) for deliver.
  name: Airbus OneAtlas Deliver API
  slug: airbus-oneatlas-deliver-api
- description: 'All operations needed to order DEM data: * place orders * show placed orders * download purchased data'
  name: Airbus OneAtlas elevation/orders API
  slug: airbus-oneatlas-elevation-orders-api
- description: Endpoint to query to price of an elevation order.
  name: Airbus OneAtlas elevation/price API
  slug: airbus-oneatlas-elevation-price-api
- description: The Feasibility API from Airbus OneAtlas — 1 operation(s) for feasibility.
  name: Airbus OneAtlas Feasibility API
  slug: airbus-oneatlas-feasibility-api
- description: The Format API from Airbus OneAtlas — 1 operation(s) for format.
  name: Airbus OneAtlas Format API
  slug: airbus-oneatlas-format-api
- description: The freemium API from Airbus OneAtlas — 1 operation(s) for freemium.
  name: Airbus OneAtlas freemium API
  slug: airbus-oneatlas-freemium-api
- description: The Get Capabilities API from Airbus OneAtlas — 1 operation(s) for get capabilities.
  name: Airbus OneAtlas Get Capabilities API
  slug: airbus-oneatlas-get-capabilities-api
- description: The Health API from Airbus OneAtlas — 1 operation(s) for health.
  name: Airbus OneAtlas Health API
  slug: airbus-oneatlas-health-api
- description: The Maps API from Airbus OneAtlas — 1 operation(s) for maps.
  name: Airbus OneAtlas Maps API
  slug: airbus-oneatlas-maps-api
- description: The Monitor API from Airbus OneAtlas — 5 operation(s) for monitor.
  name: Airbus OneAtlas Monitor API
  slug: airbus-oneatlas-monitor-api
- description: The Notifications API from Airbus OneAtlas — 1 operation(s) for notifications.
  name: Airbus OneAtlas Notifications API
  slug: airbus-oneatlas-notifications-api
- description: The Notify API from Airbus OneAtlas — 2 operation(s) for notify.
  name: Airbus OneAtlas Notify API
  slug: airbus-oneatlas-notify-api
- description: The Opensearch API from Airbus OneAtlas — 1 operation(s) for opensearch.
  name: Airbus OneAtlas Opensearch API
  slug: airbus-oneatlas-opensearch-api
- description: The Ordering API from Airbus OneAtlas — 5 operation(s) for ordering.
  name: Airbus OneAtlas Ordering API
  slug: airbus-oneatlas-ordering-api
- description: The Orders API from Airbus OneAtlas — 4 operation(s) for orders.
  name: Airbus OneAtlas Orders API
  slug: airbus-oneatlas-orders-api
- description: The payments API from Airbus OneAtlas — 1 operation(s) for payments.
  name: Airbus OneAtlas payments API
  slug: airbus-oneatlas-payments-api
- description: The Pilot API from Airbus OneAtlas — 1 operation(s) for pilot.
  name: Airbus OneAtlas Pilot API
  slug: airbus-oneatlas-pilot-api
- description: The PleiadesNeo specific API from Airbus OneAtlas — 9 operation(s) for pleiadesneo specific.
  name: Airbus OneAtlas PleiadesNeo specific API
  slug: airbus-oneatlas-pleiadesneo-specific-api
- description: The Process API from Airbus OneAtlas — 3 operation(s) for process.
  name: Airbus OneAtlas Process API
  slug: airbus-oneatlas-process-api
- description: Handle your baskets to order SAR acquisitions.
  name: Airbus OneAtlas Sar/Baskets API
  slug: airbus-oneatlas-sar-baskets-api
- description: Endpoint for SAR specific user configuration.
  name: Airbus OneAtlas Sar/Config API
  slug: airbus-oneatlas-sar-config-api
- description: Endpoint to modify an SAR acquisition.
  name: Airbus OneAtlas Sar/Editing API
  slug: airbus-oneatlas-sar-editing-api
- description: Monitor your ordered SAR acquisitions.
  name: Airbus OneAtlas Sar/Monitoring API
  slug: airbus-oneatlas-sar-monitoring-api
- description: Query the price of an SAR acquisition.
  name: Airbus OneAtlas Sar/Prices API
  slug: airbus-oneatlas-sar-prices-api
- description: The Sar/Replication API from Airbus OneAtlas — 3 operation(s) for sar/replication.
  name: Airbus OneAtlas Sar/Replication API
  slug: airbus-oneatlas-sar-replication-api
- description: Endpoint to search for possible SAR acquisitions.
  name: Airbus OneAtlas Sar/Search API
  slug: airbus-oneatlas-sar-search-api
- description: <b>Deprecated!</b> Please use baskets instead.
  name: Airbus OneAtlas Sar/Shopcart API
  slug: airbus-oneatlas-sar-shopcart-api
- description: The SPOT and Pleiades ordering API from Airbus OneAtlas — 1 operation(s) for spot and pleiades ordering.
  name: Airbus OneAtlas SPOT and Pleiades ordering API
  slug: airbus-oneatlas-spot-and-pleiades-ordering-api
- description: The SPOT and Pleiades specific API from Airbus OneAtlas — 6 operation(s) for spot and pleiades specific.
  name: Airbus OneAtlas SPOT and Pleiades specific API
  slug: airbus-oneatlas-spot-and-pleiades-specific-api
- description: The Subscriptions API from Airbus OneAtlas — 12 operation(s) for subscriptions.
  name: Airbus OneAtlas Subscriptions API
  slug: airbus-oneatlas-subscriptions-api
- description: The Tasking API from Airbus OneAtlas — 3 operation(s) for tasking.
  name: Airbus OneAtlas Tasking API
  slug: airbus-oneatlas-tasking-api
- description: The Token retrieval API from Airbus OneAtlas — 1 operation(s) for token retrieval.
  name: Airbus OneAtlas Token retrieval API
  slug: airbus-oneatlas-token-retrieval-api
- description: Endpoint for generic user information.
  name: Airbus OneAtlas user API
  slug: airbus-oneatlas-user-api
- description: The Workspaces API from Airbus OneAtlas — 10 operation(s) for workspaces.
  name: Airbus OneAtlas Workspaces API
  slug: airbus-oneatlas-workspaces-api
artifact_total: 355
collections:
- collection_type: open
  name: API Collection
  slug: open-.refine-report
- collection_type: open
  name: OneAtlas WorldDEM Access API
  slug: open-airbus-oneatlas-access-api
- collection_type: open
  name: OneAtlas WorldDEM Access accounting/balance API
  slug: open-airbus-oneatlas-accounting-balance-api
- collection_type: open
  name: OneAtlas WorldDEM Access accounting/transactions API
  slug: open-airbus-oneatlas-accounting-transactions-api
- collection_type: open
  name: OneAtlas WorldDEM Access APIkey management API
  slug: open-airbus-oneatlas-apikey-management-api
- collection_type: open
  name: OneAtlas WorldDEM Access Attempts API
  slug: open-airbus-oneatlas-attempts-api
- collection_type: open
  name: OneAtlas WorldDEM Access Catalog search API
  slug: open-airbus-oneatlas-catalog-search-api
- collection_type: open
  name: OneAtlas WorldDEM Access Contract related Information API
  slug: open-airbus-oneatlas-contract-related-information-api
- collection_type: open
  name: OneAtlas WorldDEM Access Contracts API
  slug: open-airbus-oneatlas-contracts-api
- collection_type: open
  name: OneAtlas WorldDEM Access Control API
  slug: open-airbus-oneatlas-control-api
- collection_type: open
  name: OneAtlas WorldDEM Access Convert API
  slug: open-airbus-oneatlas-convert-api
- collection_type: open
  name: OneAtlas WorldDEM Access Deliver API
  slug: open-airbus-oneatlas-deliver-api
- collection_type: open
  name: OneAtlas WorldDEM Access elevation/orders API
  slug: open-airbus-oneatlas-elevation-orders-api
- collection_type: open
  name: OneAtlas WorldDEM Access elevation/price API
  slug: open-airbus-oneatlas-elevation-price-api
- collection_type: open
  name: OneAtlas WorldDEM Access Feasibility API
  slug: open-airbus-oneatlas-feasibility-api
- collection_type: open
  name: OneAtlas WorldDEM Access Format API
  slug: open-airbus-oneatlas-format-api
- collection_type: open
  name: OneAtlas WorldDEM Access freemium API
  slug: open-airbus-oneatlas-freemium-api
- collection_type: open
  name: OneAtlas WorldDEM Access Get Capabilities API
  slug: open-airbus-oneatlas-get-capabilities-api
- collection_type: open
  name: OneAtlas WorldDEM Access Health API
  slug: open-airbus-oneatlas-health-api
- collection_type: open
  name: OneAtlas WorldDEM Access Maps API
  slug: open-airbus-oneatlas-maps-api
- collection_type: open
  name: OneAtlas WorldDEM Access Monitor API
  slug: open-airbus-oneatlas-monitor-api
- collection_type: open
  name: OneAtlas WorldDEM Access Notifications API
  slug: open-airbus-oneatlas-notifications-api
- collection_type: open
  name: OneAtlas WorldDEM Access Notify API
  slug: open-airbus-oneatlas-notify-api
- collection_type: open
  name: OneAtlas WorldDEM Access Opensearch API
  slug: open-airbus-oneatlas-opensearch-api
- collection_type: open
  name: OneAtlas WorldDEM Access Ordering API
  slug: open-airbus-oneatlas-ordering-api
- collection_type: open
  name: OneAtlas WorldDEM Access Orders API
  slug: open-airbus-oneatlas-orders-api
- collection_type: open
  name: OneAtlas WorldDEM Access payments API
  slug: open-airbus-oneatlas-payments-api
- collection_type: open
  name: OneAtlas WorldDEM Access Pilot API
  slug: open-airbus-oneatlas-pilot-api
- collection_type: open
  name: OneAtlas WorldDEM Access PleiadesNeo specific API
  slug: open-airbus-oneatlas-pleiadesneo-specific-api
- collection_type: open
  name: OneAtlas WorldDEM Access Process API
  slug: open-airbus-oneatlas-process-api
- collection_type: open
  name: OneAtlas WorldDEM Access Sar/Baskets API
  slug: open-airbus-oneatlas-sar-baskets-api
- collection_type: open
  name: OneAtlas WorldDEM Access Sar/Config API
  slug: open-airbus-oneatlas-sar-config-api
- collection_type: open
  name: OneAtlas WorldDEM Access Sar/Editing API
  slug: open-airbus-oneatlas-sar-editing-api
- collection_type: open
  name: OneAtlas WorldDEM Access Sar/Monitoring API
  slug: open-airbus-oneatlas-sar-monitoring-api
- collection_type: open
  name: OneAtlas WorldDEM Access Sar/Prices API
  slug: open-airbus-oneatlas-sar-prices-api
- collection_type: open
  name: OneAtlas WorldDEM Access Sar/Replication API
  slug: open-airbus-oneatlas-sar-replication-api
- collection_type: open
  name: OneAtlas WorldDEM Access Sar/Search API
  slug: open-airbus-oneatlas-sar-search-api
- collection_type: open
  name: OneAtlas WorldDEM Access Sar/Shopcart API
  slug: open-airbus-oneatlas-sar-shopcart-api
- collection_type: open
  name: OneAtlas WorldDEM Access SPOT and Pleiades ordering API
  slug: open-airbus-oneatlas-spot-and-pleiades-ordering-api
- collection_type: open
  name: OneAtlas WorldDEM Access SPOT and Pleiades specific API
  slug: open-airbus-oneatlas-spot-and-pleiades-specific-api
- collection_type: open
  name: OneAtlas WorldDEM Access Subscriptions API
  slug: open-airbus-oneatlas-subscriptions-api
- collection_type: open
  name: OneAtlas WorldDEM Access Tasking API
  slug: open-airbus-oneatlas-tasking-api
- collection_type: open
  name: OneAtlas WorldDEM Access Token retrieval API
  slug: open-airbus-oneatlas-token-retrieval-api
- collection_type: open
  name: OneAtlas WorldDEM Access user API
  slug: open-airbus-oneatlas-user-api
- collection_type: open
  name: OneAtlas WorldDEM Access Workspaces API
  slug: open-airbus-oneatlas-workspaces-api
- collection_type: open
  name: OneAtlas WorldDEM API
  slug: open-airbus-oneatlas-worlddem-api
- collection_type: open
  name: OneAtlas Basemap API
  slug: open-oneatlas-basemap-api
- collection_type: open
  name: OneAtlas Data Living Library
  slug: open-oneatlas-data-living-library
- collection_type: open
  name: OneAtlas Data Pay-Per-Order API
  slug: open-oneatlas-data-pay-per-order-api
- collection_type: open
  name: OneAtlas Data Pay-Per-Order Tasking API
  slug: open-oneatlas-data-pay-per-order-tasking-api
- collection_type: open
  name: Airbus OneAtlas OneAtlas Radar API
  slug: open-oneatlas-radar-api
- collection_type: open
  name: OneAtlas WorldDEM API
  slug: open-oneatlas-worlddem-api
common:
- group: other
  title: ''
  type: CapabilityMap
  url: capabilities/airbus-oneatlas-capability-edges.yml
- group: agent
  title: ''
  type: AgenticAccess
  url: agentic-access/airbus-oneatlas-agentic-access.yml
- group: auth
  title: ''
  type: VulnerabilityDisclosure
  url: security/airbus-oneatlas-vulnerability-disclosure.yml
- group: auth
  title: ''
  type: DomainSecurity
  url: security/airbus-oneatlas-domain-security.yml
- group: auth
  title: ''
  type: Authentication
  url: authentication/airbus-oneatlas-authentication.yml
- group: start
  title: ''
  type: GettingStarted
  url: https://api.oneatlas.airbus.com/getting-started/
- group: auth
  title: ''
  type: Authentication
  url: https://api.oneatlas.airbus.com/getting-started/authentication/#authenticate
- group: design
  title: ''
  type: Testing
  url: https://api.oneatlas.airbus.com/getting-started/api-testing-guide/
- group: operate
  title: ''
  type: StatusPage
  url: https://status.oneatlas.airbus.com/
- group: operate
  title: ''
  type: ChangeLog
  url: https://api.oneatlas.airbus.com/releases/
- group: start
  title: ''
  type: Portal
  url: https://api.oneatlas.airbus.com/
- group: build
  title: ''
  type: GitHub
  url: https://github.com/airbusgeo
- group: company
  title: ''
  type: LinkedIn
  url: https://www.linkedin.com/company/airbus
- group: company
  title: ''
  type: Website
  url: https://www.airbus.com/
- group: design
  title: ''
  type: SpectralRules
  url: rules/oneatlas-spectral-rules.yml
- group: design
  title: ''
  type: Vocabulary
  url: vocabulary/oneatlas-vocabulary.yaml
- group: design
  title: ''
  type: JSONLD
  url: json-ld/oneatlas-context.jsonld
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-acquisition-date-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-search-options-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-search-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-small-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-stage-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-stages-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-api-key-page-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-api-key-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-associated-data-link-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-band-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-bbox-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-buffer-description-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-catalog-delete-expired-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-catalog-item-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-control-job-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-delete-operation-result-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-deleted-item-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-delivery-pneo-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-delivery-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-dem-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-describe-process-input-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-describe-process-links-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-describe-process-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-dimap-v2-packaging-parameters-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-email-notification-parameters-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-feasibility-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-features-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-footprint-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-format-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-linestring-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-multilinestring-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-multipoint-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-multipolygon-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-point-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-polygon-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geojson-geometry-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geometry-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-get-domain-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-histogram-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-human-readable-operation-summary-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-image-size-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-ingestion-job-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-link-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-md5-checksum-parameters-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-messagequeue-notification-parameters-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-metadata-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-notification-search-options-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-notification-search-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-notification-subscription-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-optical-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-pixel-coding-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-point-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-privilege-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-privilege-search-options-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-privilege-search-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-hal-catalog-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-hal-link-catalog-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-hal-link-common-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-hal-link-describe-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-processes-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-response-buffer-information-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-deleted-options-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-deleted-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-options-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-options-v2-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-segment-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-srs-expression-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-srs-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-step-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-target-workspace-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-tasking-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-update-options-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-update-response-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-webhook-notification-parameters-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-workspace-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-workspace-search-options-schema.json
- group: docs
  title: ''
  type: JSONSchema
  url: json-schema/oneatlas-workspace-search-response-schema.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-acquisition-date-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-search-options-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-search-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-small-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-stage-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-stages-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-api-key-page-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-api-key-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-associated-data-link-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-band-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-bbox-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-buffer-description-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-catalog-delete-expired-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-catalog-item-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-control-job-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-delete-operation-result-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-deleted-item-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-delivery-pneo-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-delivery-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-dem-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-describe-process-input-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-describe-process-links-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-describe-process-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-dimap-v2-packaging-parameters-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-email-notification-parameters-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-feasibility-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-features-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-footprint-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-format-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-linestring-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-multilinestring-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-multipoint-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-multipolygon-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-point-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-polygon-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geojson-geometry-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geometry-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-get-domain-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-histogram-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-human-readable-operation-summary-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-image-size-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-ingestion-job-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-link-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-md5-checksum-parameters-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-messagequeue-notification-parameters-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-metadata-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-notification-search-options-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-notification-search-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-notification-subscription-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-optical-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-pixel-coding-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-point-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-privilege-search-options-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-privilege-search-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-privilege-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-hal-catalog-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-hal-link-catalog-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-hal-link-common-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-hal-link-describe-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-processes-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-response-buffer-information-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-deleted-options-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-deleted-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-options-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-options-v2-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-segment-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-srs-expression-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-srs-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-step-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-target-workspace-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-tasking-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-update-options-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-update-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-webhook-notification-parameters-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-workspace-search-options-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-workspace-search-response-structure.json
- group: design
  title: ''
  type: JSONStructure
  url: json-structure/oneatlas-workspace-structure.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-acquisition-date-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-activity-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-activity-search-options-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-activity-search-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-activity-small-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-activity-stage-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-activity-stages-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-api-key-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-api-key-page-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-associated-data-link-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-band-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-bbox-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-buffer-description-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-catalog-delete-expired-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-catalog-item-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-control-job-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-delete-operation-result-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-deleted-item-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-delivery-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-delivery-pneo-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-dem-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-describe-process-input-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-describe-process-links-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-describe-process-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-dimap-v2-packaging-parameters-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-email-notification-parameters-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-feasibility-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-features-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-footprint-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-format-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-geo-json-linestring-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-geo-json-multilinestring-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-geo-json-multipoint-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-geo-json-multipolygon-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-geo-json-point-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-geo-json-polygon-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-geojson-geometry-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-geometry-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-get-domain-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-histogram-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-human-readable-operation-summary-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-image-size-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-ingestion-job-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-link-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-md5-checksum-parameters-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-messagequeue-notification-parameters-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-metadata-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-notification-search-options-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-notification-search-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-notification-subscription-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-optical-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-pixel-coding-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-point-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-privilege-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-privilege-search-options-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-privilege-search-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-process-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-process-hal-catalog-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-process-hal-link-catalog-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-process-hal-link-common-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-process-hal-link-describe-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-processes-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-response-buffer-information-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-search-deleted-options-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-search-deleted-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-search-options-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-search-options-v2-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-search-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-segment-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-srs-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-srs-expression-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-step-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-target-workspace-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-tasking-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-update-options-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-update-response-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-webhook-notification-parameters-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-workspace-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-workspace-search-options-example.json
- group: build
  title: ''
  type: Examples
  url: examples/oneatlas-workspace-search-response-example.json
created: '2025-02-06'
description: Airbus OneAtlas is the commercial geospatial platform from Airbus Defence and Space, providing developer APIs for accessing high-resolution satellite imagery, elevation data, basemaps, and radar data. The platform covers optical imagery via the Living Library and Pay-Per-Order services, satellite tasking for new acquisitions, WorldDEM global elevation data, SAR radar imagery, and seamless basemap tiles.
examples:
- key_count: 6
  name: Airbus Oneatlas Get Sarcataloguereplication Example
  slug: airbus-oneatlas-get-sarcataloguereplication-example
- key_count: 6
  name: Airbus Oneatlas Get Sarcataloguerevocation Example
  slug: airbus-oneatlas-get-sarcataloguerevocation-example
- key_count: 6
  name: Airbus Oneatlas Post Sarfeasibility Example
  slug: airbus-oneatlas-post-sarfeasibility-example
- key_count: 2
  name: Oneatlas Acquisition Date Example
  slug: oneatlas-acquisition-date-example
- key_count: 13
  name: Oneatlas Activity Example
  slug: oneatlas-activity-example
- key_count: 11
  name: Oneatlas Activity Search Options Example
  slug: oneatlas-activity-search-options-example
- key_count: 4
  name: Oneatlas Activity Search Response Example
  slug: oneatlas-activity-search-response-example
- key_count: 2
  name: Oneatlas Activity Small Example
  slug: oneatlas-activity-small-example
- key_count: 5
  name: Oneatlas Activity Stage Example
  slug: oneatlas-activity-stage-example
- key_count: 1
  name: Oneatlas Activity Stages Example
  slug: oneatlas-activity-stages-example
- key_count: 5
  name: Oneatlas Api Key Example
  slug: oneatlas-api-key-example
- key_count: 1
  name: Oneatlas Api Key Page Example
  slug: oneatlas-api-key-page-example
- key_count: 5
  name: Oneatlas Associated Data Link Example
  slug: oneatlas-associated-data-link-example
- key_count: 10
  name: Oneatlas Band Example
  slug: oneatlas-band-example
- key_count: 3
  name: Oneatlas Bbox Example
  slug: oneatlas-bbox-example
- key_count: 4
  name: Oneatlas Buffer Description Example
  slug: oneatlas-buffer-description-example
- key_count: 1
  name: Oneatlas Catalog Delete Expired Example
  slug: oneatlas-catalog-delete-expired-example
- key_count: 5
  name: Oneatlas Catalog Item Example
  slug: oneatlas-catalog-item-example
- key_count: 3
  name: Oneatlas Control Job Example
  slug: oneatlas-control-job-example
- key_count: 4
  name: Oneatlas Delete Operation Result Example
  slug: oneatlas-delete-operation-result-example
- key_count: 2
  name: Oneatlas Deleted Item Example
  slug: oneatlas-deleted-item-example
- key_count: 3
  name: Oneatlas Delivery Example
  slug: oneatlas-delivery-example
- key_count: 1
  name: Oneatlas Delivery Pneo Example
  slug: oneatlas-delivery-pneo-example
- key_count: 2
  name: Oneatlas Dem Example
  slug: oneatlas-dem-example
- key_count: 3
  name: Oneatlas Describe Process Input Example
  slug: oneatlas-describe-process-input-example
- key_count: 0
  name: Oneatlas Describe Process Links Example
  slug: oneatlas-describe-process-links-example
- key_count: 7
  name: Oneatlas Describe Process Response Example
  slug: oneatlas-describe-process-response-example
- key_count: 1
  name: Oneatlas Dimap V2 Packaging Parameters Example
  slug: oneatlas-dimap-v2-packaging-parameters-example
- key_count: 1
  name: Oneatlas Email Notification Parameters Example
  slug: oneatlas-email-notification-parameters-example
- key_count: 1
  name: Oneatlas Feasibility Example
  slug: oneatlas-feasibility-example
- key_count: 2
  name: Oneatlas Features Example
  slug: oneatlas-features-example
- key_count: 2
  name: Oneatlas Footprint Example
  slug: oneatlas-footprint-example
- key_count: 3
  name: Oneatlas Format Example
  slug: oneatlas-format-example
- key_count: 3
  name: Oneatlas Geo Json Linestring Example
  slug: oneatlas-geo-json-linestring-example
- key_count: 3
  name: Oneatlas Geo Json Multilinestring Example
  slug: oneatlas-geo-json-multilinestring-example
- key_count: 3
  name: Oneatlas Geo Json Multipoint Example
  slug: oneatlas-geo-json-multipoint-example
- key_count: 3
  name: Oneatlas Geo Json Multipolygon Example
  slug: oneatlas-geo-json-multipolygon-example
- key_count: 3
  name: Oneatlas Geo Json Point Example
  slug: oneatlas-geo-json-point-example
- key_count: 3
  name: Oneatlas Geo Json Polygon Example
  slug: oneatlas-geo-json-polygon-example
- key_count: 2
  name: Oneatlas Geojson Geometry Example
  slug: oneatlas-geojson-geometry-example
- key_count: 2
  name: Oneatlas Geometry Example
  slug: oneatlas-geometry-example
- key_count: 1
  name: Oneatlas Get Domain Response Example
  slug: oneatlas-get-domain-response-example
- key_count: 1
  name: Oneatlas Histogram Response Example
  slug: oneatlas-histogram-response-example
- key_count: 1
  name: Oneatlas Human Readable Operation Summary Example
  slug: oneatlas-human-readable-operation-summary-example
- key_count: 2
  name: Oneatlas Image Size Example
  slug: oneatlas-image-size-example
- key_count: 5
  name: Oneatlas Ingestion Job Example
  slug: oneatlas-ingestion-job-example
- key_count: 5
  name: Oneatlas Link Example
  slug: oneatlas-link-example
- key_count: 1
  name: Oneatlas Md5 Checksum Parameters Example
  slug: oneatlas-md5-checksum-parameters-example
- key_count: 3
  name: Oneatlas Messagequeue Notification Parameters Example
  slug: oneatlas-messagequeue-notification-parameters-example
- key_count: 4
  name: Oneatlas Metadata Example
  slug: oneatlas-metadata-example
- key_count: 1
  name: Oneatlas Notification Search Options Example
  slug: oneatlas-notification-search-options-example
- key_count: 1
  name: Oneatlas Notification Search Response Example
  slug: oneatlas-notification-search-response-example
- key_count: 5
  name: Oneatlas Notification Subscription Example
  slug: oneatlas-notification-subscription-example
- key_count: 3
  name: Oneatlas Optical Example
  slug: oneatlas-optical-example
- key_count: 2
  name: Oneatlas Pixel Coding Example
  slug: oneatlas-pixel-coding-example
- key_count: 2
  name: Oneatlas Point Example
  slug: oneatlas-point-example
- key_count: 7
  name: Oneatlas Privilege Example
  slug: oneatlas-privilege-example
- key_count: 1
  name: Oneatlas Privilege Search Options Example
  slug: oneatlas-privilege-search-options-example
- key_count: 1
  name: Oneatlas Privilege Search Response Example
  slug: oneatlas-privilege-search-response-example
- key_count: 5
  name: Oneatlas Process Example
  slug: oneatlas-process-example
- key_count: 0
  name: Oneatlas Process Hal Catalog Example
  slug: oneatlas-process-hal-catalog-example
- key_count: 0
  name: Oneatlas Process Hal Link Catalog Example
  slug: oneatlas-process-hal-link-catalog-example
- key_count: 3
  name: Oneatlas Process Hal Link Common Example
  slug: oneatlas-process-hal-link-common-example
- key_count: 0
  name: Oneatlas Process Hal Link Describe Example
  slug: oneatlas-process-hal-link-describe-example
- key_count: 2
  name: Oneatlas Processes Response Example
  slug: oneatlas-processes-response-example
- key_count: 3
  name: Oneatlas Response Buffer Information Example
  slug: oneatlas-response-buffer-information-example
- key_count: 3
  name: Oneatlas Search Deleted Options Example
  slug: oneatlas-search-deleted-options-example
- key_count: 4
  name: Oneatlas Search Deleted Response Example
  slug: oneatlas-search-deleted-response-example
- key_count: 23
  name: Oneatlas Search Options Example
  slug: oneatlas-search-options-example
- key_count: 0
  name: Oneatlas Search Options V2 Example
  slug: oneatlas-search-options-v2-example
- key_count: 6
  name: Oneatlas Search Response Example
  slug: oneatlas-search-response-example
- key_count: 11
  name: Oneatlas Segment Example
  slug: oneatlas-segment-example
- key_count: 2
  name: Oneatlas Srs Example
  slug: oneatlas-srs-example
- key_count: 2
  name: Oneatlas Srs Expression Example
  slug: oneatlas-srs-expression-example
- key_count: 3
  name: Oneatlas Step Example
  slug: oneatlas-step-example
- key_count: 2
  name: Oneatlas Target Workspace Example
  slug: oneatlas-target-workspace-example
- key_count: 20
  name: Oneatlas Tasking Example
  slug: oneatlas-tasking-example
- key_count: 2
  name: Oneatlas Update Options Example
  slug: oneatlas-update-options-example
- key_count: 1
  name: Oneatlas Update Response Example
  slug: oneatlas-update-response-example
- key_count: 1
  name: Oneatlas Webhook Notification Parameters Example
  slug: oneatlas-webhook-notification-parameters-example
- key_count: 7
  name: Oneatlas Workspace Example
  slug: oneatlas-workspace-example
- key_count: 2
  name: Oneatlas Workspace Search Options Example
  slug: oneatlas-workspace-search-options-example
- key_count: 1
  name: Oneatlas Workspace Search Response Example
  slug: oneatlas-workspace-search-response-example
finops:
- name: Airbus Oneatlas Finops
  service_category: API
  slug: airbus-oneatlas-finops
image: https://kinlane-images.s3.amazonaws.com/shared/apis-json/icons/airbus-oneatlas.png
json_schemas:
- name: AcquisitionDate
  property_count: 2
  slug: oneatlas-acquisition-date
- name: Activity
  property_count: 13
  slug: oneatlas-activity
- name: ActivitySearchOptions
  property_count: 11
  slug: oneatlas-activity-search-options
- name: ActivitySearchResponse
  property_count: 4
  slug: oneatlas-activity-search-response
- name: ActivitySmall
  property_count: 2
  slug: oneatlas-activity-small
- name: ActivityStage
  property_count: 5
  slug: oneatlas-activity-stage
- name: ActivityStages
  property_count: 1
  slug: oneatlas-activity-stages
- name: ApiKeyPage
  property_count: 1
  slug: oneatlas-api-key-page
- name: ApiKey
  property_count: 5
  slug: oneatlas-api-key
- name: AssociatedDataLink
  property_count: 5
  slug: oneatlas-associated-data-link
- name: band
  property_count: 10
  slug: oneatlas-band
- name: bbox
  property_count: 3
  slug: oneatlas-bbox
- name: bufferDescription
  property_count: 4
  slug: oneatlas-buffer-description
- name: catalog-delete-expired
  property_count: 1
  slug: oneatlas-catalog-delete-expired
- name: CatalogItem
  property_count: 5
  slug: oneatlas-catalog-item
- name: ControlJob
  property_count: 3
  slug: oneatlas-control-job
- name: DeleteOperationResult
  property_count: 4
  slug: oneatlas-delete-operation-result
- name: deleted-item
  property_count: 2
  slug: oneatlas-deleted-item
- name: deliveryPNEO
  property_count: 1
  slug: oneatlas-delivery-pneo
- name: delivery
  property_count: 3
  slug: oneatlas-delivery
- name: dem
  property_count: 2
  slug: oneatlas-dem
- name: DescribeProcessInput
  property_count: 3
  slug: oneatlas-describe-process-input
- name: DescribeProcessLinks
  property_count: 0
  slug: oneatlas-describe-process-links
- name: DescribeProcessResponse
  property_count: 7
  slug: oneatlas-describe-process-response
- name: DimapV2PackagingParameters
  property_count: 1
  slug: oneatlas-dimap-v2-packaging-parameters
- name: EmailNotificationParameters
  property_count: 1
  slug: oneatlas-email-notification-parameters
- name: Feasibility
  property_count: 1
  slug: oneatlas-feasibility
- name: Features
  property_count: 2
  slug: oneatlas-features
- name: footprint
  property_count: 2
  slug: oneatlas-footprint
- name: Format
  property_count: 3
  slug: oneatlas-format
- name: geo-json-linestring
  property_count: 3
  slug: oneatlas-geo-json-linestring
- name: geo-json-multilinestring
  property_count: 3
  slug: oneatlas-geo-json-multilinestring
- name: geo-json-multipoint
  property_count: 3
  slug: oneatlas-geo-json-multipoint
- name: geo-json-multipolygon
  property_count: 3
  slug: oneatlas-geo-json-multipolygon
- name: geo-json-point
  property_count: 3
  slug: oneatlas-geo-json-point
- name: geo-json-polygon
  property_count: 3
  slug: oneatlas-geo-json-polygon
- name: GeojsonGeometry
  property_count: 2
  slug: oneatlas-geojson-geometry
- name: geometry
  property_count: 2
  slug: oneatlas-geometry
- name: get-domain-response
  property_count: 1
  slug: oneatlas-get-domain-response
- name: HistogramResponse
  property_count: 1
  slug: oneatlas-histogram-response
- name: HumanReadableOperationSummary
  property_count: 1
  slug: oneatlas-human-readable-operation-summary
- name: imageSize
  property_count: 2
  slug: oneatlas-image-size
- name: IngestionJob
  property_count: 5
  slug: oneatlas-ingestion-job
- name: Link
  property_count: 5
  slug: oneatlas-link
- name: Md5ChecksumParameters
  property_count: 1
  slug: oneatlas-md5-checksum-parameters
- name: MessagequeueNotificationParameters
  property_count: 3
  slug: oneatlas-messagequeue-notification-parameters
- name: Metadata
  property_count: 4
  slug: oneatlas-metadata
- name: NotificationSearchOptions
  property_count: 1
  slug: oneatlas-notification-search-options
- name: NotificationSearchResponse
  property_count: 1
  slug: oneatlas-notification-search-response
- name: NotificationSubscription
  property_count: 5
  slug: oneatlas-notification-subscription
- name: Optical
  property_count: 3
  slug: oneatlas-optical
- name: pixelCoding
  property_count: 2
  slug: oneatlas-pixel-coding
- name: point
  property_count: 2
  slug: oneatlas-point
- name: Privilege
  property_count: 7
  slug: oneatlas-privilege
- name: PrivilegeSearchOptions
  property_count: 1
  slug: oneatlas-privilege-search-options
- name: PrivilegeSearchResponse
  property_count: 1
  slug: oneatlas-privilege-search-response
- name: processHALCatalog
  property_count: 0
  slug: oneatlas-process-hal-catalog
- name: processHALLinkCatalog
  property_count: 0
  slug: oneatlas-process-hal-link-catalog
- name: processHALLinkCommon
  property_count: 3
  slug: oneatlas-process-hal-link-common
- name: processHALLinkDescribe
  property_count: 0
  slug: oneatlas-process-hal-link-describe
- name: process
  property_count: 5
  slug: oneatlas-process
- name: ProcessesResponse
  property_count: 2
  slug: oneatlas-processes-response
- name: responseBufferInformation
  property_count: 3
  slug: oneatlas-response-buffer-information
- name: search-deleted-options
  property_count: 3
  slug: oneatlas-search-deleted-options
- name: search-deleted-response
  property_count: 4
  slug: oneatlas-search-deleted-response
- name: SearchOptions
  property_count: 23
  slug: oneatlas-search-options
- name: search-options-v2
  property_count: 0
  slug: oneatlas-search-options-v2
- name: SearchResponse
  property_count: 6
  slug: oneatlas-search-response
- name: segment
  property_count: 11
  slug: oneatlas-segment
- name: srsExpression
  property_count: 2
  slug: oneatlas-srs-expression
- name: srs
  property_count: 2
  slug: oneatlas-srs
- name: step
  property_count: 3
  slug: oneatlas-step
- name: TargetWorkspace
  property_count: 2
  slug: oneatlas-target-workspace
- name: tasking
  property_count: 20
  slug: oneatlas-tasking
- name: update-options
  property_count: 2
  slug: oneatlas-update-options
- name: update-response
  property_count: 1
  slug: oneatlas-update-response
- name: WebhookNotificationParameters
  property_count: 1
  slug: oneatlas-webhook-notification-parameters
- name: Workspace
  property_count: 7
  slug: oneatlas-workspace
- name: WorkspaceSearchOptions
  property_count: 2
  slug: oneatlas-workspace-search-options
- name: WorkspaceSearchResponse
  property_count: 1
  slug: oneatlas-workspace-search-response
json_structures:
- name: Oneatlas Acquisition Date Structure
  property_count: 0
  slug: oneatlas-acquisition-date-structure
- name: Oneatlas Activity Search Options Structure
  property_count: 0
  slug: oneatlas-activity-search-options-structure
- name: Oneatlas Activity Search Response Structure
  property_count: 0
  slug: oneatlas-activity-search-response-structure
- name: Oneatlas Activity Small Structure
  property_count: 0
  slug: oneatlas-activity-small-structure
- name: Oneatlas Activity Stage Structure
  property_count: 0
  slug: oneatlas-activity-stage-structure
- name: Oneatlas Activity Stages Structure
  property_count: 0
  slug: oneatlas-activity-stages-structure
- name: Oneatlas Activity Structure
  property_count: 0
  slug: oneatlas-activity-structure
- name: Oneatlas Api Key Page Structure
  property_count: 0
  slug: oneatlas-api-key-page-structure
- name: Oneatlas Api Key Structure
  property_count: 0
  slug: oneatlas-api-key-structure
- name: Oneatlas Associated Data Link Structure
  property_count: 0
  slug: oneatlas-associated-data-link-structure
- name: Oneatlas Band Structure
  property_count: 0
  slug: oneatlas-band-structure
- name: Oneatlas Bbox Structure
  property_count: 0
  slug: oneatlas-bbox-structure
- name: Oneatlas Buffer Description Structure
  property_count: 0
  slug: oneatlas-buffer-description-structure
- name: Oneatlas Catalog Delete Expired Structure
  property_count: 0
  slug: oneatlas-catalog-delete-expired-structure
- name: Oneatlas Catalog Item Structure
  property_count: 0
  slug: oneatlas-catalog-item-structure
- name: Oneatlas Control Job Structure
  property_count: 0
  slug: oneatlas-control-job-structure
- name: Oneatlas Delete Operation Result Structure
  property_count: 0
  slug: oneatlas-delete-operation-result-structure
- name: Oneatlas Deleted Item Structure
  property_count: 0
  slug: oneatlas-deleted-item-structure
- name: Oneatlas Delivery Pneo Structure
  property_count: 0
  slug: oneatlas-delivery-pneo-structure
- name: Oneatlas Delivery Structure
  property_count: 0
  slug: oneatlas-delivery-structure
- name: Oneatlas Dem Structure
  property_count: 0
  slug: oneatlas-dem-structure
- name: Oneatlas Describe Process Input Structure
  property_count: 0
  slug: oneatlas-describe-process-input-structure
- name: Oneatlas Describe Process Links Structure
  property_count: 0
  slug: oneatlas-describe-process-links-structure
- name: Oneatlas Describe Process Response Structure
  property_count: 0
  slug: oneatlas-describe-process-response-structure
- name: Oneatlas Dimap V2 Packaging Parameters Structure
  property_count: 0
  slug: oneatlas-dimap-v2-packaging-parameters-structure
- name: Oneatlas Email Notification Parameters Structure
  property_count: 0
  slug: oneatlas-email-notification-parameters-structure
- name: Oneatlas Feasibility Structure
  property_count: 0
  slug: oneatlas-feasibility-structure
- name: Oneatlas Features Structure
  property_count: 0
  slug: oneatlas-features-structure
- name: Oneatlas Footprint Structure
  property_count: 0
  slug: oneatlas-footprint-structure
- name: Oneatlas Format Structure
  property_count: 0
  slug: oneatlas-format-structure
- name: Oneatlas Geo Json Linestring Structure
  property_count: 0
  slug: oneatlas-geo-json-linestring-structure
- name: Oneatlas Geo Json Multilinestring Structure
  property_count: 0
  slug: oneatlas-geo-json-multilinestring-structure
- name: Oneatlas Geo Json Multipoint Structure
  property_count: 0
  slug: oneatlas-geo-json-multipoint-structure
- name: Oneatlas Geo Json Multipolygon Structure
  property_count: 0
  slug: oneatlas-geo-json-multipolygon-structure
- name: Oneatlas Geo Json Point Structure
  property_count: 0
  slug: oneatlas-geo-json-point-structure
- name: Oneatlas Geo Json Polygon Structure
  property_count: 0
  slug: oneatlas-geo-json-polygon-structure
- name: Oneatlas Geojson Geometry Structure
  property_count: 0
  slug: oneatlas-geojson-geometry-structure
- name: Oneatlas Geometry Structure
  property_count: 0
  slug: oneatlas-geometry-structure
- name: Oneatlas Get Domain Response Structure
  property_count: 0
  slug: oneatlas-get-domain-response-structure
- name: Oneatlas Histogram Response Structure
  property_count: 0
  slug: oneatlas-histogram-response-structure
- name: Oneatlas Human Readable Operation Summary Structure
  property_count: 0
  slug: oneatlas-human-readable-operation-summary-structure
- name: Oneatlas Image Size Structure
  property_count: 0
  slug: oneatlas-image-size-structure
- name: Oneatlas Ingestion Job Structure
  property_count: 0
  slug: oneatlas-ingestion-job-structure
- name: Oneatlas Link Structure
  property_count: 0
  slug: oneatlas-link-structure
- name: Oneatlas Md5 Checksum Parameters Structure
  property_count: 0
  slug: oneatlas-md5-checksum-parameters-structure
- name: Oneatlas Messagequeue Notification Parameters Structure
  property_count: 0
  slug: oneatlas-messagequeue-notification-parameters-structure
- name: Oneatlas Metadata Structure
  property_count: 0
  slug: oneatlas-metadata-structure
- name: Oneatlas Notification Search Options Structure
  property_count: 0
  slug: oneatlas-notification-search-options-structure
- name: Oneatlas Notification Search Response Structure
  property_count: 0
  slug: oneatlas-notification-search-response-structure
- name: Oneatlas Notification Subscription Structure
  property_count: 0
  slug: oneatlas-notification-subscription-structure
- name: Oneatlas Optical Structure
  property_count: 0
  slug: oneatlas-optical-structure
- name: Oneatlas Pixel Coding Structure
  property_count: 0
  slug: oneatlas-pixel-coding-structure
- name: Oneatlas Point Structure
  property_count: 0
  slug: oneatlas-point-structure
- name: Oneatlas Privilege Search Options Structure
  property_count: 0
  slug: oneatlas-privilege-search-options-structure
- name: Oneatlas Privilege Search Response Structure
  property_count: 0
  slug: oneatlas-privilege-search-response-structure
- name: Oneatlas Privilege Structure
  property_count: 0
  slug: oneatlas-privilege-structure
- name: Oneatlas Process Hal Catalog Structure
  property_count: 0
  slug: oneatlas-process-hal-catalog-structure
- name: Oneatlas Process Hal Link Catalog Structure
  property_count: 0
  slug: oneatlas-process-hal-link-catalog-structure
- name: Oneatlas Process Hal Link Common Structure
  property_count: 0
  slug: oneatlas-process-hal-link-common-structure
- name: Oneatlas Process Hal Link Describe Structure
  property_count: 0
  slug: oneatlas-process-hal-link-describe-structure
- name: Oneatlas Process Structure
  property_count: 0
  slug: oneatlas-process-structure
- name: Oneatlas Processes Response Structure
  property_count: 0
  slug: oneatlas-processes-response-structure
- name: Oneatlas Response Buffer Information Structure
  property_count: 0
  slug: oneatlas-response-buffer-information-structure
- name: Oneatlas Search Deleted Options Structure
  property_count: 0
  slug: oneatlas-search-deleted-options-structure
- name: Oneatlas Search Deleted Response Structure
  property_count: 0
  slug: oneatlas-search-deleted-response-structure
- name: Oneatlas Search Options Structure
  property_count: 0
  slug: oneatlas-search-options-structure
- name: Oneatlas Search Options V2 Structure
  property_count: 0
  slug: oneatlas-search-options-v2-structure
- name: Oneatlas Search Response Structure
  property_count: 0
  slug: oneatlas-search-response-structure
- name: Oneatlas Segment Structure
  property_count: 0
  slug: oneatlas-segment-structure
- name: Oneatlas Srs Expression Structure
  property_count: 0
  slug: oneatlas-srs-expression-structure
- name: Oneatlas Srs Structure
  property_count: 0
  slug: oneatlas-srs-structure
- name: Oneatlas Step Structure
  property_count: 0
  slug: oneatlas-step-structure
- name: Oneatlas Target Workspace Structure
  property_count: 0
  slug: oneatlas-target-workspace-structure
- name: Oneatlas Tasking Structure
  property_count: 0
  slug: oneatlas-tasking-structure
- name: Oneatlas Update Options Structure
  property_count: 0
  slug: oneatlas-update-options-structure
- name: Oneatlas Update Response Structure
  property_count: 0
  slug: oneatlas-update-response-structure
- name: Oneatlas Webhook Notification Parameters Structure
  property_count: 0
  slug: oneatlas-webhook-notification-parameters-structure
- name: Oneatlas Workspace Search Options Structure
  property_count: 0
  slug: oneatlas-workspace-search-options-structure
- name: Oneatlas Workspace Search Response Structure
  property_count: 0
  slug: oneatlas-workspace-search-response-structure
- name: Oneatlas Workspace Structure
  property_count: 0
  slug: oneatlas-workspace-structure
jsonld:
- class_count: 0
  name: Oneatlas Context
  property_count: 18
  slug: oneatlas-context
layout: provider
modified: '2026-05-19'
name: Airbus OneAtlas
nav: Providers
network: true
overview: 'Airbus OneAtlas publishes 49 APIs on the [APIs.io](https://apis.io/) network, including OneAtlas Data Living Library, OneAtlas Data Pay-Per-Order API, OneAtlas Data Pay-Per-Order Tasking API, and 46 more. Tagged areas include Imagery and Satellites.


  The Airbus OneAtlas catalog on APIs.io includes 1 JSON-LD context and 3 Spectral governance rulesets.


  Airbus OneAtlas'' developer surface includes authentication, getting-started guide, changelog, developer portal, GitHub presence, code examples, and 251 more developer resources.'
plans:
- name: Airbus Oneatlas Plans Pricing
  plan_count: 4
  slug: airbus-oneatlas-plans-pricing
random_paper: 13
rate_limits:
- limit_count: 5
  name: Airbus Oneatlas Rate Limits
  slug: airbus-oneatlas-rate-limits
rules:
- effective_rule_count: 5
  extends: []
  name: Airbus OneAtlas API Rules
  rule_count: 5
  severity_counts:
    error: 0
    hint: 0
    info: 2
    warn: 3
  slug: airbus-oneatlas-jsonschema-spectral-rules
- effective_rule_count: 50
  extends:
  - spectral:oas
  name: Airbus OneAtlas API Rules
  rule_count: 9
  severity_counts:
    error: 1
    hint: 0
    info: 2
    warn: 6
  slug: airbus-oneatlas-spectral-rules
- effective_rule_count: 12
  extends: []
  name: Airbus OneAtlas API Rules
  rule_count: 12
  severity_counts:
    error: 4
    hint: 0
    info: 0
    warn: 8
  slug: oneatlas-spectral-rules
score:
  band: thin
  composite: 39.1
  coverage:
    artifact_dirs: 16
    catalog_gap: 61.5
    catalog_max: 115.0
    note: Disclosure, not a penalty. catalog_gap is rubric points API Evangelist could add with no action by this provider; it is our backlog, not their gap, and it is NOT subtracted from the composite above.
  delta: 0.0
  facets:
    access_clarity: 15.8
    commercial_clarity: 15.8
    contract_governance: 28.8
    contract_quality: 56.4
    developer_ergonomics: 38.1
    discoverability: 50.0
    governance: 28.8
    operational_transparency: 44.7
  previous_composite: 39.1
  provenance:
    agentic_access: derived
    contracts:
      callable: 100.0
      derived: 0
      marker_coverage: 0.0
      total: 44
  schema_version: 0.17.2
  scored_at: '2026-08-30'
  trend: flat
screenshot: https://raw.githubusercontent.com/api-evangelist/airbus-oneatlas/refs/heads/main/screenshots/airbus-oneatlas-2026-06-20T171419.png
security:
- kind: authentication
  name: Airbus Oneatlas Authentication
  slug: airbus-oneatlas-authentication
  summary_line: apiKey/http · 2 schemes
- kind: domain-security
  name: Airbus Oneatlas Domain Security
  slug: airbus-oneatlas-domain-security
  summary_line: TLSv1.3 · HSTS · DMARC
- kind: vulnerability-disclosure
  name: Airbus Oneatlas Vulnerability Disclosure
  slug: airbus-oneatlas-vulnerability-disclosure
  summary_line: security.txt · contact published
slug: airbus-oneatlas
tags:
- Imagery
- Satellites
website: https://www.airbus.com/
---
